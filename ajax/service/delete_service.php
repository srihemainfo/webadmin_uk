...
if ($method == "list_agent") {
    ...
    if ($user_register['nr'] > 0) {
        ...
        foreach ($user_register['result'] as $key => $value) {
            $FRONT_img = select_query($rcon, "user_images", "", "`user_id`='$value[id]' and `img_url` != '' and `type` = 'FRONT' and `status`='0' and `deletes`='1' ORDER BY `id` DESC LIMIT 1", "", "");
            $BACK_img = select_query($rcon, "user_images", "", "`user_id`='$value[id]' and `img_url` != '' and `type` = 'BACK' and `status`='0' and `deletes`='1' ORDER BY `id` DESC LIMIT 1", "", "");
            ...
            if ($roll_id == 1) {
                ...
                } else if (in_array($value['roll_id'], [2, 3, 4, 5])) {
            ...
            $result[] = ["buildinglocation" => utf8_encode($value['address']), "Country" => utf8_encode($value['nationality']), "City" => utf8_encode($value['city']), "bulidingname" => $value['building_name'], "createdat" => $value['created_at'], "action" => $action, "id" => $value['id'], "nid" => str_pad($value['id'], 7, "0", STR_PAD_LEFT), "rollType" => $value['user'], "name" => $value['name'] . ' ' . $value['lname'], "t_point" => $value['t_point'], "mobile" => $value['mobile'], "passport" => utf8_encode($value['passport']), "email" => $value['email']];
        }
    }
    ...
    // var_dump("SELECT nd.*, ur.name, ur.lname, ur.email FROM `ndticket` nd JOIN `user_register` ur ON nd.userId = ur.id WHERE " . $contype . "  nd.id != ''");die;
    // $NDticketquery = mysqli_query($con, "SELECT * FROM `ndticket` WHERE $contype `id`!= ''");
    
    if($userType1 == 'agent'){
        
        // var_dump('test1');die;
    
    $NDticketquery = mysqli_query($con, "SELECT nd.*, ur.name, ur.lname, ur.mobile, ur.email,ur2.name AS agentName,nd FROM `ndticket` nd JOIN `user_register` ur ON nd.userId = ur.id JOIN `user_register` ur2 ON nd.agentId = ur2.id WHERE " . $contype . " nd.agentId != 0 AND nd.id != '' AND nd.deletes = '1'");
    
    } else{
        
         //var_dump("SELECT nd.*, ur.name, ur.lname, ur.mobile, ur.email, '' as agentName FROM `ndticket` nd JOIN `user_register` ur ON nd.userId = ur.id WHERE " . $contype . " nd.agentId = 0 AND nd.id != '' AND nd.deletes = '1' LIMIT 0, 25");
         //die;
        ...
        // var_dump("SELECT nd.*, ur.name, ur.lname, ur.mobile, ur.email,ur2.name AS agentName FROM `ndticket` nd JOIN `user_register` ur ON nd.userId = ur.id JOIN `user_register` ur2 ON nd.agentId = ur2.id WHERE " . $contype . " nd.agentId = 0 AND nd.id != '' AND nd.deletes != '1'");die;
...
// var_dump($result);die;
    ...
    $message = $_POST['message'];
       ...
       $inv_arr = array("deletes" => '0', "RevokeReason" => $message);
    //   $ND_update = update($con, "ndticket", "`id` = '$transid'", $inv_arr, "", "", "", "1");
        $ndticket_update = update($con, "ndticket", "`id` = '$transid'", ["deletes" => '0', "deleteReason" => $message], "", "", "", "");
        
        $invoice_update = update($con, "invoice", "`ticketId` = '$transid'", ["deletes" => '0'], "", "", "", "");
    

        
         if ( $ndticket_update) {
            $result["result"] = "Ticket has been Revoked Successfully";
            $result["type"] = '1';
        } else {
...
// var_dump($result);die;
    ...
    if ($Ticket_lines['nr'] > 0) {
        ...
        foreach ($Ticket_lines['result'] as $key => $value) {
            ...
            if ($_SESSION['memid'] == 1 || $roll_id != 11) {
                ...
                if ($roll_id == 1) {
                    $action .= '<a class="btn text-danger btn-sm" style="cursor: pointer;" data-bs-original-title="Delete"><span class="fa fa-trash" style="color: red;font-size: 18px;" onclick="revokeTicket(' . "'$transaction_id'" . ')"></span></a>';
                }
            }
    ...
    if ($gatewayname != '') {
        ...
        // else 
        if ($gatewayname == 'cequens' || $gatewayname == 'dataslice' || $gatewayname == 'expresso' || $gatewayname == 'precise' || $gatewayname == 'firebase' || $gatewayname == 'twilio') {
    ...
    $SMSquery = mysqli_query($con, "SELECT * FROM `smslog` WHERE $gatcon $contype `id`!= '' AND  `gateway` IN ('cequens', 'dataslice', 'expresso', 'precise', 'firebase', 'twilio')");
...
///////// NEW //////////
    ...
    try {
        ...
        if ($id != '') {
            ...
            if ($img_url != '') {
                $result['img'][] = (strpos($img_url, "nationaldraw") === 0) ? $baseurl . $img_url : constant('assetURL') . $img_url;
            }
            ...
            if ($FRONT_img['result'][0]['img_url'] != '') {
                $result['img'][] = (strpos($img_url, "nationaldraw") === 0) ? $baseurl . $FRONT_img['result'][0]['img_url'] : constant('assetURL') . $FRONT_img['result'][0]['img_url'];
            }
            ...
            if ($BACK_img['result'][0]['img_url'] != '') {
                $result['img'][] = (strpos($img_url, "nationaldraw") === 0) ?  $baseurl . $BACK_img['result'][0]['img_url'] : constant('assetURL') . $BACK_img['result'][0]['img_url'];
            }
            ...
            foreach ($result['img'] as $value) {
                ...
                // if (strpos($value, "nationaldraw") !== 0) {
                //     // echo '"nationaldraw" is present in the string.';
