...
if ($method == 'send_whatsapp') {
    ...
    $body           = mysqli_real_escape_string($con, $_POST['body'] ?? '');
    $media_url      = mysqli_real_escape_string($con, $_POST['photobase64'] ?? '');
    $variables_json = mysqli_real_escape_string($con, $_POST['variables_json'] ?? '');

    // --- NEW: Backend Variable Calculation (Bulletproof sync with frontend) ---
    $raw_body = $_POST['body'] ?? '';
    $raw_json = $_POST['variables_json'] ?? '';
    $maxVar = 0;
    
    // 1. Match variables {{1}}, {{2}} in the body text
    if (preg_match_all('/\{\{(\d+)\}\}/', $raw_body, $matches)) {
        if (!empty($matches[1])) {
            $maxVar = max($matches[1]);
        }
    }
    
    // 2. Match variables inside dynamic URLs in the buttons JSON
    if (preg_match_all('/\{\{(\d+)\}\}/', $raw_json, $jsonMatches)) {
        if (!empty($jsonMatches[1])) {
            $maxJsonVar = max($jsonMatches[1]);
            if ($maxJsonVar > $maxVar) {
                $maxVar = $maxJsonVar;
            }
        }
    }

    // 3. Determine correct m_type and var_count
    $determined_m_type    = ($maxVar > 0) ? 'dynamic' : 'static';
    $determined_var_count = (int)$maxVar;

    // Escape for DB insertion
    $whatsappCon = mysqli_real_escape_string($con, $determined_m_type);
    $whatsappVar = mysqli_real_escape_string($con, (string)$determined_var_count);
    // -------------------------------------------------------------------------

    // --- Local Server Upload Logic ---
    $header_image_url = '';
    
    // Check if an image was selected in the frontend
    if (isset($_FILES['header_image_file']) && $_FILES['header_image_file']['error'] === UPLOAD_ERR_OK) {
        
        $uploadDir = $_SERVER['DOCUMENT_ROOT'] . '/uploads/whatsapp_templates/';
        ...
        $fileTmpPath = $_FILES['header_image_file']['tmp_name'];
        $fileName = $_FILES['header_image_file']['name'];
        ...
        if (in_array($fileExtension, $allowedfileExtensions)) {
            ...
            // Move the file from temp folder to our permanent uploads folder
            if (move_uploaded_file($fileTmpPath, $dest_path)) {
                // Construct the public URL
                $header_image_url = "https://datahub.goride.uk/uploads/whatsapp_templates/" . $newFileName;
            }
        }
    }
    
    // Escape the URL before inserting into the DB
    $header_image_url = mysqli_real_escape_string($con, $header_image_url);
    // --- END Upload Logic ---

    // Safety fallback for $admin_name (ensure it exists in your script's scope)
    $admin_name_safe = isset($admin_name) ? mysqli_real_escape_string($con, $admin_name) : 'Admin';

    // Execute Insertion
    $query = "
        INSERT INTO wamail_templates (name, channel, body, media_url, created_by, m_type, var_count, variables_json, header_image)
        VALUES ('$name', '$channel', '$body', '$media_url', '$admin_name_safe', '$whatsappCon', '$whatsappVar', '$variables_json', '$header_image_url')
    ";
...
else if ($method == 'edit_whatsapp_template2') {
    ...
    // Escape for DB update
    $whatsappCon = mysqli_real_escape_string($con, $determined_m_type);
    $whatsappVar = mysqli_real_escape_string($con, (string)$determined_var_count);
    // -------------------------------------------------------------------------

    // --- Local Server Upload Logic (Only if a NEW image is uploaded) ---
    $imageUpdateSql = ""; 
    ...
    if (isset($_FILES['header_image_file']) && $_FILES['header_image_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedfileExtensions)) {
            ...
            if (move_uploaded_file($fileTmpPath, $dest_path)) {
                // Construct the public URL
                $header_image_url = "https://datahub.goride.uk/uploads/whatsapp_templates/" . $newFileName;
                $header_image_url_safe = mysqli_real_escape_string($con, $header_image_url);
                
                // Append this to our SQL update string
                $imageUpdateSql = ", header_image = '$header_image_url_safe'";
            }
        }
    }
    // --- END Upload Logic ---
    ...
    // Execute Update
    // Note: 'name' is not updated here because Meta does not allow renaming an approved template
    $query = "
    ...
    if (!$result) {
        echo json_encode(["success" => false, "status" => false, "error" => mysqli_error($con)]);
    } else {
        echo json_encode(["success" => true, "status" => true, "id" => $id]);
    }
    exit;
}
else if ($method == 'get_all_templates2') {
    
    // Ensure proper encoding for JSON strings and emojis
    mysqli_set_charset($con, "utf8mb4");
    ...
    // Explicitly select columns to ensure variables_json is fetched correctly
    $query = "SELECT id, name, channel, body, created_at, approval_status, whatsapp_template_id, rejection_reason, variables_json FROM wamail_templates $where ORDER BY id DESC";
    $result = mysqli_query($con, $query);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Safely handle null values for the JSON string
        $row['variables_json'] = $row['variables_json'] ?? '';
...
else if ($method == 'get_single_template_agency') {
    $id = intval($_POST['id']);
    // var_dump('hii');die;
    ...
    // Ensure the message is clean for JSON
    $new_entry = [
        'date' => date('Y-m-d H:i:s'), 
        'message' => $message
    ];
    $json_new_entry = json_encode($new_entry);
    
    // Use JSON_MERGE_PRESERVE as an alternative if ARRAY_APPEND is finicky
    $update_query = "UPDATE `goride_agency_ad_leads` 
                     SET `whatsapp_log` = JSON_ARRAY_APPEND(
                        IF(JSON_VALID(`whatsapp_log`), `whatsapp_log`, '[]'), 
                        '$', 
                        JSON_OBJECT('date', ?, 'message', ?)
                     ) 
                     WHERE `id` = ?";
    ...
    if ($stmt) {
        // We pass the date and message separately to let MySQL build the object
        mysqli_stmt_bind_param($stmt, "ssi", $new_entry['date'], $new_entry['message'], $a_id);
        mysqli_stmt_execute($stmt);
    }

    echo json_encode(['success' => true, 'data' => $template_data]);
    exit;
}

else if ($method == 'get_single_template_webBook') {
    $id = intval($_POST['id']);
    // var_dump('hii');die;
    $a_id = intval($_POST['a_id']);
    $message = isset($_POST['mess']) ? $_POST['mess'] : '';
    ...
    // Use JSON_MERGE_PRESERVE as an alternative if ARRAY_APPEND is finicky
    $update_query = "UPDATE `cus_job_temp` 
                     SET `whatsapp_log` = JSON_ARRAY_APPEND(
                        IF(JSON_VALID(`whatsapp_log`), `whatsapp_log`, '[]'), 
                        '$', 
                        JSON_OBJECT('date', ?, 'message', ?)
