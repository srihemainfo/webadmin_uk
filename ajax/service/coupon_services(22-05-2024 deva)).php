...
if ($method == "generatedcopoun") {
    ...
    if ($formdate != '' && $todate != '') {
        ...
        $contype1 = "AND cticket.createdon BETWEEN '$fd 00:00:00' AND '$td 23:59:59' ";
        // $contype1 = "AND cticket.purchase_datetime BETWEEN '$fd 00:00:00' AND '$td 23:59:59' ";
    } else {

        $da = 'DESC';
        $contype1 = "AND cticket.createdon LIKE '%$now%'";
    ...
    while ($value = mysqli_fetch_assoc($wtticket)) {
        ...
        $result[] = ["codeC" => $ro['c_code'], "action" => $action, "transaction_id" => $value['transactionid'], "proamt" => $value['productamt'], "RaffleID" => $value['raffleid'], "My3Numbers" => $value['my3number'], "email" => $value['email'], "mobile" => $value['mobile'], "cusname" => $value['name'] . ' ' . $value['lname'], "ticketno" => $value['ticket_no'], "purdate" => $value['purchasedatetime']];
    }
