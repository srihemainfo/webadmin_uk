...
//     // FIX: Group empty strings and NULLs together as 'N/A' so their totals are combined accurately
//     $final_where .= " GROUP BY `utm_campaign`, `utm_source`, `utm_medium`, COALESCE(NULLIF(`current_state`, ''), 'N/A') ORDER BY total_customers DESC";

//     // FIX: Select the unified state using an alias (current_state_clean)
//     $columns = "`utm_campaign`, `utm_source`, `utm_medium`, COALESCE(NULLIF(`current_state`, ''), 'N/A') as current_state_clean, COUNT(`id`) as total_customers";
...
//                 // Pulling the safely grouped value straight from the database alias
//                 "current_state"   => $value['current_state_clean'], 
...
//     } else {
//         echo json_encode(["status" => false, "message" => "No UTM campaign data found for the selected dates."]);
//     }
//     exit;
// }
if ($method == "utm_customer_report") {
...
if ($method == 'admin_get_driver_list') {
    global $con; 
    $result = [];
...
//                     "image_url" => $value['image_url'] // Passed image_url to JS frontend
//                 ];
//             }
//         }
//         echo json_encode($result);
//     }
// Existing Regular Push Report Code (Refined)
elseif ($method == 'push_report') {
    $result = [];
    $now = date('Y-m-d');
...
//         $upload_dir = '/home2/airportrides/datahub.goride.uk/uploads/push_notification_images';
...
//     if ($user_type === 'specific_mobiles') {
//         $targetDesc = 'Scheduled for specific ' . $specific_mobiles_type . 's';
//         $req_json_array = [
...
//             (user_type, sent_by, title, body, image_url, route, scheduled_at, status, req_json, res_json, created_at, updated_at, deletes) 
//             VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, 0)";
            
//     $stmt = $con->prepare($sql);
    
//     if ($stmt) {
//         $stmt->bind_param("sisssssssss", $user_type, $admin_id, $title, $body, $imageUrl, $route, $mysql_scheduled_time, $req_json, $res_json, $created_at, $created_at);
...
elseif ($method == 'vehicleApprove') {
    try {
        ...
        $send_whatsapp = $_POST['isWhatsapp'] ?? '';
        $fields = [
            'back_view_admin_status', 'boot_admin_status', 'car_top_view_admin_status',
            'extra_image_1_admin_status', 'extra_image_2_admin_status', 'front_view_admin_status',
            'interior_front_admin_status', 'interior_rear_admin_status', 'side_view_admin_status',
            'special_features_admin_status', 'fc_admin_status', 'insurance_admin_status',
            'puc_admin_status', 'rc_front_admin_status', 'rc_back_admin_status'
        ];
