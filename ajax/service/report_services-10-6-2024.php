...
if ($method == "searchTicket") {
    ...
    try {
        ...
        $drawtype = BlockSQLInjectionforagent($_REQUEST["drawtype"]);
        $searchTxt = BlockSQLInjectionforagent($_REQUEST["searchTxt"]);
        // $Monthly_draw_new_id = BlockSQLInjectionforagent($_REQUEST["Monthly_draw_new_id"]);

        // $drawtype = $_REQUEST['drawtype'];
        // $searchTxt = $_REQUEST['searchTxt'];
        // var_dump($drawtype);die;
        ...
        if ($draw['nr'] > 0) {
            foreach ($draw['result'] as $key => $value) {
                ...
                $dailyName = $value['dailyThrillName'];
                $dailyStatus = $value['dailyThirllStatus'];

                $weeklyName = $value['weeklyBoosterName'];
                $weeklyStatus = $value['weeklyBoosterStatus'];

                $monthlyName = $value['monthlyBumperName'];
                $monthlyBStatus = $value['monthlyBumperStatus'];
            }
        }

        if ($drawtype == 'Daily_Thrill') {

            $draw = select_query($con, "draw", "", "`id`='$draw_new_id' AND `dailyThirllStatus` = 'Active' ", "", "");
        } else if ($drawType == 'Weekly_Raffle') {

            $draw = select_query($con, "draw", "", "`id`='$draw_new_id' AND `weeklyBoosterStatus` = 'Active' ", "", "");
        } else if ($drawType == 'Month_Bumper') {

            $draw = select_query($con, "draw", "", "`id`='$draw_new_id' AND `monthlyBumperStatus` = 'Active' ", "", "");
        }
        ...
        if ($draw['nr'] > 0) {
            ...
            $query = mysqli_query($con, "SELECT nd.*,nd.ticketNo,nd.id, ur.name, ur.lname, nd.netTotal,nd.raffleIds, ur.mobile, ur.email, nd.purchaseDatetime FROM `ndticket` nd JOIN `user_register` ur ON ur.id = nd.userId WHERE $draw_Con nd.endDate >= '$saleDate' AND ur.deletes != 1 AND nd.deletes = '0'");
            ...
            $result['type'] = '0';
            $result['value'] = 'please select correct draw';
        }
    ...
    if ($drawType == 'Daily_Thrill') {
        ...
        $query = "SELECT * FROM draw WHERE `dailyThirllStatus` = 'Active' AND `deletes`='0'ORDER BY `id` ASC";

        //   var_dump("SELECT * FROM draw WHERE `dailyThirllStatus` = 'Active' AND `deletes`='0'ORDER BY `id` ASC");die;

    } else if ($drawType == 'Weekly_Raffle') {

        $query = "SELECT * FROM draw WHERE `weeklyBoosterStatus` = 'Active' AND `deletes`='0'ORDER BY `id` ASC";
    } else if ($drawType == 'Month_Bumper') {

        $query = "SELECT * FROM draw WHERE `monthlyBumperStatus` = 'Active' AND `deletes`='0'ORDER BY `id` ASC";
    }
