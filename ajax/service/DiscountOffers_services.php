...
// var_dump($result);die;
    ...
    $selectQuery = "SELECT * FROM `discount_periods` WHERE $contype $Discout_type `id` != ''  AND `type` != 'flatAgent'  ORDER BY `id` DESC;";
         ...
         if(intval($DiscountAED) >= intval($product_amount)  ){
             ...
             if (mysqli_num_rows($testResult) > 0) {
                ...
                if($Dis_type == 'general'){
                    ...
                    $checkQuery = "(SELECT * FROM `discount_periods` WHERE deletes = '0' AND type != 'coupon' AND type != 'flatAgent' AND product_id = '$product' AND start_date <= '$startdate' AND end_date >= '$startdate' LIMIT 1)
                    UNION ALL
                    (SELECT * FROM `discount_periods` WHERE deletes = '0' AND type != 'coupon' AND type != 'flatAgent' AND product_id = '$product' AND start_date <= '$enddate' AND end_date >= '$enddate' LIMIT 1)";
