...
$result = [];
    ...
    $query = "SELECT * FROM draw WHERE dailyThirllStatus = 'Completed'";
    
    $draws = mysqli_query($con, $query);
    
    
    while ($value = mysqli_fetch_assoc($draws)) {
        $drawid = $value['id'];
        $action = '';

        // Determine draw_name based on status
        if ($value['dailyThirllStatus'] == 'Completed' && $value['weeklyBoosterStatus'] == 'Completed' && $value['monthlyBumperStatus'] == 'Completed') {
            $draw_name = $value['dailyThrillName'] . ' ,' . $value['weeklyBoosterName'] . ' ,' . $value['monthlyBumperName'];
        } elseif ($value['dailyThirllStatus'] == 'Completed' && $value['monthlyBumperStatus'] == 'Completed') {
            $draw_name = $value['dailyThrillName'] . ' ,' . $value['monthlyBumperName'];
        } elseif ($value['dailyThirllStatus'] == 'Completed' && $value['weeklyBoosterStatus'] == 'Completed') {
            $draw_name = $value['dailyThrillName'] . ' ,' . $value['weeklyBoosterName'];
        } else {
            $draw_name = $value['dailyThrillName'];
        }

        // Adding action buttons
        if ($value['monthlyBumperStatus'] == 'Completed') {
                $action .= '<a href="' . constant('cronURL') . 'cron/salesReport1.php?drawid=' . $drawid . '&download=open&sales=yes" ><button class="btn" style="color: blue;"><i class="fa fa-download"></i> Sales Report</button></a>';
        ...
        $result[] = [
            "dailyDrawNo" => $value['dailyDrawNo'],
            "name" => $draw_name,
            "resultDate" => $value['resultDate'],
            "todayGoldPrize" => $value['todayGoldPrize'],
            "output" => $value['output'], // Assuming you have this field in your database
            "action" => $action
        ];
    }
