...
/////////  NEW /////////////////
    ...
    // $query = "SELECT uname, umobile, uemail, transaction_type, opening_balance, total, closeing_balance, reward_type, createdon FROM `wallet_history` WHERE deletes ='0' AND reward_type ='ADWALLETTRANSFER' or 'WALLETTRANSFER' $timeCon $draw_Con $gatewayCon;";
    $query = "SELECT uname, umobile, uemail, transaction_type, opening_balance, total, closeing_balance, reward_type, createdon FROM `wallet_history` WHERE deletes = '0' AND (reward_type = 'ADWALLETTRANSFER' OR reward_type = 'WALLETTRANSFER' OR reward_type = 'RENEWAL') $timeCon $draw_Con $gatewayCon;";
    ...
    // var_dump($formdate,$todate,$paymentStatus,$draw_new_id,$gatewayname,$ccAvenue);die;
    ...
    if ($ccAvenue == 'notGenerated') {
        ...
        if (!empty($paymentStatus)) {
            $paymentStatusCon = "AND p.paymentStatus LIKE '$paymentStatus'";
        }
    ...
    $query = "SELECT  p.transaction_id, p.gateway, p.createdon, p.paymentStatus,p.finaltotal,p.grandtotal, u.name, u.lname,u.mobile, u.email, u.lastlogin, t.ticketNo , pl.response as 'payMentFinal'
        FROM payment_history p 
        INNER JOIN user_register u ON p.user_id = u.id

        LEFT JOIN payment_history_log pl ON pl.id = p.paymentLogID

        LEFT JOIN ndticket t ON t.referenceID = p.ticketReferenceID AND t.deletes = '0'
        LEFT JOIN draw d ON d.dailyDrawNo = p.draw_id AND d.deletes = '0' 
        WHERE p.id != '' $ccAvenueCon $timeCon $drawCon $gatewayCon $paymentStatusCon";
    ...
    while ($row = mysqli_fetch_assoc($payment_history)) {
        ...
        if ($row['paymentStatus'] == 'Success') {
            $action = '<a target="_blank" href="' . $baseurl . 'ticket-view/' . $row['transaction_id'] . '" class="btn text-secondary btn-sm" data-bs-toggle="tooltip" data-bs-original-title="View Tickets"><span style="font-size: 18px;" class="fa fa-files-o"></span></a>
                <a target="_blank" href="' . $baseurl . 'invoice/' . $row['transaction_id'] . '" class="btn text-primary btn-sm" data-bs-toggle="tooltip" data-bs-original-title="View Receipt"><span style="font-size: 18px;" class="fa fa-file-text-o"></span></a>';
        } else {
            $action = '';
        }
        ...
        $result[] = [
            ...
            "baseurl" => $baseurl,
            "action" => $action,
            "errorMessage" => ($row['payMentFinal'] != null ? json_decode($row['payMentFinal'])->status_message : '')
            // "gresponse" => json_decode($row['response'], true)
        ];
    }
    // var_dump($result);die;
