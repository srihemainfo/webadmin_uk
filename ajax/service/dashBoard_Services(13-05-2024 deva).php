...
if ($method == "collect_earning") {
    ...
    if ($roll_id == 1) {
        ...
        $oticket = mysqli_query($con, "SELECT SUM(I.netTotal) AS totalAmount FROM `invoice` AS I JOIN ndticket ON ndticket.id = I.ticketId WHERE I.ticketId = ndticket.id AND I.deletes ='0'AND DATE(I.createdon) = '$currentDate';");
    ...
    // var_dump("SELECT SUM(I.netTotal) AS totalAmount FROM `invoice` AS I JOIN ndticket ON ndticket.id = I.ticketId WHERE I.agent_id != '0' AND I.ticketId = ndticket.id AND I.deletes ='0';");die;
    // $row1 = mysqli_fetch_array($oticket);
    // $totalAmount = $row1['totalAmount'];
    ...
    $output .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xl-12"> <div class="row"> <div class="col-lg-6 col-md-6 col-sm-12 col-xl-3"><div class="card overflow-hidden"><div class="card-body"><div class="d-flex"><div class="mt-2"><h6 class="">Deleted Account Request</h6><h2 class="mb-0 number-font" id="totalonline">' . intval($totalAmount) . '</h2>';
    $output .= '</div></div></div></div></div>';
