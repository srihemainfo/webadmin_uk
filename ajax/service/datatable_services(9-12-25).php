...
// }
    ...
    // var_dump($gatcon);die;
    $SMSquery = mysqli_query($con, "SELECT * FROM `smslog` WHERE $gatcon $contype `id` != '' AND  `gateway` IN ('shiwhatsapp')");
...
elseif ($method == 'kycDetails2') {
    ...
    try {
        ...
        if ($u_type == 'all') {
            ...
            $filterParts[] = "
            (
                cu.vehicle_verify != '2' AND cu.vehicle_verify != '0'
...
$what_mess .= 'Your GoRide account is active for *posting jobs* and ready to use.
Next, *add your vehicle details* to start placing bids and ride with GoRide.
