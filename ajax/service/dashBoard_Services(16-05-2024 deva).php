...
// $dubaidate_time = '2024-4-29 00:00:00';
...
if ($method == "collect_earning") {
    ...
    if ($roll_id == 1) {
        ...
        $oticket = mysqli_query($con, "SELECT SUM(I.netTotal) AS totalAmount FROM `invoice` AS I JOIN ndticket ON ndticket.id = I.ticketId WHERE I.ticketId = ndticket.id AND I.agent_id='0'AND I.deletes ='0' AND DATE(I.createdon) = '$currentDate'");
        $row1 = mysqli_fetch_array($oticket);
        $totalAmount = $row1['totalAmount'];
    ...
    $oticket = mysqli_query($con, "SELECT SUM(I.netTotal) AS totalAmount FROM `invoice` AS I JOIN ndticket ON ndticket.id = I.ticketId WHERE I.agent_id != '0' AND DATE(I.createdon) = CURDATE() AND I.renewalStatus = 'new' AND  I.ticketId = ndticket.id;");
    $row1 = mysqli_fetch_array($oticket);
    $totalAmount = $row1['totalAmount'];
    ...
    //////////////////The Pickup the Store ///////////////////
     $picuptostore = mysqli_query($con, "SELECT COUNT(id) as count, SUM(shipamount) as amount FROM invoice WHERE DATE(createdon) = '$currentDate' AND deliveryType = 'deliveryToMe' AND delivery_status != ''AND renewalStatus = 'DELIVERYTOCUSTOMER';");
    
    $row1 = mysqli_fetch_array($picuptostore);
    $picup_count = $row1['count'];
    $picup_amount = $row1['amount'];
    $output .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xl-12"> <div class="row"> <div class="col-lg-6 col-md-6 col-sm-12 col-xl-3"><div class="card overflow-hidden"><div class="card-body"><div class="d-flex"><div class="mt-2"><h6 class="">Today Delivery Schedule</h6><h5 class="mb-0 number-font" id="totalonline">Count - ' . intval($picup_count) . '<br> Amount - '.$picup_amount.' </h5>';
    $output .= '</div></div></div></div></div>';
    
    //////////////////The Pickup the Store ///////////////////
    $picuptostore = mysqli_query($con, "SELECT COUNT(id) as count, SUM(shipamount) as amount FROM invoice WHERE DATE(createdon) = '$currentDate' AND deliveryType = 'pickUpToStore' AND delivery_status != '' AND renewalStatus = 'PICKUPTOSTORE';");
    ...
    $picup_count = $row1['count'];
    $picup_amount = $row1['amount'];
    $output .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xl-3"><div class="card overflow-hidden"><div class="card-body"><div class="d-flex"><div class="mt-2"><h6 class="">Today Pick UP Schedule</h6><h5 class="mb-0 number-font" id="totalonline">Count - ' . intval($picup_count) . '<br> Amount - '.$picup_amount.' </h5>';
    $output .= '</div></div></div></div></div>';

    //////////////////Whatsapp message Count ///////////////////
    $whatsapp_send = mysqli_query($con, "SELECT COUNT(*) AS whatsapp_send
        FROM `smslog`
        WHERE gateway = 'doubleTick' AND DATE(datetime) = '$currentDate'");
    ...
    $output .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xl-3"><div class="card overflow-hidden"><div class="card-body"><div class="d-flex"><div class="mt-2"><h6 class="">ND Price Detail Download Count</h6><h2 class="mb-0 number-font" id="totalonline">' . intval($sms_count) . '</h2>';
    $output .= '</div></div></div></div></div>';
