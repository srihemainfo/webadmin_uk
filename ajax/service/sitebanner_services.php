...
if ($method == "result_uplaod1") {
    ...
    $fd = date("YYYY-MM-DD HH:mm:ss", strtotime($formdate));
    $td = date("YYYY-MM-DD HH:mm:ss", strtotime($todate));
    ...
    if (isset($_FILES['mfile']['name']) != '') {
        ...
        // var_dump($file_temp_loc, $file_type,$file_name);die;
        $loginID = '1';
        $filePath = new CURLFile($file_temp_loc, $file_type, $file_name);
        ...
        if ($fileUpload['status'] = 'success') {
            ...
            $wd_arr["image_url"] = $fileUpload['data']['digitalURL'];
            // var_dump($wd_arr["image_url"]);die;

            $image_url = $wd_arr["image_url"];

            $sql2 = "INSERT INTO `home_banner` (`path`, `status`, `device_type`, `start_date`,`end_date`,`page_link`,`url_link`)
            VALUES ('$image_url', 'Active', '$device', ' $formdate',' $todate','','$linkInput');";

            $runn = mysqli_query($con, $sql2);

            if ($runn) {
                $result['type'] = '1';
                $result['result'] = 'Image has been Uploaded Successfully';
                goto resultGVIS;
            } else {
                $result['type'] = '0';
                $result['result'] = 'Image Upload Process Failed';
                goto resultGVIS;
            }
        } else {
        ...
        $result['type'] = '0';
        $result['result'] = 'Please select the image';
        goto resultGVIS;
    }
