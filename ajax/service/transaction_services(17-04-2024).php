            ...
            if (intval($balance_point) > 0) {
                ...
                if ($point_req['nr'] > 0) {
                    ...
                    $from_t_point = select_top_name($con, "user_register", "t_point", "`id`='$form_id' and `deletes`='0'", "t_point", "");
                     $from_walletBalance = select_top_name($con, "user_register", "walletBalance", "`id`='$form_id' and `deletes`='0'", "walletBalance", "");
...
// var_dump($formdate,$todate,$paymentStatus,$draw_new_id,$gatewayname,$ccAvenue);die;
    ...
    $query = "SELECT  p.transaction_id, p.gateway, p.createdon, p.paymentStatus,p.finaltotal, u.name, u.lname,u.mobile, u.email, u.lastlogin, t.ticketNo 
        FROM payment_history p 
        INNER JOIN user_register u ON p.user_id = u.id
