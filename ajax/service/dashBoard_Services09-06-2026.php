...
elseif ($method == 'app_jobs_chart') {
    ...
    // Base query setup for frequency_job table
    $sql = "SELECT f.*, d.name as driver_name, d.mobile as driver_mobile
            FROM `frequency_job` f
            LEFT JOIN `customer_register` d ON f.user_id = d.id
            WHERE f.global_type = 'carpool'";

    // Apply Search (Searching within the JSON text block and driver details)
    if (!empty($search)) {
        $sql .= " AND (f.job_data LIKE '%$search%' OR d.name LIKE '%$search%' OR d.mobile LIKE '%$search%')";
    }

    // Apply Date Range (Forced to created_at for Scheduled tab)
    if (!empty($startDate) && !empty($endDate)) {
        $start = date('Y-m-d 00:00:00', strtotime($startDate));
        $end = date('Y-m-d 23:59:59', strtotime($endDate));
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        while ($row = mysqli_fetch_assoc($res)) {
            // Parse JSON job_data to extract necessary display values
            ...
            $row['filled_seats'] = 0; // Routine template implies 0 active bookings
            $row['job_no'] = !empty($row['last_generated_job_no']) ? $row['last_generated_job_no'] : '';
            $row['poolType'] = 'public'; // Enforced Public
    ...
    echo json_encode([
        ...
        'data' => [
            'public' => $publicPools,
            'private' => [] // Always empty for scheduled to keep it strictly public
        ]
    ]);
    exit;
}
