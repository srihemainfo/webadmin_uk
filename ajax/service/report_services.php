...
if ($method == "searchTicket") {
    ...
    try {
        ...
        $draw_type = BlockSQLInjectionforagent($_REQUEST["drawtype"]);
        $searchTxt = BlockSQLInjectionforagent($_REQUEST["searchTxt"]);
        $now = date('Y-m-d');

        // var_dump($draw_new_id,$draw_type,$searchTxt);die;
        ...
        if ($draw_type != '') {
            ...
            if ($draw_type == 'Daily_Thrill') {
                $draw_name = 'dailyThrill';
            }

            if ($draw_type == 'Weekly_Raffle') {
                $draw_name = 'weeklyBooster';
            }

            if ($draw_type == 'Month_Bumper') {
                $draw_name = 'monthlyBumper';
            }

            $draw_Con .= "`drawType` = '$draw_name' AND ";
        }
        if ($draw_new_id != '') {
        ...
        // var_dump($draw_Con);die;

        // $winnerlist = select_query($rcon, "winnerlist", "", "$draw_Con `userid` != '' AND `drawType` = 'dailyThrill' AND EXISTS (SELECT 1 FROM `draw` WHERE `winnerlist`.`draw_id` = `draw`.`id` AND `draw`.`dailyThirllStatus` = 'Completed') ORDER BY `id` DESC", "", "");


        // $winnerlist = select_query($rcon, "winnerlist", "", "$draw_Con `userid` != '' ", "", "");
        ...
        if ($winnerlist['nr'] > 0) {
            ...
            foreach ($winnerlist['result'] as $key => $value) {
                ...
                $ticketNo = select_top_name($rcon, "ndticket", "ticketNo", "`id`={$value['ticketID']} and `deletes`='0'", "ticketNo", "");
                $ivoiceNo = select_top_name($rcon, "ndticket", "invoiceNo", "`id`={$value['ticketID']} and `deletes`='0'", "invoiceNo", "");
                ...
                $result[] = array(
                    "soldby" => $soldby, "name" => $cusname, "email" => $email, "mobile" => $mobile, "id" => $value['id'], "RaffleNO" => $winningNumber, "ivoiceNo" => $ivoiceNo, "TicketID" => $ticketNo, "prize" => $value['prize'], "prizeamt" => $value['prize_amt'], "ticketReferenceID" => $value['ticketReferenceID'], 'action' => $action, 'drawType' => $drawType,
                    'winningDrawName' => $value['winningDrawName']
                );
            }
        }
...
} else if ($method == "winner_list_search") {

    // var_dump('hellow');die;

    $result = [];
    $drawType = BlockSQLInjectionforagent($_REQUEST["drawType"]);

    if ($drawType == 'Daily_Thrill') {

        $query = "SELECT * FROM draw WHERE `dailyThirllStatus` = 'Completed' AND `deletes`='0'ORDER BY `id` ASC";

        $result = mysqli_query($con, $query);

        if (mysqli_num_rows($result) > 0) {
            echo '<option value="">Select Draw</option>';

            while ($row = mysqli_fetch_assoc($result)) {
                $naeme = explode("#", $row['resultDate']);
                $drawDate = date("D", strtotime($row['resultDate']));
                echo '<option value="' . $row['dailyDrawNo'] . '">Draw No #' . str_pad($row['dailyDrawNo'], 3, "0", STR_PAD_LEFT) . ' - ' . str_replace("Draw", "", $naeme[0]) . ' (' . $drawDate . ')</option>';
            }
        } else {
            echo '<option value="">No Draws Available</option>';
        }

        //   var_dump("SELECT * FROM draw WHERE `dailyThirllStatus` = 'Active' AND `deletes`='0'ORDER BY `id` ASC");die;

    } else if ($drawType == 'Weekly_Raffle') {

        $query = "SELECT * FROM draw WHERE `weeklyBoosterStatus` = 'Completed' AND `deletes`='0'ORDER BY `id` ASC";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                $naeme = explode("#", $row['resultDate']);
                $drawDate = date("D", strtotime($row['resultDate']));
                echo '<option value="' . $row['dailyDrawNo'] . '">Draw No #' . str_pad($row['weeklyDrawNo'], 3, "0", STR_PAD_LEFT) . ' - ' . str_replace("Draw", "", $naeme[0]) . ' (' . $drawDate . ')</option>';
            }
        } else {
            echo '<option value="">No Draws Available</option>';
        }
    } else if ($drawType == 'Month_Bumper') {

        $query = "SELECT * FROM draw WHERE `monthlyBumperStatus` = 'Completed' AND `deletes`='0'ORDER BY `id` ASC";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                $naeme = explode("#", $row['resultDate']);
                $drawDate = date("D", strtotime($row['resultDate']));
                echo '<option value="' . $row['dailyDrawNo'] . '">Draw No #' . str_pad($row['bumperDrawNo'], 3, "0", STR_PAD_LEFT) . ' - ' . str_replace("Draw", "", $naeme[0]) . ' (' . $drawDate . ')</option>';
            }
        } else {
            echo '<option value="">No Draws Available</option>';
        }
    ...
    // Echo the options for the second select
    // if (mysqli_num_rows($result) > 0) {
    //      echo '<option value="">Select Draw</option>';
    ...
    try {
        ...
        // $winnerlist = select_query($rcon, "winnerlist", "", "$draw_Con `userid` != '' AND `drawType` = 'weeklyRaffle' ORDER BY `id` DESC", "", "");
        $winnerlist = select_query($rcon, "winnerlist", "", "$draw_Con `userid` != '' AND `drawType` = 'weeklyBooster' AND EXISTS (SELECT 1 FROM `draw` WHERE `winnerlist`.`draw_id` = `draw`.`id` AND `draw`.`weeklyBoosterStatus` = 'Completed') ORDER BY `id` DESC", "", "");
        if ($winnerlist['nr'] > 0) {
    ...
    try {
        ...
        $winnerlist = select_query($rcon, "winnerlist", "", "$draw_Con `userid` != '' AND `drawType` = 'monthlyBumper' AND EXISTS (SELECT 1 FROM `draw` WHERE `winnerlist`.`draw_id` = `draw`.`id` AND `draw`.`monthlyBumperStatus` = 'Completed') ORDER BY `id` DESC", "", "");
        if ($winnerlist['nr'] > 0) {
    ...
    try {
        ...
        if (!empty($searchTxt)) {
            $whereClause .= "AND `winRaffleId` LIKE '%$searchTxt%' OR `ticketID` LIKE '%$searchTxt%' OR `mobile` LIKE '%$searchTxt%'  ";
        }
    ...
    try {
        ...
        // Prepare the WHERE clause for the SQL query
        $whereClause = "`userid` != '' AND `drawType` = 'weeklyRaffle'";
        if (!empty($searchTxt)) {
            $whereClause .= "AND `winRaffleId` LIKE '%$searchTxt%' OR `ticketID` LIKE '%$searchTxt%' OR `mobile` LIKE '%$searchTxt%'";
        }
    ...
    try {
        ...
        $tableName = "eligible_raffles_" . $draw_new_id;
        ...
        if ($checkTable->num_rows == 0) {
            $result['type'] = '0';
            $result['result'] = 'No Participation List Avaliable!';
            $result['userList'] = [];
            goto resultsGVI;
        } else {
            // echo "Table '$tableName' already exists<br>";

            $gettWinner = "SELECT e.*, CONCAT(u.name, ' ', u.lname) AS fullName , u.mobile, u.email, n.ticketNo, n.grandtotal, n.netTotal, n.endDate, n.purchaseDatetime
                                                FROM $tableName e 
                                                LEFT JOIN ndticket n ON n.referenceID = e.ticketReferenceID 
                                                LEFT JOIN user_register u ON u.id = e.userid where e.id != '' ";


            if ($searchTxt != '' && $searchTxt != null) {
                $gettWinner .= " AND (u.name LIKE '%$searchTxt%' OR u.email LIKE '%$searchTxt%' OR u.mobile LIKE '%$searchTxt%') ";
            }

            if ($drawType == 'Daily_Thrill') {
                $gettWinner .= " AND e.`is_thrill` = 'YES' ";
            } else if ($drawType == 'Weekly_Raffle') {
                $gettWinner .= " AND e.`is_weekly` = 'YES' ";
            } else if ($drawType == 'Month_Bumper') {
                $gettWinner .= " AND e.`is_bumper` = 'YES' ";
            }

            $gettWinner .= " ORDER BY RAND()";

            $getDetails = mysqli_query($con, $gettWinner);
            ...
            if (mysqli_num_rows($getDetails) > 0) {
                ...
                $result['type'] = '1';
                $result['result'] = 'Draw List has been collected!';
                $result['userList'] =  mysqli_fetch_all($getDetails, MYSQLI_ASSOC);
                goto resultsGVI;
            } else {
...
} else if ($method == "feature_participation_list") {
    try {
        $result = [];
        ...
        if ($draw_new_id == '' || $draw_new_id == null) {
            $result['type'] = '0';
            $result['result'] = 'No Data Available';
            $result['userList'] = [];
            goto resultsGVI22;
        ...
        // $sql = "SHOW TABLES LIKE '$tableName'";
        // $sql = "SELECT *  FROM `draw` WHERE `id` = $draw_new_id AND deletes = '0' ORDER BY `id` DESC LIMIT 1;";
        // $checkTable = mysqli_query($con, $sql);

        $checkTable = select_query($con, "draw", "", "`id` = $draw_new_id AND deletes = '0' ORDER BY `id` DESC LIMIT 1;", "", "");


        // If the table doesn't exist, create it
        if ($checkTable['nr'] < 1) {
            $result['type'] = '0';
            $result['result'] = 'No Draws Avaliable!';
            $result['userList'] = [];
            goto resultsGVI22;
        } else {


            $resultDate = $checkTable['result'][0]['resultDate'];
            ...
            //     $sqlGetnQuery = "SELECT  g.ticketNo AS 'ticketNo',
            ...
            //  WHERE 
            //      userId != 0 
            //      AND purchaseDatetime < '{$resultDate}' 
            //      AND STR_TO_DATE(endDate, '%Y-%m-%d') >= '{$resultDate}' 
            //      AND deletes = '0'
            //      AND JSON_UNQUOTE(JSON_EXTRACT(raffleIds, CONCAT('$[', numbers.n, ']'))) IS NOT NULL   
            //  ORDER BY id DESC) AS g WHERE g.id != '' ";



            $sqlGetnQuery = "SELECT j.*, CONCAT(u.name, ' ', u.lname) AS fullName , u.mobile, u.email, n.grandtotal, n.netTotal, n.endDate, n.purchaseDatetime FROM ( SELECT * FROM( SELECT 
                            referenceID AS ticketReferenceID,
                            userId AS userid,
                            ticketNo,
                            endDate,
                        ...
                        WHERE 
                            ...
                            AND deletes = '0'
                            AND JSON_UNQUOTE(JSON_EXTRACT(raffleIds, CONCAT('$[', numbers.n, ']'))) IS NOT NULL   
                        ORDER BY RAND()) AS h where h.ticketReferenceID != '' ";


            if ($drawType == 'Daily_Thrill') {
                $sqlGetnQuery .= " AND h.`is_thrill` = 'YES' ";
            } else if ($drawType == 'Weekly_Raffle') {
                $sqlGetnQuery .= " AND h.`is_weekly` = 'YES' ";
            } else if ($drawType == 'Month_Bumper') {
                $sqlGetnQuery .= " AND h.`is_bumper` = 'YES' ";
            }

            $sqlGetnQuery .= " ORDER BY RAND() ) AS j 
                                LEFT JOIN user_register u ON u.id = j.userid
                                LEFT JOIN ndticket n ON n.referenceID = j.ticketReferenceID";
            $sqlGetnQuery .= " ORDER BY RAND() ";

            // var_dump($sqlGetnQuery);
            // die;

            $dailyactiveQuery = mysqli_query($con, $sqlGetnQuery);
            ...
            if (mysqli_num_rows($dailyactiveQuery) > 0) {

                $result['type'] = '1';
                $result['result'] = 'Draw List has been collected!';
                $result['userList'] =  mysqli_fetch_all($dailyactiveQuery, MYSQLI_ASSOC);
                goto resultsGVI22;
            } else {
...
} else if ($method == "participation_list_search") {
    ...
    $query = "SELECT * FROM draw WHERE `id` != '' AND `deletes`='0' ";

    if ($drawType == 'Daily_Thrill') {

        $query .= "AND `dailyThirllStatus` != 'Pending' ";
    } else if ($drawType == 'Weekly_Raffle') {

        $query .= "AND `weeklyBoosterStatus` != 'Pending' ";
    } else if ($drawType == 'Month_Bumper') {

        $query .= "AND `monthlyBumperStatus` != 'Pending' ";
    }

    $query .= " AND `resultDate` <= '" . date("Y-m-d") . "' ORDER BY `id` DESC";
    ...
    if (mysqli_num_rows($SMSquery) > 0) {
        ...
        $result['type'] = '1';
        $result['result'] = 'Draw List has been collected!';
        $result['drawList'] =  mysqli_fetch_all($SMSquery, MYSQLI_ASSOC);
        goto resultFJI;
    } else {
        // echo '<option value="">No Draws Available</option>';
        $result['type'] = '0';
        $result['result'] = 'No Draws Available';
        $result['drawList'] = [];
        goto resultFJI;
    }
    ...
    resultFJI:
    echo json_encode($result);
} else if ($method == "Feature_participation_list_search") {
    ...
    $query .= " AND `resultDate` > '" . date("Y-m-d") . "' ORDER BY `id` ASC";
