...
//     // Matches the buildKycWhereClause time formatting exactly
//     $start = $startDate . ' 00:00:00';
//     $end = $endDate . ' 23:59:59';

//     // Exact match for fetch functions
//     $query = "SELECT c.id as user_id, c.doc_verify, c.vehicle_details, 
//                      k.selfie_status, k.dl_status as kyc_dl_status, k.created_at as k_created_at,
//                      o_dl.status as ocr_status, o_dl.req_response as dl_response
//               FROM customer_register c
//               LEFT JOIN kyc_carpool k ON c.id = k.user_id
//               LEFT JOIN (
//                   SELECT o1.* FROM ocr_request o1 
//                   INNER JOIN (
...
//               WHERE c.deletes = '0' 
//               AND (k.created_at BETWEEN '$start' AND '$end' OR c.created_at BETWEEN '$start' AND '$end')";
...
//             // ------------------------------------------------------------------
//             $has_selfie = false;
//             if ($row['k_created_at'] !== null || $row['selfie_status'] !== null || $row['kyc_dl_status'] !== null) {
//                 $has_selfie = true; 
//             }
//             if ($row['doc_verify'] > 0) {
//                 $has_selfie = true;
...
//             if ($has_selfie) {
//                 $selfie['total']++;
//                 $s_stat = strtolower(trim((string)$row['selfie_status']));
                
//                 if ($s_stat == 'approved' || $s_stat == 'verified' || $row['doc_verify'] == 1) {
//                     $selfie['verified']++;
...
//             // ------------------------------------------------------------------
//             // DETERMINE DL STATUS (Needed for DL Count AND Vehicle Blocker)
//             // ------------------------------------------------------------------
//             $current_dl_status = '';

//             if ($has_vd && isset($vd['dl_status'])) {
//                 $current_dl_status = strtolower(trim($vd['dl_status']));
//             }
//             if (empty($current_dl_status) && !empty($row['kyc_dl_status'])) {
//                 $current_dl_status = strtolower(trim($row['kyc_dl_status']));
//             }
//             if (empty($current_dl_status) && !empty($row['ocr_status'])) {
//                 $current_dl_status = strtolower(trim($row['ocr_status']));
//             }
...
//             // ------------------------------------------------------------------
//             $is_dl_present = false;
//             if ($row['ocr_status'] !== null || $row['dl_response'] !== null) $is_dl_present = true;
//             if (!empty($row['kyc_dl_status'])) $is_dl_present = true;
//             if (strpos((string)$row['vehicle_details'], 'dl_status') !== false) $is_dl_present = true;

//             if ($is_dl_present) {
//                 $dl['total']++;
                
//                 $final_dl_stat = empty($current_dl_status) ? 'pending' : $current_dl_status;

//                 if ($final_dl_stat == 'approved' || $final_dl_stat == 'verified') {
//                     $dl['verified']++;
//                 } else if ($final_dl_stat == 'rejected') {
//                     $dl['rejected']++;
//                 } else {
//                     $dl['pending']++;
//                 }
...
//             // ------------------------------------------------------------------
//             // 3. VEHICLE LOGIC (With Strict DL Blocker)
//             // ------------------------------------------------------------------
//             if (!empty($row['vehicle_details']) && $row['vehicle_details'] != 'null' && trim($row['vehicle_details']) != '') {
                
//                 // FINAL BLOCKER: Only count if DL is explicitly approved/verified
//                 if ($current_dl_status === 'approved' || $current_dl_status === 'verified') {
                    
//                     $vehicle['total']++;
//                     $v_stat = 'pending';

//                     if ($has_vd && isset($vd['car_type_status'])) {
//                         $v_stat = strtolower(trim($vd['car_type_status']));
//                     }

//                     if ($v_stat == 'approved' || $v_stat == 'verified') {
//                         $vehicle['verified']++;
...
// }
    ...
    // Exact match for fetch functions
    $query = "SELECT c.id as user_id, c.doc_verify, c.vehicle_details, 
                     k.selfie_status, k.dl_status as kyc_dl_status, k.updated_at as k_updated_at,
                     o_dl.status as ocr_status, o_dl.req_response as dl_response
              FROM customer_register c
              LEFT JOIN kyc_carpool k ON c.id = k.user_id
              LEFT JOIN (
              ...
              WHERE c.deletes = '0' 
              AND (k.updated_at BETWEEN '$start' AND '$end' OR c.updated_at BETWEEN '$start' AND '$end')";
    ...
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // ------------------------------------------------------------------
            $has_selfie = false;
            if ($row['k_updated_at'] !== null || $row['selfie_status'] !== null || $row['kyc_dl_status'] !== null || $row['doc_verify'] > 0) {
                $has_selfie = true; 
            }

            if ($has_selfie) {
                $selfie['total']++;
                $raw_selfie = strtolower(trim((string)$row['selfie_status']));
                if (empty($raw_selfie)) $raw_selfie = 'pending';
                
                if (in_array($raw_selfie, ['approved', '1', 'verified']) || $row['doc_verify'] == 1) {
                    $selfie['verified']++;
                } else if (in_array($raw_selfie, ['rejected', 'failed', '0'])) {
                    $selfie['rejected']++;
                } else {
                    $selfie['pending']++;
                }
            ...
            // ------------------------------------------------------------------
            $is_dl_present = false;
            if ($row['ocr_status'] !== null || $row['dl_response'] !== null || !empty($row['kyc_dl_status']) || strpos((string)$row['vehicle_details'], 'dl_status') !== false) {
                $is_dl_present = true;
            }
            ...
            if ($is_dl_present) {
                ...
                if ($has_vd && isset($vd['dl_status']) && trim($vd['dl_status']) !== '') {
                    $raw_dl = strtolower(trim($vd['dl_status']));
                } else {
                    if (!empty($row['kyc_dl_status'])) {
                        $raw_dl = strtolower(trim($row['kyc_dl_status']));
                    } elseif (!empty($row['ocr_status'])) {
                        $raw_dl = strtolower(trim($row['ocr_status']));
                    }

                    // DL Error parsing for frontend matching
                    if (!empty($row['dl_response']) && $row['dl_response'] !== 'null') {
                        $r = json_decode($row['dl_response'], true);
                        if (is_array($r) && (isset($r['code']) || !isset($r['id_no']))) {
                            $raw_dl = 'rejected';
                        }
                    }
                }

                if (in_array($raw_dl, ['approved', '1', 'verified'])) {
                    $dl['verified']++;
                } else if (in_array($raw_dl, ['rejected', 'failed', '0'])) {
                    $dl['rejected']++;
                } else {
                    $dl['pending']++;
                }
            ...
            // ------------------------------------------------------------------
            // 3. VEHICLE LOGIC (Strictly matches fetch_vehicle_data logic)
            // ------------------------------------------------------------------
            if (!empty($row['vehicle_details']) && $row['vehicle_details'] != 'null' && trim($row['vehicle_details']) != '') {
                
                // Mirror the exact DL status check from fetch_vehicle_data
                $current_dl_status = '';
                ...
                // THE BLOCKER: Only count vehicles if the DL is approved
                if ($current_dl_status === 'approved') {
                    $vehicle['total']++;
                    $raw_veh = 'pending';

                    if ($has_vd && isset($vd['car_type_status'])) {
                        $raw_veh = strtolower(trim($vd['car_type_status']));
                    }

                    if (in_array($raw_veh, ['approved', '1', 'verified'])) {
                        $vehicle['verified']++;
                    } else if (in_array($raw_veh, ['rejected', 'failed', '0'])) {
                        $vehicle['rejected']++;
                    } else {
                        $vehicle['pending']++;
                    }
...
elseif ($method == 'app_jobs_chart') {
    ...
    // ONLY USE cus_job_temp
    $sql = "SELECT c.*, d.name as driver_name, cf.rating as fb_rating, 
            cf.review as fb_review, jc.created_at as c_cancelled_at
            FROM `cus_job_temp` c
            ...
            WHERE c.deletes = '0' AND c.job_no NOT LIKE 'GRP-%'";

    // Apply filters based on new requirements
    if ($status == 'completed') {
        $sql .= " AND c.job_status = 'completed'";
    } elseif ($status == 'started') {
        $sql .= " AND c.job_status = 'started'";
    } elseif ($status == 'incompleted') {
        // Assigned but pickup time exceeded and not started/completed
        $sql .= " AND c.job_status IN ('accept', 'accepted') AND c.pickup_date < '$currentDateTime'";
    } elseif ($status == 'accepted') {
        // Normally Assigned
        $sql .= " AND c.job_status IN ('accept', 'accepted') AND c.pickup_date >= '$currentDateTime'";
    } elseif ($status == 'not_complete') {
        // Unassigned
        $sql .= " AND c.job_status NOT IN ('accept', 'accepted', 'started', 'completed', 'cancelled')";
    ...
    $sql .= " ORDER BY c.pickup_date ASC";
    $res = mysqli_query($con, $sql);
    ...
    // PRE-FETCH BULK DATA (Eliminating N+1 Queries)
    ...
    foreach ($jobsMap as $job_no => $row) {
        ...
        if (($currentStatus === 'accept' || $currentStatus === 'accepted' || $currentStatus === 'started' || $currentStatus === 'completed') && !empty($row['id'])) {
            $push_jobs_to_fetch[$row['id']] = $row['id'];
        }
    }
    ...
    foreach ($jobsMap as $job_no => $row) {
        ...
        $row['count'] = 0;
        if (!empty($row['id']) && isset($push_data[$row['id']])) {
            $row['count'] = $push_data[$row['id']];
        }
...
//     $sql = "SELECT c.*, 
//                   CASE WHEN c.global_type = 'dr_carpool' THEN u.name ELSE d.name END as driver_name, 
//                   CASE WHEN c.global_type = 'dr_carpool' THEN u.mobile ELSE d.mobile END as driver_mobile";
    
//     if ($tabType == 'cancelled') {
//         $sql .= ", jc.reason as cancellation_reason ";
//     }
    
//     $sql .= " FROM `cus_job_temp` c
//               LEFT JOIN `customer_register` d ON (c.assigned_to = d.id OR c.user_id = d.id) AND c.global_type = 'carpool'
//               LEFT JOIN `user_register` u ON (c.assigned_to = u.id OR c.user_id = u.id) AND c.global_type = 'dr_carpool'";
              
//     if ($tabType == 'cancelled') {
//         $sql .= " LEFT JOIN `job_cancellations` jc ON c.id = jc.job_id ";
//     }
    
//     // EXCLUDE DELETED CUSTOMERS: Added condition to ensure carpool records only show if the customer exists and deletes = 0
//     $sql .= " WHERE c.global_type IN ('carpool', 'dr_carpool') AND c.deletes = '0' 
//               AND (c.global_type = 'dr_carpool' OR (c.global_type = 'carpool' AND d.id IS NOT NULL AND d.deletes = '0'))";
...
//     if (!empty($search)) {
//         $sql .= " AND (c.job_no LIKE '%$search%' OR d.name LIKE '%$search%' OR u.name LIKE '%$search%' OR d.mobile LIKE '%$search%' OR u.mobile LIKE '%$search%' OR c.from_place LIKE '%$search%' OR c.to_place LIKE '%$search%')";
//     }
...
//         // EXCLUDE DELETED CUSTOMERS FROM INVITATIONS: Added cr1.deletes = 0 and cr2.deletes = 0
//         $invSql = "SELECT i.*, 
//                           cr1.name as inviter_name, cr1.mobile as inviter_mobile, 
//                           CASE WHEN i.global_type = 'dr_carpool' THEN ur.name ELSE cr2.name END as invitee_name,
//                           CASE WHEN i.global_type = 'dr_carpool' THEN ur.mobile ELSE cr2.mobile END as invitee_mobile
//                   FROM invitations i
//                   LEFT JOIN customer_register cr1 ON i.inviter_id = cr1.id AND cr1.deletes = '0'
//                   LEFT JOIN customer_register cr2 ON i.invitee_user_id = cr2.id AND i.global_type != 'dr_carpool' AND cr2.deletes = '0'
//                   LEFT JOIN user_register ur ON i.invitee_user_id = ur.id AND i.global_type = 'dr_carpool'
...
if ($method == 'carpoolJobList') {
    ...
    $startDate = isset($_POST['startDate']) ? mysqli_real_escape_string($con, $_POST['startDate']) : '';
    $endDate = isset($_POST['endDate']) ? mysqli_real_escape_string($con, $_POST['endDate']) : '';
    $filterType = isset($_POST['filterType']) ? $_POST['filterType'] : 'created_at';
    $tabType = isset($_POST['tabType']) ? $_POST['tabType'] : 'current';
    $roleType = isset($_POST['roleType']) ? $_POST['roleType'] : 'customer';

    $dateCol = ($filterType == 'pickup') ? 'c.pickup_date' : 'c.created_at';

    $sql = "SELECT c.*, 
                   IF(c.global_type = 'dr_carpool', u.name, d.name) as driver_name, 
                   IF(c.global_type = 'dr_carpool', u.mobile, d.mobile) as driver_mobile";
    ...
    $sql .= " FROM `cus_job_temp` c
              LEFT JOIN `customer_register` d ON d.id = COALESCE(NULLIF(c.assigned_to, 0), c.user_id) AND c.global_type = 'carpool'
              LEFT JOIN `user_register` u ON u.id = COALESCE(NULLIF(c.assigned_to, 0), c.user_id) AND c.global_type = 'dr_carpool'";
              
    if ($tabType == 'cancelled') {
        $sql .= " LEFT JOIN `job_cancellations` jc ON c.id = jc.job_id ";
    }
    
    $sql .= " WHERE c.deletes = '0' ";

    if ($roleType === 'driver') {
        $sql .= " AND c.global_type = 'dr_carpool' ";
        ...
        if ($tabType == 'cancelled') {
            ...
            $sql .= " AND c.job_status NOT IN ('cancelled', 'completed', 'started') 
                      AND c.pickup_date < CURDATE() 
                      AND (c.filled_seat IN (0, '') OR c.filled_seat IS NULL) ";
        } else { 
            $sql .= " AND c.job_status NOT IN ('cancelled', 'completed', 'started') 
                      AND c.pickup_date >= CURDATE() ";
        }
        ...
        $sql .= " AND c.global_type = 'carpool' AND d.id IS NOT NULL AND d.deletes = '0' ";
        ...
        if ($tabType == 'cancelled') {
            ...
            $sql .= " AND c.job_status != 'cancelled' AND c.pickup_date < '$currentDateTime' AND (c.filled_seat IN (0, '') OR c.filled_seat IS NULL)";
        } else { 
            $sql .= " AND c.job_status != 'cancelled' AND c.pickup_date >= '$currentDateTime'";
        }
    }
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        while ($row = mysqli_fetch_assoc($res)) {
            $row['invitations'] = []; 
            $row['per_seat_fare'] = $row['fare'];
            $row['filled_seats'] = !empty($row['filled_seat']) ? (int)$row['filled_seat'] : 0;
            $row['total_seats'] = !empty($row['pass_count']) ? (int)$row['pass_count'] : 4;
    ...
    if (!empty($jobsDict)) {
        $jobIdsStr = implode(',', array_keys($jobsDict));
        
        $invSql = "SELECT i.*, 
                          cr1.name as inviter_name, cr1.mobile as inviter_mobile, 
                          IF(i.global_type = 'dr_carpool', ur.name, cr2.name) as invitee_name,
                          IF(i.global_type = 'dr_carpool', ur.mobile, cr2.mobile) as invitee_mobile
                   FROM invitations i
                   LEFT JOIN customer_register cr1 ON i.inviter_id = cr1.id AND cr1.deletes = '0'
                   LEFT JOIN customer_register cr2 ON i.invitee_user_id = cr2.id AND i.global_type != 'dr_carpool' AND cr2.deletes = '0'
                   LEFT JOIN user_register ur ON i.invitee_user_id = ur.id AND i.global_type = 'dr_carpool'
        ...
        if ($invRes && mysqli_num_rows($invRes) > 0) {
            while ($inv = mysqli_fetch_assoc($invRes)) {
                $jobsDict[$inv['job_id']]['invitations'][] = $inv;
            }
        }
    }
...
else if ($method == 'carpoolScheduledList') {
    header('Content-Type: application/json; charset=utf-8');
    ...
    $startDate = isset($_POST['startDate']) ? mysqli_real_escape_string($con, $_POST['startDate']) : '';
    $endDate = isset($_POST['endDate']) ? mysqli_real_escape_string($con, $_POST['endDate']) : '';
    $roleType = isset($_POST['roleType']) ? mysqli_real_escape_string($con, $_POST['roleType']) : 'customer';
    
    $sql = "SELECT f.*, 
                   CASE WHEN f.global_type = 'dr_carpool' THEN u.name ELSE d.name END as driver_name, 
                   CASE WHEN f.global_type = 'dr_carpool' THEN u.mobile ELSE d.mobile END as driver_mobile
            FROM `frequency_job` f
            LEFT JOIN `customer_register` d ON f.user_id = d.id AND f.global_type = 'carpool'
            LEFT JOIN `user_register` u ON f.user_id = u.id AND f.global_type = 'dr_carpool'
            WHERE 1=1 ";

    // Role filtering mapping 
    if ($roleType === 'driver') {
        $sql .= " AND f.global_type = 'dr_carpool' ";
    } else {
        $sql .= " AND f.global_type = 'carpool' AND d.id IS NOT NULL AND d.deletes = '0' ";
    }

    if (!empty($search)) {
        $sql .= " AND (f.job_data LIKE '%$search%' OR d.name LIKE '%$search%' OR u.name LIKE '%$search%' OR d.mobile LIKE '%$search%' OR u.mobile LIKE '%$search%')";
    }

    if (!empty($startDate) && !empty($endDate)) {
        $start = date('Y-m-d 00:00:00', strtotime($startDate));
        $end = date('Y-m-d 23:59:59', strtotime($endDate));
        $sql .= " AND f.created_at BETWEEN '$start' AND '$end'";
    }
    ...
    if ($res && mysqli_num_rows($res) > 0) {
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            if (is_array($jobData)) {
                $row['from_place'] = isset($jobData['from_place']) ? $jobData['from_place'] : 'N/A';
                $row['to_place'] = isset($jobData['to_place']) ? $jobData['to_place'] : 'N/A';
                $row['pickup_date'] = isset($jobData['pickup_date']) ? $jobData['pickup_date'] : null;
                $row['per_seat_fare'] = isset($jobData['fare']) ? $jobData['fare'] : 0;
                $row['total_seats'] = isset($jobData['pass_count']) && $jobData['pass_count'] != '' ? (int)$jobData['pass_count'] : 4;
            } else {
                $row['from_place'] = 'N/A';
                $row['to_place'] = 'N/A';
            ...
            $row['filled_seats'] = 0;
            $row['job_no'] = !empty($row['last_generated_job_no']) ? $row['last_generated_job_no'] : '';
            $row['poolType'] = 'public';
...
// elseif ($method === 'user_pending_counts') {
//     $s_query = "
//         (
//             SELECT 'kyc_count' AS type, COUNT(*) AS total
//             FROM kyc_details kd
//             JOIN user_register cu ON cu.id = kd.user_id
//             WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'dl_pending_count' AS type, COUNT(DISTINCT kd.user_id) AS total
//             FROM kyc_details kd
//             JOIN user_register cu ON cu.id = kd.user_id
//             WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0' 
//               AND LOWER(kd.dl_status) IN ('inreview', 'pending', 'approval_pending')
//         )
//         UNION ALL
//         (
//             SELECT 'aadhar_pending_count' AS type, COUNT(DISTINCT kd.user_id) AS total
//             FROM kyc_details kd
//             JOIN user_register cu ON cu.id = kd.user_id
//             WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0' 
//               AND LOWER(kd.proof_status) IN ('inreview', 'pending', 'approval_pending')
//         )
//         UNION ALL
//         (
//             SELECT 'selfie_pending_count' AS type, COUNT(DISTINCT kd.user_id) AS total
//             FROM kyc_details kd
//             JOIN user_register cu ON cu.id = kd.user_id
//             WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0' 
//               AND LOWER(kd.selfie_status) IN ('inreview', 'pending', 'approval_pending')
//         )
//         UNION ALL
//         (
//             SELECT 'pan_pending_count' AS type, COUNT(DISTINCT cr.user_id) AS total
//             FROM ocr_request cr
//             JOIN user_register cu ON cu.id = cr.user_id
//             JOIN kyc_details kd ON kd.user_id = cu.id
//             WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0'
//               AND cr.doc_type = 'PAN' AND LOWER(cr.status) IN ('inreview', 'pending', 'approval_pending')
//         )
//         UNION ALL
//         (
//             SELECT 'gst_pending_count' AS type, COUNT(DISTINCT cr.user_id) AS total
//             FROM ocr_request cr
//             JOIN user_register cu ON cu.id = cr.user_id
//             JOIN kyc_details kd ON kd.user_id = cu.id
//             WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0'
//               AND cr.doc_type = 'GST' AND LOWER(cr.status) IN ('inreview', 'pending', 'approval_pending')
//         )
//         UNION ALL
//         (
//             SELECT 'name_board_pending_count' AS type, COUNT(DISTINCT cr.user_id) AS total
//             FROM ocr_request cr
//             JOIN user_register cu ON cu.id = cr.user_id
//             JOIN kyc_details kd ON kd.user_id = cu.id
//             WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0'
//               AND cr.doc_type = 'NAME_BOARD' AND LOWER(cr.status) IN ('inreview', 'pending', 'approval_pending')
//         )
//         UNION ALL
//         (
//             SELECT 'vehicle_count' AS type, COUNT(DISTINCT cu.id) AS total
//             FROM user_register cu
//             WHERE cu.vehicle_verify > 0 AND LOWER(cu.vehicle_details->>'$.vehicle_review_message') LIKE '%wait for admin approval%' AND cu.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'rc_pending_count' AS type, COUNT(DISTINCT cu.id) AS total
//             FROM user_register cu
//             WHERE cu.vehicle_verify > 0 AND LOWER(cu.vehicle_details->>'$.vehicle_review_message') LIKE '%wait for admin approval%' AND cu.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'puc_pending_count' AS type, COUNT(DISTINCT cu.id) AS total
//             FROM user_register cu
//             WHERE cu.vehicle_verify > 0 AND LOWER(cu.vehicle_details->>'$.puc_details.puc_admin_status') IN ('0', 'pending', 'inreview', 'approval_pending') AND cu.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'insurance_pending_count' AS type, COUNT(DISTINCT cu.id) AS total
//             FROM user_register cu
//             WHERE cu.vehicle_verify > 0 AND LOWER(cu.vehicle_details->>'$.insurance_details.insurance_admin_status') IN ('0', 'pending', 'inreview', 'approval_pending') AND cu.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'fc_pending_count' AS type, COUNT(DISTINCT cu.id) AS total
//             FROM user_register cu
//             WHERE cu.vehicle_verify > 0 AND LOWER(cu.vehicle_details->>'$.fc_details.fc_admin_status') IN ('0', 'pending', 'inreview', 'approval_pending') AND cu.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'expiry_count' AS type, COUNT(*) AS total
//             FROM open_jobs oj
//             WHERE oj.job_status IN ('created','bidding') AND oj.pickup_date < NOW() AND oj.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'cancelled_count' AS type, COUNT(*) AS total
//             FROM open_jobs oj
//             WHERE oj.job_status = 'cancelled' AND oj.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'n_complete_count' AS type, COUNT(*) AS total
//             FROM open_jobs oj
//             WHERE oj.job_status IN ('created','bidding') AND oj.pickup_date > NOW() AND oj.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'kyc_n_count' AS type, COUNT(*) AS total
//             FROM kyc_details kd
//             JOIN user_register cu ON cu.id = kd.user_id
//             WHERE cu.doc_verify = '0' AND kd.o_status < '3' AND cu.dialCode = '91' AND cu.deletes = '0' AND kd.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'error_log' AS type, COUNT(*) AS total
//             FROM api_error_log err
//             WHERE err.created_at BETWEEN CURDATE() AND NOW() AND err.deletes = '0' AND err.status = 0
//             AND err.url NOT LIKE 'https://www.goride.run/api/user'
//             AND err.url NOT LIKE 'https://www.goride.run/api/getChat-con'
//         )
//         UNION ALL
//         (
//             SELECT 'v_driver' AS type, COUNT(*) AS total
...
//         UNION ALL
//         (
//             SELECT 'v_owner' AS type, COUNT(*) AS total
//             FROM user_register ur JOIN kyc_details kd ON kd.user_id = ur.id
//             WHERE ur.deletes = '0' AND ur.doc_verify = 1 AND kd.type = 'Owner' AND kd.o_status = 3
//         )
//         UNION ALL
//         (
//             SELECT 'k_v_driver' AS type, COUNT(*) AS total
//             FROM user_register ur JOIN kyc_details kd ON kd.user_id = ur.id
//             WHERE ur.deletes = '0' AND ur.doc_verify = 1 AND ur.vehicle_verify != 2 AND kd.type = 'Driver' AND kd.o_status = 3
//         )
//         UNION ALL
//         (
//             SELECT 'v_v_driver' AS type, COUNT(*) AS total
//             FROM user_register ur JOIN kyc_details kd ON kd.user_id = ur.id
//             WHERE ur.deletes = '0' AND ur.vehicle_verify = 2 AND kd.type = 'Driver'
//         )
//         UNION ALL
//         (
//             SELECT 'sms_count' AS type, COUNT(*) AS total
//             FROM smslog sms
//             WHERE sms.datetime BETWEEN CURDATE() AND NOW() AND sms.gateway = 'smsportal' 
//         )
//         UNION ALL
//         (
//           SELECT 'shi_count' AS type, COUNT(*) AS total
//             FROM smslog sms
//             WHERE sms.datetime BETWEEN CURDATE() AND NOW() AND sms.gateway = 'shiwhatsapp' 
//         )
//         UNION ALL
//         (
//           SELECT 'wh_count' AS type, COUNT(*) AS total
//             FROM whatsapp_bulk_message wh
//             WHERE wh.created_at BETWEEN CURDATE() AND NOW() AND wh.status = 'success' 
//         )
//         UNION ALL
//         (
//             SELECT 'consent_count' AS type, COUNT(ud.id) AS total
//             FROM user_register cu
//             JOIN kyc_details kd ON cu.id = kd.user_id
//             LEFT JOIN users_docs ud ON cu.id = ud.user_id
//             WHERE cu.deletes = '0' AND kd.deletes = '0'
//         ) 
//         UNION ALL
//         (
//              SELECT 'reject_count' AS type, COUNT(cu.id) AS total
//             FROM user_register cu
//             LEFT JOIN kyc_details kd ON cu.id = kd.user_id
//             WHERE cu.vehicle_verify > 0
//             AND LOWER(vehicle_details->>'$.vehicle_review_message') NOT LIKE '%vehicle details verified%'
//             AND LOWER(vehicle_details->>'$.vehicle_review_message') NOT LIKE '%wait for admin approval%'
//             AND cu.deletes = '0' AND kd.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'notverifi_count' AS type, COUNT(cu.id) AS total
//             FROM user_register cu
//             LEFT JOIN kyc_details kd ON cu.id = kd.user_id
//             WHERE cu.vehicle_verify > 0 AND LOWER(vehicle_details->>'$.vehicle_review_message') LIKE '%wait for admin approval%'
//             AND cu.deletes = '0' AND kd.deletes = '0'
//         )
//         UNION ALL
//         (
//             SELECT 'kyc_missing_count' AS type, COUNT(*) AS total
//             FROM user_register ur
//             WHERE ur.deletes = '0' AND ur.dialCode = '91'
//             AND NOT EXISTS (
//                 SELECT 1 FROM kyc_details kd WHERE kd.user_id = ur.id AND kd.deletes = '0'
//             )
//         )
//         UNION ALL
//         (
//             SELECT 'lead_web' AS type, COUNT(*) AS total
...
//         UNION ALL
//         (
//             SELECT 'post_web' AS type, COUNT(*) AS total
//             FROM cus_job_temp cs
//             WHERE cs.deletes = '0' AND cs.user_id != 0 AND cs.user_details IS NOT NULL AND cs.job_status IN ('created', 'bidding')
//         )
//     ";
...
elseif ($method === 'user_pending_counts') {
    ...
    $s_query = "
        (
            ...
            JOIN user_register cu ON cu.id = kd.user_id
            WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0'
            AND kd.created_at BETWEEN '$startDateTime' AND '$endDateTime'
        )
        UNION ALL
        (
            SELECT 'dl_pending_count' AS type, COUNT(DISTINCT kd.user_id) AS total
        ...
        (
            ...
            WHERE cu.doc_verify = '0' AND kd.o_status = '3' AND cu.deletes = '0' AND kd.deletes = '0'
              AND cr.doc_type = 'GST' AND LOWER(cr.status) IN ('inreview', 'pending', 'approval_pending')
              AND cr.created_at BETWEEN '$startDateTime' AND '$endDateTime'
        )
        UNION ALL
        (
            SELECT 'name_board_pending_count' AS type, COUNT(DISTINCT cr.user_id) AS total
        ...
        (
            ...
            FROM user_register cu
            WHERE cu.vehicle_verify > 0 AND LOWER(cu.vehicle_details->>'$.vehicle_review_message') LIKE '%wait for admin approval%' AND cu.deletes = '0'
            AND cu.created_at BETWEEN '$startDateTime' AND '$endDateTime'
        )
        UNION ALL
        (
            SELECT 'rc_pending_count' AS type, COUNT(DISTINCT cu.id) AS total
        ...
        (
            SELECT 'error_log' AS type, COUNT(*) AS total
            FROM api_error_log err
            WHERE err.deletes = '0' AND err.status = 0
            AND err.url NOT LIKE 'https://www.goride.run/api/user'
            AND err.url NOT LIKE 'https://www.goride.run/api/getChat-con'
        )
        UNION ALL
        ...
        (
          SELECT 'shi_count' AS type, COUNT(*) AS total
            FROM smslog sms
            WHERE sms.datetime BETWEEN CURDATE() AND NOW() AND sms.gateway = 'fbwhatsapp' 
        )
        UNION ALL
        (
          SELECT 'wh_count' AS type, COUNT(*) AS total
