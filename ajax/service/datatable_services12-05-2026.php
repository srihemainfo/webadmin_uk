...
// --- Place this inside ajax/service/datatable_services.php ---
...
// 2. Change Status and Insert to blocked_user Table
elseif ($method == 'admin_change_driver_status') {
    ...
    if ($driverId > 0 && $memid > 0) {
        ...
        if ($newStatus == 1) {
            // 1. Get the RAW expiry date first (Do not escape yet)
            $raw_expiry_date = (isset($_POST['expiry_date']) && !empty($_POST['expiry_date'])) ? $_POST['expiry_date'] : date('Y-m-d', strtotime('+1 day'));
            
            $block_date = date('Y-m-d');
            $current_datetime = date('Y-m-d H:i:s');
            
            // 2. Calculate the difference in days using strtotime (bulletproof method)
            $diff_in_seconds = strtotime($raw_expiry_date) - strtotime($block_date);
            $day_count = round($diff_in_seconds / 86400); // 86400 seconds = 1 day

            // Optional: Ensure day_count is never negative if they select a past date
            if ($day_count < 0) {
                $day_count = 0;
            }

            // 3. NOW escape the expiry date for the SQL query
            $safe_expiry_date = mysqli_real_escape_string($con, $raw_expiry_date);
            ...
            $logSql = "INSERT INTO `blocked_user` (`user_id`, `reported_by`, `reason_for_rejection`, `block_date`, `expiry_date`, `day_count`, `created_at`, `updated_at`) 
                       VALUES ('$driverId', '$memid', '$reason', '$block_date', '$safe_expiry_date', '$day_count', '$current_datetime', '$current_datetime')";
...
// 3. Fetch Block History from blocked_user
...
// 4. Update Driver Permissions (NEW METHOD)
