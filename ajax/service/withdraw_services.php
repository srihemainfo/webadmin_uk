...
/////////  NEW /////////////////
    ...
    // 1. Force JSON header so the browser knows not to render HTML errors
    header('Content-Type: application/json; charset=utf-8');
    $result = [];
    $contype = '';
    $da = '';
    $now = date('Y-m-d');
    // 2. Prevent undefined index notices on $_POST
    $formdate = isset($_POST["formdate"]) ? trim(BlockSQLInjectionforagent($_POST["formdate"])) : '';
    $todate = isset($_POST["todate"]) ? trim(BlockSQLInjectionforagent($_POST["todate"])) : '';
    $status = isset($_POST["status"]) ? trim(BlockSQLInjectionforagent($_POST["status"])) : '';
    $trans_mode = isset($_POST["trans_mode"]) ? trim(BlockSQLInjectionforagent($_POST["trans_mode"])) : '';
    // FIX: Safely Handle Status Filter (Strict check for '0' / Process)
    if ($status === '' || $status === 'all') {
        $statuscon = "";
    } elseif ($status === '0') {
        $statuscon = "(`status` = '0' OR `status` IS NULL OR `status` = '') AND ";
    } else {
    ...
    if ($formdate != '') {
        ...
        $da = 'DESC';
        $contype = ""; // <--- DO NOT restrict to today by default
    }
    $permission = array(3, 6, 5);
    $session_memid = isset($_SESSION['memid']) ? $_SESSION['memid'] : 0;
    $roll_id = select_top_name($con, "user_register", "roll_id", "`id`='$session_memid' and `deletes`='0'", "roll_id", "");
    $agent = '';
    if (in_array($roll_id, $permission)) {
        $agent = "`from_id` = '$session_memid' AND ";
    }
    if ($formdate == '' && $todate == '' && ($status === '' || $status === 'all') && ($trans_mode === '' || $trans_mode === 'all')) {
        $contype = "`createdon` LIKE '%$now%' AND ";
    }
    $with_his = select_query($con, "withdraw_request", "", " $trans_mode_con $agent $statuscon $contype `deletes`='0' ORDER BY `id` DESC", "", "");
    if (isset($with_his['nr']) && $with_his['nr'] > 0) {
        foreach ($with_his['result'] as $key => $value) {
            $req_by = $value['requested_by'];
            $request_id = $value['request_id'];
            // =======================================================
            // 🚀 DETERMINE USER TABLE BASED ON REQUEST ID PREFIX
            // =======================================================
            if (strpos($request_id, 'WRC-') === 0) {
                // Withdrawal request for a customer
                $tableName = "customer_register";
                $userType = 'Customer'; // default type for customers
            } else {
                // Otherwise it belongs to a driver/owner
                $tableName = "user_register";
                $userType = null; // will determine later
            }
            // Fetch user details from the determined table
            $userQuery = mysqli_query($con, "SELECT name, lname, mobile, address, city, walletBalance, idProFront, idProBack, roll_id, nationality, residinglocation FROM `$tableName` WHERE id='$req_by'");
            $userData = [];
            if ($userQuery && mysqli_num_rows($userQuery) > 0) {
                $userData = mysqli_fetch_assoc($userQuery);
            } else {
                // Fallback: user not found in expected table – log or handle silently
                $userData = [];
            }
            // Determine type if table is user_register
            if ($tableName == 'user_register') {
                // Check kyc_details for type (Driver/Owner)
                $kyc_query = mysqli_query($con, "SELECT type FROM kyc_details WHERE user_id='$req_by' AND deletes='0' LIMIT 1");
                if ($kyc_query && mysqli_num_rows($kyc_query) > 0) {
                    $kycData = mysqli_fetch_assoc($kyc_query);
                    $userType = !empty($kycData['type']) ? $kycData['type'] : 'Driver'; // default to Driver if type empty
                } else {
                    // No kyc, try role name
                    $roll_id_user = $userData['roll_id'] ?? 0;
                    if ($roll_id_user != 0) {
                        $roleName = select_top_name($con, "role", "name", "`id`='$roll_id_user'", "name", "");
                        $userType = !empty($roleName) ? $roleName : 'Unknown';
                    } else {
                        $userType = 'Unknown';
                    }
                }
            }
            // If still null (should not happen), default to 'Unknown'
            ...
            $From_roll_id = $userData['roll_id'] ?? 0;
            // FIX: FETCH UPI ID FROM THE WITHDRAW_REQUEST TABLE
            $upiID = $value['upiID'] ?? '';
            $to_name = ''; // Required to prevent JSON breakage for Non-Admin
            $createdon = $value['createdon'];
            $dateParts = explode(' ', $createdon);
            $dateOnly = $dateParts[0];
            $attachmentimg = $value['attachment_url'];
            $safeAssetUrl = defined('assetURL') ? constant('assetURL') : '';
            $safeBaseUrl = isset($baseurl) ? $baseurl : '';
            $path = (strpos($attachmentimg, "nationaldraw") === 0) ? $safeAssetUrl . $attachmentimg : $safeBaseUrl . $attachmentimg;
            $path1 = $safeAssetUrl . $idProFront;
            $path2 = $safeAssetUrl . $idProBack;
            $status = '';
            $action = '';
            if ($value['status'] == 0 || $value['status'] == '' || $value['status'] == null) {
                if ($roll_id == 1 || $roll_id == 2 || $roll_id == 4) {
                    $action .= '<a class="btn text-danger btn-sm transfer" style="cursor: pointer;" data-bs-original-title="Transfer"><span class="fa fa-money" onclick="withdrawtransfer(' . "'$request_id'" . ')"> Transfer</span></a>';
            ...
            if ($value['status'] == 1) {
                ...
                $status = 'Reject';
            } else if ($value['status'] == 0 || $value['status'] == '' || $value['status'] == null) {
                $status = 'Process';
            }
            if ($value['status'] == 1 || $value['status'] == 2) {
                ...
                $action .= '<a class="btn text-danger btn-sm transfer" style="cursor: pointer;" data-bs-original-title="Bank Details"><span class="fa fa-eye" style="color: #ff0000;" onclick="Bankdeatils(' . "'$request_id'" . ')"> Bank Details</span></a>';
                $action .= '<a class="btn text-danger btn-sm transfer" style="cursor: pointer;" data-bs-original-title="ID Proof"><span class="fa fa-picture-o" style="color: #a4d47a;" onclick="user_id_image(' . "'$request_id', '$path1', '$path2'" . ')"> Id Proof </span></a>';
                if ($attachmentimg != '') {
                    $action .= ' <a href="' . $path . '"  target="_blank"><button class="btn"><i  style="color: lime;" class="fa fa-eye"></i> View</button></a>';
                }
            }
            $result[] = array(
                ...
                "upiId" => $upiID, // Passed dynamically to JS
    ...
    // Always return a JSON array to prevent empty table crashes
    ...
    if ($wd_data['nr'] == 0) {
        $result['type'] = '0';
        $result['result'] = 'Request Not Found or Already Processed';
        echo json_encode($result);
        exit;
    }
    $withdraw = $wd_data['result'][0];
    ...
    // ------------------- FILE UPLOAD -------------------
    if (empty($_FILES['filesent']['name'])) {
        $result['type'] = '0';
        $result['result'] = 'Please select a file to upload';
        echo json_encode($result);
        exit;
    }
    if ($_FILES['filesent']['error'] !== UPLOAD_ERR_OK) {
        $errorMessages = [
        ...
        $errorCode = $_FILES['filesent']['error'];
        $result['type'] = '0';
        $result['result'] = 'Upload error: ' . ($errorMessages[$errorCode] ?? 'Unknown error');
        echo json_encode($result);
        exit;
    }
    $allowed = ['jpg', 'png', 'webp', 'pdf'];
    $file_size = $_FILES['filesent']['size'];
    $file_temp = $_FILES['filesent']['tmp_name'];
    $file_type = $_FILES['filesent']['type'];
    $file_name = $_FILES['filesent']['name'];
    $ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
    if (($file_size / 1024) >= 2048) {
        $result['type'] = '0';
        $result['result'] = 'File size must be less than 2MB';
        echo json_encode($result);
        exit;
    }
    ...
    $filePath = new CURLFile($file_temp, $file_type, $file_name);
    $postData = ['image' => $filePath, 'id' => $loginID];
    $uploadResponse = fileMoveS3($postData);
    $fileUpload = json_decode($uploadResponse, true);
    if (!is_array($fileUpload)) {
        $result['type'] = '0';
        $result['result'] = 'Invalid response from upload server';
        echo json_encode($result);
        exit;
    }
    if (isset($fileUpload['status']) && $fileUpload['status'] === true) {
        $fileNEW = $fileUpload['data']['url'] ?? '';
        if (empty($fileNEW)) {
            $result['type'] = '0';
            $result['result'] = 'Upload succeeded but no URL returned';
            echo json_encode($result);
            exit;
        }
    } else {
        $errorMsg = $fileUpload['message'] ?? 'File upload failed';
        $result['type'] = '0';
    ...
    try {
        ...
        // Build mobile condition if available
        $mobile_cond = "";
        if (!empty($req_mobile_check)) {
            $mobile_cond = " OR mobile='$req_mobile_check'";
        }
        // Fetch user details from the determined table
        $u_data = mysqli_query($con, "SELECT name, lname, mobile, walletBalance FROM `$tableName` WHERE (id='$from_id' $mobile_cond) AND deletes='0' LIMIT 1");
        if (!$u_data || mysqli_num_rows($u_data) == 0) {
            throw new Exception("User details not found in $tableName.");
        }
        $userRow = mysqli_fetch_assoc($u_data);
        $current_balance = floatval($userRow['walletBalance'] ?? 0);
        $req_name = trim($userRow['name'] ?? '');
        $req_lname = trim($userRow['lname'] ?? '');
        $req_fullname = trim($req_name . ' ' . $req_lname);
        if ($req_fullname == '') $req_fullname = 'Unknown';
        $req_mobile = $userRow['mobile'] ?? '';
        // Check balance
        if ($current_balance < $withdraw_amount) {
            throw new Exception("Insufficient wallet balance in $tableName.");
        }
        $new_balance = $current_balance - $withdraw_amount;
        // Deduct from the determined table
        $update_wallet = mysqli_query($con, "UPDATE `$tableName` SET `walletBalance`='$new_balance' WHERE (`id`='$from_id' OR `mobile`='$req_mobile') AND deletes='0'");
        if (!$update_wallet) {
            throw new Exception("Failed to update wallet balance in $tableName");
        }
        // Insert into walletBalance_history
        $history_data = [
            'userid'           => $from_id,
            'uname'            => $req_fullname,
            'umobile'          => $req_mobile,
        ...
        $insert_history = insert($con, "walletBalance_history", "", $history_data, "", "", "");
        if ($insert_history['errors']) {
            throw new Exception('Failed to insert wallet history: ' . $insert_history['errors']);
        }
        ...
        if ($wd_update['errors']) {
            throw new Exception('Failed to update withdrawal request: ' . $wd_update['errors']);
        }
        mysqli_commit($con);
        // ------------------- SEND NOTIFICATIONS -------------------
        $sms_message = "Hi $req_fullname, your withdrawal of INR $withdraw_amount (Request ID: $request_id) has been successfully transferred. It will reflect in your bank account within 24 hours. Thank you for participating!";
        sendNotification([
            'mobile'   => $req_mobile,
        ...
        $whatsapp_endpoint = 'https://www.goride.run/api/send-withdrawal-whatsapp';
        $whatsapp_payload = json_encode([
            'mobile'     => $req_mobile,
            'name'       => $req_fullname,
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, $whatsapp_payload);
        ...
        curl_close($ch);
        $result['type'] = '1';
        $result['result'] = 'Transfer completed successfully.';
    } catch (Exception $e) {
        mysqli_rollback($con);
        $result['type'] = '0';
...
//     // var_dump($request,$mobile,$from_name,$createdon);die;
//     if ($request != '') {
//         $point_req1 = select_query($con, "withdraw_request", "", "`request_id` = '$request' and `deletes` = '0' and `status` ='0'  ORDER BY `id` DESC", "", "");
//         if ($point_req1['nr'] > 0) {
//             // $result['rejectbtn'] = '<button class="btn btn-primary fa fa-whatsapp" onclick="reject_request(' . "'$request',$mobile,'$from_name','$createdon'" . ')"> Send</button>';
//             $result['rejectbtn'] = '<button class="btn-primary" onclick="reject_request(' . "'$request',$mobile,'$from_name','$createdon'" . ')"> Send</button>';
//             $result['result'] = '<textarea id="rejectreasontext" placeholder="Enter the reason for rejection"></textarea>';
//             $result['type'] = 1;
//         } else {
//             $result['result'] = 'Request ID Not Found!';
//             $result['type'] = 0;
...
// } 
    ...
    if ($request != '') {
        $point_req1 = select_query($con, "withdraw_request", "", "`request_id` = '$request' and `deletes` = '0' and `status` ='0'  ORDER BY `id` DESC", "", "");
        if ($point_req1['nr'] > 0) {
            // Removed WhatsApp icon classes; button is now a normal Bootstrap primary button
            $result['rejectbtn'] = '<button class="btn btn-primary" onclick="reject_request(' . "'$request',$mobile,'$from_name','$createdon'" . ')"> Send</button>';
            $result['result'] = '<textarea id="rejectreasontext" placeholder="Enter the reason for rejection"></textarea>';
            $result['type'] = 1;
        } else {
            $result['result'] = 'Request ID Not Found!';
...
//     if ($request != '' && $reason != '') {
//         $point_req = select_query($con, "withdraw_request", "", "`request_id` = '$request' and `deletes` = '0' and `status` ='0'  ORDER BY `id` DESC", "", "");
//         if ($point_req['nr'] > 0) {
...
//                         if ($erarn_trans['id'] != '') {
//                             $message = 'Hello {{name}}, 📤We regret to inform you that your recent withdrawal request, with reference ID {{ID}} has been rejected. Kindly find the reason for rejection here👇 {{rejected reason}} 🗳️Correct the above-rejected reason and submit again!!! 📮';
//                             $message = str_replace('{{name}}', $req_fullname, $message);
//                             $message = str_replace('{{ID}}', $request, $message);
//                             $message = str_replace('{{rejected reason}}', $reason, $message);
//                             $request = [
//                                 'mobile' => $req_mobile,
//                                 'messages' => $message,
//                                 'con' => $con,
...
//                             } else {
//                                 $result['type'] = '1';
//                                 $result['result'] = 'Rejected Successfully But message not send';
//                             }
//                         }
//                     }
//                 }
...
// } 
    ...
    $request = $_REQUEST['request'] ?? '';
    $reason  = $_REQUEST['reason'] ?? '';
    $dubaidate_time = date('Y-m-d H:i:s');   // define timestamp
    if ($request == '' || $reason == '') {
        $result['result'] = 'Request ID or Reason missing!';
        $result['type'] = 0;
        echo json_encode($result);
        exit;
    }
    // Fetch the pending withdrawal request
    $point_req = select_query(
        $con,
        "withdraw_request",
        "",
        "`request_id` = '$request' AND `deletes` = '0' AND `status` = '0' ORDER BY `id` DESC",
        "",
        ""
    );
    if ($point_req['nr'] == 0) {
        $result['result'] = 'Request ID Not Found or Already Processed!';
        $result['type'] = 0;
        echo json_encode($result);
        exit;
    }
    $withdraw = $point_req['result'][0];
    $user_id  = $withdraw['requested_by'];
    $amt      = $withdraw['amount'];
    $Wid      = $withdraw['id'];               // withdraw_request primary key
    $cus_prefer = $withdraw['cus_prefer'] ?? 'bank';
    $request_id = $withdraw['request_id'];
    // Determine which user table to use based on request ID prefix
    if (strpos($request_id, 'WRC-') === 0) {
        $userTable = 'customer_register';
    } else {
        $userTable = 'user_register';
    }
    // Build SELECT list – include email only for user_register (to avoid errors)
    if ($userTable == 'user_register') {
        $selectFields = "name, lname, mobile, email, walletBalance";
    } else {
        $selectFields = "name, lname, mobile, walletBalance";  // email may not exist
    }
    // Fetch current user details from the correct table
    $userQuery = mysqli_query(
        $con,
        "SELECT $selectFields 
         FROM `$userTable` 
         WHERE id = '$user_id' AND deletes = '0' LIMIT 1"
    );
    if (!$userQuery || mysqli_num_rows($userQuery) == 0) {
        $result['result'] = 'User not found in ' . $userTable;
        $result['type'] = 0;
        echo json_encode($result);
        exit;
    }
    $user = mysqli_fetch_assoc($userQuery);
    $req_name     = trim($user['name'] ?? '');
    $req_lname    = trim($user['lname'] ?? '');
    $req_fullname = $req_name . ' ' . $req_lname;
    if (trim($req_fullname) == '') $req_fullname = 'Unknown';
    $req_mobile = $user['mobile'] ?? '';
    $req_email  = $user['email'] ?? '';        // empty for customers, fine
    $open_amt   = floatval($user['walletBalance'] ?? 0);
    $total_amt  = $open_amt + floatval($amt);
    // Begin transaction
    mysqli_begin_transaction($con);
    // 1. Update withdraw_request status to rejected (2)
    $wd_arr = [
        "submited_by" => $_SESSION['memid'] ?? 0,
        "reasontext"  => $reason,
    ...
    $wd_update = update(
        $con,
        "withdraw_request",
        "`request_id` = '$request' AND `deletes`='0'",
        $wd_arr,
        "",
        "",
        "",
    ...
    if ($wd_update['errors']) {
        mysqli_rollback($con);
        $result['type'] = '0';
        $result['result'] = 'Failed to update withdrawal request: ' . $wd_update['errors'];
        echo json_encode($result);
        exit;
    }
    // 2. Credit amount back to user's wallet
    $update_wallet = mysqli_query(
        $con,
        "UPDATE `$userTable` SET `walletBalance` = '$total_amt' 
         WHERE id = '$user_id' AND deletes = '0'"
    );
    if (!$update_wallet) {
        mysqli_rollback($con);
        $result['type'] = '0';
        $result['result'] = 'Failed to credit wallet balance in ' . $userTable;
        echo json_encode($result);
        exit;
    }
    // 3. Insert into walletBalance_history (same fields as in transfer_wd)
    $reward_type = ($cus_prefer == 'exchange') ? 'RE_EXCHANGEWITHDRAWAL' : 'RE_BANKWITHDRAWAL';
    $walletHistoryData = [
        'userid'           => $user_id,
        'uname'            => $req_fullname,
        'umobile'          => $req_mobile,
        'username'         => '',                     // matches transfer_wd
        'opening_balance'  => $open_amt,
        'total'            => $amt,
        'closing_balance'  => $total_amt,
        ...
        'deletes'          => '0',
        'status'           => '1',
        'created_by'       => $_SESSION['memid'] ?? 0
    ];
    $insertHistory = insert($con, "walletBalance_history", "", $walletHistoryData, "", "", "");
    if ($insertHistory['errors']) {
        mysqli_rollback($con);
        $result['type'] = '0';
        $result['result'] = 'Failed to insert wallet history: ' . $insertHistory['errors'];
        echo json_encode($result);
        exit;
    }
    // Commit transaction
    mysqli_commit($con);
    // Return success (no SMS sent)
    $result['type'] = '1';
    $result['result'] = 'Rejected Successfully';
    echo json_encode($result);
    exit;
    ...
    foreach ($user_register['result'] as $key => $value) {
        $result[] = [
            ...
            "email" => $value['email'],
            "livein" => $value['id'],
            "creadedon" => $value['created_at'],
            "lostlogin" => $value['lastlogin'],
            "name" => $value['name']
        ];
    }
    resultJ00:
