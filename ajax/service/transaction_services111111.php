   ...
   if ($roll_id == 1) {
    ...
    $sql = mysqli_query($con, "SELECT P.id, ROUND(P.rate) AS 'productAmt', IFNULL(ROUND(I.totalAmt), 0) AS 'totalAmt'
        ...
        LEFT JOIN (
            SELECT I.product_id, SUM(I.netTotal) AS 'totalAmt'
            FROM `invoice` AS I
            WHERE I.deletes = '0' AND I.agent_id='0'AND DATE(I.createdon) = CURDATE() AND I.renewalStatus = 'new'
            GROUP BY I.product_id
        ) AS I ON P.id = I.product_id
        WHERE P.deletes = '0';");
    ...
    $renewalSql = mysqli_query($con, "SELECT P.id, ROUND(P.rate) AS 'productAmt', IFNULL(ROUND(I.totalAmt), 0) AS 'totalAmt'
        ...
        LEFT JOIN (
            SELECT I.product_id, SUM(I.netTotal) AS 'totalAmt'
            FROM `invoice` AS I
            WHERE I.deletes = '0' AND I.agent_id='0'AND DATE(I.createdon) = CURDATE() AND I.renewalStatus = 'renewal'
            GROUP BY I.product_id
        ) AS I ON P.id = I.product_id
        WHERE P.deletes = '0';");
    ...
    $output .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xl-9"> <div class="row"> <div class="col-lg-6 col-md-6 col-sm-12 col-xl-4"><div class="card overflow-hidden"><div class="card-body"><div class="d-flex"><div class="mt-2"><h6 class="">Deleted Account Request</h6><h2 class="mb-0 number-font" id="totalonline">' . intval($totalAmount) . '</h2>';
    $output .= '</div></div></div></div></div>';
    ...
    $search_id1 = BlockSQLInjectionforagent($_POST["search_id1"]);
    
    // var_dump($search_id1);die;
    ...
    if ($formdate != '' && $todate != '') {
     ...
     if ($search_id1 != '') {
         $contype = '';
            $conition = "(`uname` LIKE '%$search_id1%' OR `umobile` LIKE '%$search_id1%' OR `uemail` LIKE '%$search_id1%') AND ";
    ...
    if ($tablename == 'pointtable') {
        ...
        // $point_Trans = select_query($rcon, "ndbank_history", "", "$conition $contype `deletes`='0' AND `reference_id` !='0  ORDER BY `bu` DESC' ", "", "");
        $point_Trans = select_query($rcon, "ndbank_history", "", "$conition $contype `deletes`='0' AND `reference_id` != '0' ORDER BY `id` DESC", "", "");
        ...
        if ($point_Trans['nr'] > 0) {
            foreach ($point_Trans['result'] as $key => $value) {
                ...
                $cusname = $value['uname'];
                $cusMobile = $value['umobile'];
                $cusemail = $value['uemail'];
                $cusold_wallet = $value['opening_balance'];
                $cus_re_point = $value['total'];
                $cus_wallet_balance = $value['closeing_balance'];
                $cus_status = $value['transaction_type'];
                $cus_Transactiontype = $value['point_type'];
                $old_Balance = $value['ND_Bank'];
               

               
                $result['result'][] = array("name" => $cusname, "moble" => $cusMobile, "email" => $cusemail, "ND_Bank" => $old_Balance, "opening_balance" => $cusold_wallet, "re_point" => $cus_re_point, "wallet_balance" => $cus_wallet_balance, "status" => $cus_status, "transaction_type" => $cus_Transactiontype, "date" => date("d-M-Y g:i a", strtotime($value['createdon'])));
            }
