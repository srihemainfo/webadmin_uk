...
// }
    ...
    // $pushnotifi = select_query($rcon, "push_notifications", "title,body,status,created_at,id", " $contype `id`!= '' ", "", "");
    ...
    $sql = "
        ...
        LEFT JOIN user_register AS user 
            ON user.id = push_notifications.user_id WHERE $contype 
    ...
    if ($pushnotifi && mysqli_num_rows($pushnotifi) > 0) {
        ...
        while ($value = mysqli_fetch_assoc($pushnotifi)) {
            ...
            $result[] = [
                ...
                "name"       => $value['name'],
                "title"      => $value['title'],
                "body"       => '<textarea readonly>' . htmlspecialchars($value['body']) . '</textarea>',
                "status"     => $value['status'] == 1 ? 'Delivered' : 'Not send',
