...
elseif ($method == 'cron_deduction_report') {
    ...
    // Fetches the deduction, live balance, refund date, and the log data (if refunded)
    $query = "SELECT 
                w.id AS ID, 
                w.userid,
                w.uname AS name, 
                ...
                (SELECT reason FROM manual_job_refund_logs ml 
                 WHERE ml.job_no = COALESCE(p1.job_no, p2.job_no, cjt.job_no, w.reference_id) 
                 ORDER BY id DESC LIMIT 1) AS refund_reason,
                 
                (SELECT ur.name FROM manual_job_refund_logs ml 
                 JOIN user_register ur ON ml.mem_id = ur.id 
                 WHERE ml.job_no = COALESCE(p1.job_no, p2.job_no, cjt.job_no, w.reference_id) 
                 ORDER BY ml.id DESC LIMIT 1) AS refunded_by_name
    ...
    foreach ($rows as $row) {
        ...
        if ($is_refunded) {
            ...
            // Populate extended details for Modal
            $payment_details[] = [
                'amount' => number_format($deducted, 2),
                'date' => $row['refund_date'],
                'transaction_id' => '-',
                'gateway' => 'REFUNDED',
                'refunded_by' => !empty($row['refunded_by_name']) ? $row['refunded_by_name'] : 'N/A',
                'reason' => !empty($row['refund_reason']) ? $row['refund_reason'] : 'Due to Job Cancelled'
            ];
        ...
        // Apply Status Filter BEFORE adding to output array
        if (!empty($statusFilter) && $displayStatus !== $statusFilter) {
            continue; // Skip the row if it does not match the dropdown filter
        }
...
elseif ($method == 'process_manual_refund') {
    
    $job_no = mysqli_real_escape_string($con, trim($_POST['job_no']));
    $reason = mysqli_real_escape_string($con, trim($_POST['reason']));
    $mem_id = isset($_POST['mem_id']) ? intval($_POST['mem_id']) : 0; 
    
    if (empty($job_no) || empty($reason)) {
        echo json_encode(['status' => false, 'message' => 'Missing required fields.']);
        exit;
    }

    // 1. Get Job Details First (Matches webCancelJob logic)
    $jobQuery = mysqli_query($con, "SELECT id, assigned_to, deductAmt FROM cus_job_temp WHERE job_no = '$job_no' LIMIT 1");
    ...
    $deductAmt = (float)$job['deductAmt'];

    if ($assigned_to == 0 || $deductAmt == 0) {
        echo json_encode(['status' => false, 'message' => 'No deduction amount exists or no driver was assigned to this job.']);
        exit;
    }

    // 2. Get Driver Info from user_register (Matches webCancelJob logic)
    $driverQuery = mysqli_query($con, "SELECT id, name, mobile, email, walletBalance FROM user_register WHERE id = '$assigned_to' AND deletes = '0' LIMIT 1");
    
    if (!$driverQuery || mysqli_num_rows($driverQuery) == 0) {
        echo json_encode(['status' => false, 'message' => 'Driver not found or account deleted.']);
        exit;
    }
    
    $driver = mysqli_fetch_assoc($driverQuery);

    // 3. Verify the Original Deduction Exists (Matches checkDeduct in webCancelJob)
    $checkDeductQuery = mysqli_query($con, "SELECT id FROM walletBalance_history WHERE userid = '$assigned_to' AND transaction_type = 'DEBIT' AND reference_id = '$job_id' AND reference_table = 'cus_job_temp' AND total = '$deductAmt' LIMIT 1");
    
    if (mysqli_num_rows($checkDeductQuery) == 0) {
        echo json_encode(['status' => false, 'message' => 'Original deduction not found for this driver and job.']);
        exit;
    }

    // 4. Prevent Double Refund Check
    $checkRefundQuery = mysqli_query($con, "SELECT id FROM walletBalance_history WHERE userid = '$assigned_to' AND transaction_type = 'REFUND' AND reference_id = '$job_id' AND reference_table = 'cus_job_temp' LIMIT 1");
    
    if (mysqli_num_rows($checkRefundQuery) > 0) {
        echo json_encode(['status' => false, 'message' => 'This job deduction has already been refunded.']);
        exit;
    }

    // 5. Calculate Wallet Balances
    $opening        = (float)$driver['walletBalance'];
    $expectedAmount = $deductAmt;
    $closing        = $opening + $expectedAmount;
    
    // Clean driver data for insertion
    $uname   = mysqli_real_escape_string($con, $driver['name']);
    $umobile = mysqli_real_escape_string($con, $driver['mobile']);
    $uemail  = mysqli_real_escape_string($con, $driver['email']);
    $now     = date('Y-m-d H:i:s');
    $ip      = isset($_SERVER['REMOTE_ADDR']) ? mysqli_real_escape_string($con, $_SERVER['REMOTE_ADDR']) : 'MANUAL_REFUND';

    // 6. Insert into Wallet History (Includes Name, Email, Mobile just like webCancelJob)
    $insertWallet = "INSERT INTO walletBalance_history 
                    (userid, uname, umobile, uemail, opening_balance, total, closeing_balance, point_type, transaction_type, reward_type, reference_id, reference_table, ip, createdon, updatedon)
                    VALUES 
                    ('$assigned_to', '$uname', '$umobile', '$uemail', '$opening', '$expectedAmount', '$closing', 'WALLET', 'REFUND', 'JOB', '$job_id', 'cus_job_temp', '$ip', '$now', '$now')";
                    
    if (mysqli_query($con, $insertWallet)) {
        
        // 7. Update Driver's Wallet Balance in user_register
        mysqli_query($con, "UPDATE user_register SET walletBalance = '$closing' WHERE id = '$assigned_to'");

        // 8. Log the Manual Refund action
        $logQuery = "INSERT INTO manual_job_refund_logs (job_no, mem_id, reason, created_at) 
                     VALUES ('$job_no', '$mem_id', '$reason', '$now')";
        mysqli_query($con, $logQuery);

        echo json_encode(['status' => true, 'message' => 'Job deduction has been successfully refunded to the driver.']);
    } else {
        echo json_encode(['status' => false, 'message' => 'Database error while processing refund.']);
    }
