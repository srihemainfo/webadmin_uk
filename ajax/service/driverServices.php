...
if ($method == "update_driver_profile") {
    ...
    // Fetch KYC data specifically to track the old Experience value accurately
    $getKYC = select_query($con, "kyc_details", "exp", "user_id = '$driver_id' AND deletes = 0 ORDER BY id DESC LIMIT 1", "", "");
    $old_exp = ($getKYC['nr'] > 0) ? $getKYC['result'][0]['exp'] : '';
    ...
    // ==========================================
    // 🔥 FIX: Escaping JSON strings for SQL safely
    // ==========================================
    ...
    // Update experience in kyc_details (matched to where get_driver_details selects it)
    if ($exp !== '') {
        update($con, "kyc_details", "`user_id` = '$driver_id'", ['exp' => $exp], "", "", "", "");
    }
...
elseif($method=== "fetch_job_logs") {
    ...
    // CHANGED from !empty() to isset() to allow clearing dates
...
elseif ($method === "fetch_job_push_status") {
    ...
    $job_status = isset($_POST['job_status']) ? strtolower(trim($_POST['job_status'])) : '';
    ...
    // FIX: If it's an assigned job, OR if the status is accepted/completed (which means it's coming from open_jobs)
    // Grab the original ID from cus_job_temp using job_no
    if ($is_assigned_job || in_array($job_status, ['accept', 'accepted', 'completed'])) {
        if (!empty($job_no)) {
    ...
    // Using LIKE ensures it catches the ID regardless of MySQL JSON function support or spacing
    $query = "SELECT id, title, body, res_json, created_at FROM push_notifications 
              WHERE req_json LIKE '%\"job_id\": $job_id%' 
                 OR req_json LIKE '%\"job_id\":$job_id%' 
