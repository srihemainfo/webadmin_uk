...
if ($method == "collect_earning") {
    ...
    exit;
        ...
        // Ã°Å¸Å¡â‚¬ BUG FIX: Sync logic perfectly with fetch_customer_ocr to prevent count mismatches
        // Now filtering based on the document submission dates, not the account creation date
        $query = "SELECT c.id, c.doc_verify, k.proof_status, k.selfie_status 
                  ...
                  WHERE c.deletes = '0' 
                  AND (k.user_id IS NOT NULL OR o_rc.user_id IS NOT NULL OR o_dl.user_id IS NOT NULL)
                  AND (k.created_at BETWEEN '$startDateTime' AND '$endDateTime' 
                       OR o_rc.created_at BETWEEN '$startDateTime' AND '$endDateTime' 
        ...
        if ($res) {
            while ($row = mysqli_fetch_assoc($res)) {
                ...
                if ($row['doc_verify'] == 1) {
                    $verified++;
                } else if ($row['proof_status'] == 'rejected' || $row['selfie_status'] == 'rejected') {
                    $rejected++;
                } else {
                    $pending++;
                }
...
elseif ($method == 'app_jobs_chart') {
    ...
    $tabType = isset($_POST['tabType']) ? mysqli_real_escape_string($con, $_POST['tabType']) : 'current'; // NEW: Get Tab parameter
    ...
    // --- NEW: Apply Tab specific logic ---
    if ($tabType == 'cancelled') {
        // Only cancelled jobs
        $sql .= " AND c.job_status = 'cancelled'";
    } else if ($tabType == 'completed') {
        // Not cancelled AND Pickup Date + 1 Day has passed
        $sql .= " AND c.job_status != 'cancelled' AND DATE_ADD(c.pickup_date, INTERVAL 1 DAY) < NOW()";
    } else { 
        // Current: Not cancelled AND Pickup Date + 1 Day is in the future
        $sql .= " AND c.job_status != 'cancelled' AND DATE_ADD(c.pickup_date, INTERVAL 1 DAY) >= NOW()";
    }
    // -------------------------------------
