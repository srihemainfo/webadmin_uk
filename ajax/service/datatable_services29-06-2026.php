...
if ($method == "list_agent") {
    ...
    // NEW: Capture State and District filters
    $filter_state = isset($_POST['filter_state']) ? mysqli_real_escape_string($con, trim($_POST['filter_state'])) : '';
    if ($filter_state !== '') {
        $query_parts[] = "`current_state`='$filter_state'";
    }
    $filter_district = isset($_POST['filter_district']) ? mysqli_real_escape_string($con, trim($_POST['filter_district'])) : '';
    if ($filter_district !== '') {
        $query_parts[] = "`current_district`='$filter_district'";
    }
    $type = $_POST['type'] ?? '';
    ...
    if ($datefrom == '' && $datefill == '' && $fieldname == '' && $deletedstatus == '' && $filter_state == '' && $filter_district == '' && $type != 'agent' && $type != 'staffs') {
        ...
        foreach ($user_register['result'] as $key => $value) {
            ...
            if ($roll_id == 1) {
                if (in_array($value['roll_id'], [5, 6, 7, 8, 0])) {
                    ...
                    $action .= '<a href="javascript:void(0);" onclick="openPushModal(\'' . $value['id'] . '\')" style="color: #49117cc9; font-weight: bold;"><i class="fa fa-bell"></i> SEND PUSH</a>';
                    if (intval($deletes) != 1) {
            ...
            $result[] = [
                ...
                "mobile" => $value['mobile'] ?? '',
                "email" => $value['email'] ?? '',
                "walletBalance" => $value['walletBalance'] ?? '0.00',
                "cash_points" => $value['cash_points'] ?? '0',
                "dob" => $value['dob'] ?? '',
                "password" => $value['password'] ?? '',
                "current_district" => $value['current_district'] ?? '', // Added
                "current_state" => $value['current_state'] ?? '',       // Added
                "current_address" => $value['current_address'] ?? ''    // Added
            ];
        }
    }
    // Force JSON output
    ...
    exit;
} elseif ($method == "list_agent_customer") {
    $query_parts = [];
    global $con, $rcon, $adminurl, $role, $whereClause;
    $datefill = isset($_POST["datefill"]) ? mysqli_real_escape_string($con, trim($_POST["datefill"])) : '';
    $datefrom = isset($_POST["datefrom"]) ? mysqli_real_escape_string($con, trim($_POST["datefrom"])) : '';
    $now = date('Y-m-d');
    if ($datefrom != '' && $datefill != '') {
    ...
    $state_filter = isset($_POST["state_filter"]) ? mysqli_real_escape_string($con, trim($_POST["state_filter"])) : '';
    if ($state_filter != '') {
        $query_parts[] = "`current_state` = '$state_filter'";
    }
    $deletedstatus = isset($_POST['deletedstatus']) ? mysqli_real_escape_string($con, trim($_POST['deletedstatus'])) : '';
    if ($deletedstatus !== '') {
        $query_parts[] = "`deletes`='$deletedstatus'";
    ...
    if ($datefrom == '' && $datefill == '' && $fieldname == '' && $state_filter == '' && $deletedstatus == '') {
        $query_parts[] = "`created_at` LIKE '%$now%'";
    }
    if (!empty($role)) {
        $clean_role = trim(preg_replace('/AND\s*$/i', '', trim($role)));
    ...
    $columns = "`id`, `deletes`, `address`, `nationality`, `city`, `residinglocation`, `created_at`, `user`, `deviceType`, `name`, `lname`, `t_point`, `mobile`, `email`, `walletBalance`, `cash_points`, `dob`, `password`, `current_state`, `current_district`, `current_address`";
    $customer_register = select_query($rcon, "customer_register", $columns, "$final_where ORDER BY `id` DESC", "", "");
    $result = [];
    if (isset($customer_register['nr']) && $customer_register['nr'] > 0) {
        foreach ($customer_register['result'] as $value) {
            $id = $value['id'];
            $deletes = intval($value['deletes'] ?? 0);
            $action = '<a href="javascript:void(0);" onclick="viewCustomerJobs(' . $id . ')" style="color: #0b8e36; font-weight: bold; margin-left: 10px;"><i class="fa fa-briefcase"></i> VIEW BOOKINGS</a>&nbsp;&nbsp;' .
                '<a href="javascript:void(0);" onclick="openPushModal(\'' . $id . '\')" style="color: #49117cc9; font-weight: bold;"><i class="fa fa-bell"></i> SEND PUSH</a>&nbsp;&nbsp;';
            if ($deletes !== 1) {
                $action .= '<a class="btn text-danger btn-sm" data-bs-target="#deleteinfo" data-bs-toggle="modal" data-bs-toggle="tooltip" style="cursor: pointer;" data-bs-original-title="Delete"><span class="fe fe-trash-2" style="color: #ff0000;font-size: 16px;font-weight: bold;" onclick="deletelist(' . $id . ')"></span></a>';
            }
            $result[] = [
                "buildinglocation" => utf8_encode($value['address'] ?? ''),
                "o_count"          => 0,
                ...
                "dob"              => $value['dob'] ?? '',
                "password"         => $value['password'] ?? '',
                "current_state"    => !empty(trim($value['current_state'] ?? '')) ? $value['current_state'] : 'N/A',
                "current_district" => !empty(trim($value['current_district'] ?? '')) ? $value['current_district'] : 'N/A',
                "current_address"  => !empty(trim($value['current_address'] ?? '')) ? $value['current_address'] : 'N/A'
            ];
        }
    }
    unset($customer_register);
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode($result, JSON_INVALID_UTF8_SUBSTITUTE);
    exit;
} elseif ($method == "utm_customer_report") {
    $query_parts = [];
    global $con, $rcon, $adminurl, $role, $whereClause;
    // Fetch date filters sent from frontend
    $datefrom = isset($_POST["from_date"]) ? mysqli_real_escape_string($con, trim($_POST["from_date"])) : '';
    $dateto = isset($_POST["to_date"]) ? mysqli_real_escape_string($con, trim($_POST["to_date"])) : '';
    if ($datefrom != '' && $dateto != '') {
        $query_parts[] = "DATE(`created_at`) BETWEEN '$datefrom' AND '$dateto'";
    }
    // Filter for active customers only
    $query_parts[] = "`deletes`='0'";
    // Exclude empty or NULL UTM Campaigns so "Direct/None" doesn't show
    $query_parts[] = "`utm_campaign` IS NOT NULL AND `utm_campaign` != ''";
    $final_where = implode(" AND ", $query_parts);
    // Grouping by Campaign, Source, and Medium only
    $final_where .= " GROUP BY `utm_campaign`, `utm_source`, `utm_medium` ORDER BY total_customers DESC";
    $columns = "`utm_campaign`, `utm_source`, `utm_medium`, COUNT(`id`) as total_customers";
    $utm_report = select_query($rcon, "customer_register", $columns, $final_where, "", "");
    $result = [];
    if (isset($utm_report['nr']) && $utm_report['nr'] > 0) {
        foreach ($utm_report['result'] as $value) {
            $result[] = [
                "utm_campaign"    => $value['utm_campaign'],
                "utm_source"      => !empty(trim($value['utm_source'])) ? $value['utm_source'] : '(Not Set)',
                "utm_medium"      => !empty(trim($value['utm_medium'])) ? $value['utm_medium'] : '(Not Set)',
                "total_customers" => intval($value['total_customers'])
            ];
        }
        echo json_encode(["status" => true, "data" => $result], JSON_INVALID_UTF8_SUBSTITUTE);
    } else {
        echo json_encode(["status" => false, "message" => "No UTM campaign data found for the selected criteria."]);
    }
    exit;
} else if ($method == 'get_customer_jobs') {
    $user_id = isset($_POST['user_id']) ? intval($_POST['user_id']) : 0;
    $result = ['status' => false, 'regular' => [], 'carpool' => []];
    if ($user_id > 0) {
        // 👉 Added 'fare_breakdown' to the query so we can extract exact pricing
        $sql = "SELECT job_no, created_at, pickup_date, distance, from_place, to_place, fare, fare_breakdown, job_status, global_type, filled_seat
                FROM cus_job_temp
                WHERE user_id = '$user_id' AND deletes = '0'
                ORDER BY id DESC";
        ...
        if ($res && mysqli_num_rows($res) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($res)) {
                $isCarpool = (strtolower($row['global_type']) === 'carpool' || strpos($row['job_no'], 'GRP-') === 0);
                $distance = !empty($row['distance']) ? $row['distance'] : '0';
                $status = strtolower($row['job_status']);
                $pickupDate = strtotime($row['pickup_date']);
                // ==========================================
                // 👉 FARE CALCULATION (SYNCED WITH ADMIN LIST)
                // ==========================================
                $fareBreakdown = [];
                ...
                if ($status == 'created' || $status == 'bidding' || $status == 'schedule') {
                    $totalFare = array_key_exists('total_fare', $fareBreakdown) ? $fareBreakdown['total_fare'] : ($row['fare'] ?? 0);
                } else {
                    $totalFare = $row['fare'] ?? 0;
                }
                $formattedFare = number_format((float)$totalFare, 2);
                // ==========================================
                // STATUS LOGIC (Applied to Regular Jobs only)
                if (!$isCarpool) {
                    if ($status == 'cancelled') {
                        ...
                        $display_status = ($pickupDate < $currentTime) ? 'Incomplete' : 'Assigned';
                    } else {
                        $display_status = ($pickupDate < $currentTime) ? 'Expired' : 'Unassigned';
                    }
                }
                $jobData = [
                    'job_no'      => $row['job_no'],
                    'created_at'  => date('d M Y, h:i A', strtotime($row['created_at'])),
                    'pickup_date' => date('d M Y, h:i A', $pickupDate),
                    'distance'    => $distance,
                    'from_place'  => $row['from_place'],
                    'to_place'    => $row['to_place'],
                    'fare'        => $formattedFare // Mapped to our newly calculated fare
                ];
                // Only append status if it's a regular job
                if (!$isCarpool) {
                    $jobData['status'] = $display_status;
                }
                if ($isCarpool) {
                    $result['carpool'][] = $jobData;
                } else {
                    $result['regular'][] = $jobData;
                }
            }
            $result['status'] = true;
        }
    ...
    echo json_encode($result);
} else if ($_POST['method'] == 'get_synced_users_list') {
    // 1. Fetches the dashboard list of users and their total contact count
    $query = "SELECT cr.id, cr.name, cr.mobile, COUNT(uc.id) as contact_count 
              FROM customer_register cr 
              INNER JOIN user_contacts uc ON cr.id = uc.user_id 
              GROUP BY cr.id 
    ...
    if ($result) {
        ...
        echo json_encode(['type' => 1, 'data' => $data]);
    } else {
        echo json_encode(['type' => 0, 'message' => 'Failed to fetch users: ' . mysqli_error($con)]);
    }
    exit;
} else if ($_POST['method'] == 'get_all_synced_contacts') {
    // 2. Used for BACKGROUND PRELOADING. Fetches all contacts and groups them by user.
    $query = "SELECT user_id, contact_name, phone FROM user_contacts ORDER BY contact_name ASC";
    $result = mysqli_query($con, $query);
    $data = [];
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // Clean phone for the Copy All button
            $cleanPhone = preg_replace('/[^0-9+]/', '', $row['phone']);
            if (!empty($cleanPhone)) {
                $data[$uid]['raw_phones'][] = $cleanPhone;
            }
        }
        // Finalize comma separated strings
        foreach ($data as $uid => $user_data) {
            $data[$uid]['all_phones_str'] = implode(',', array_unique($user_data['raw_phones']));
            unset($data[$uid]['raw_phones']); // Remove temporary array to save bandwidth
        }
        echo json_encode(['type' => 1, 'data' => $data]);
    } else {
        echo json_encode(['type' => 0, 'message' => mysqli_error($con)]);
    }
    exit;
} else if ($_POST['method'] == 'get_user_contacts_details') {
    // 3. Fallback method just in case the preloader hasn't finished yet
    $user_id = mysqli_real_escape_string($con, $_POST['user_id']);
    $query = "SELECT contact_name, phone FROM user_contacts WHERE user_id = '$user_id' ORDER BY contact_name ASC";
    $result = mysqli_query($con, $query);
    $data = [];
    $phones = [];
    if ($result) {
        ...
        $commaSeparatedPhones = implode(',', array_unique($phones));
        echo json_encode(['type' => 1, 'data' => $data, 'all_phones_str' => $commaSeparatedPhones]);
    } else {
        echo json_encode(['type' => 0, 'message' => mysqli_error($con)]);
    }
    exit;
    ...
    $searchCondition = "";
    if ($searchKeyword != '') {
        $searchCondition = " AND (`ur`.`name` LIKE '%$searchKeyword%' 
                              OR `ur`.`email` LIKE '%$searchKeyword%' 
                              OR `ur`.`mobile` LIKE '%$searchKeyword%')";
    }
    // Added isBidding, isOpenjob, isPostJob, isPostSchJob to the SELECT query
    $sql = "SELECT `ur`.`id`, `ur`.`name`, `ur`.`email`, `ur`.`mobile`, 
            `ur`.`isBidding`, `ur`.`isOpenjob`, `ur`.`isPostJob`, `ur`.`isPostSchJob`,
            (SELECT IF(COUNT(*) > 0, 1, 0) FROM `blocked_user` 
             WHERE `user_id` = `ur`.`id` AND `deletes` = '0' AND `expiry_date` >= CURDATE()) as status 
            FROM `user_register` as `ur`
            INNER JOIN `kyc_details` as `kd` ON `kd`.`user_id` = `ur`.`id`
            LEFT JOIN `drivers_current_location` as `drl` ON `ur`.`id` = `drl`.`user_id`
            WHERE $baseCondition $searchCondition 
            ORDER BY `ur`.`id` DESC";
    $driverQuery = mysqli_query($con, $sql);
    if ($driverQuery && mysqli_num_rows($driverQuery) > 0) {
        $result = mysqli_fetch_all($driverQuery, MYSQLI_ASSOC);
    }
    echo json_encode($result);
    exit;
} elseif ($method == 'admin_change_driver_status') {
    global $con;
    $driverId = isset($_POST['driver_id']) ? (int)$_POST['driver_id'] : 0;
    $newStatus = isset($_POST['status']) ? (int)$_POST['status'] : 0;
    $reason = isset($_POST['reason']) ? mysqli_real_escape_string($con, $_POST['reason']) : 'No reason provided';
    $memid = isset($_SESSION['memid']) ? (int)$_SESSION['memid'] : (isset($_POST['memid']) ? (int)$_POST['memid'] : 0);
    if ($driverId > 0 && $memid > 0) {
        $success = false;
        if ($newStatus == 1) {
            $expiry_date = isset($_POST['expiry_date']) && !empty($_POST['expiry_date']) ? mysqli_real_escape_string($con, $_POST['expiry_date']) : date('Y-m-d', strtotime('+1 day'));
            $block_date = date('Y-m-d');
            $current_datetime = date('Y-m-d H:i:s');
            $datetime1 = new DateTime($block_date);
            $datetime2 = new DateTime($expiry_date);
            $interval = $datetime1->diff($datetime2);
            $day_count = $interval->days;
            $logSql = "INSERT INTO `blocked_user` (`user_id`, `reported_by`, `reason_for_rejection`, `block_date`, `expiry_date`, `day_count`, `created_at`, `updated_at`) 
                       VALUES ('$driverId', '$memid', '$reason', '$block_date', '$expiry_date', '$day_count', '$current_datetime', '$current_datetime')";
            $success = mysqli_query($con, $logSql);
        } else {
            $unblockSql = "UPDATE `blocked_user` SET `deletes` = '1', `updated_at` = '" . date('Y-m-d H:i:s') . "' WHERE `user_id` = '$driverId' AND `deletes` = '0'";
            $success = mysqli_query($con, $unblockSql);
        }
        if ($success) {
            echo json_encode(['status' => true, 'message' => 'Driver status updated successfully']);
        } else {
            echo json_encode(['status' => false, 'message' => 'Failed to update database']);
        }
    } else {
        echo json_encode(['status' => false, 'message' => 'Invalid Driver ID or Admin Session expired']);
    }
    exit;
} elseif ($method == 'admin_get_driver_block_history') {
    global $con;
    $result = [];
    $driverId = isset($_POST['driver_id']) ? (int)$_POST['driver_id'] : 0;
    $sql = "SELECT b.*, u.name as rejected_by_name 
            FROM `blocked_user` b
            LEFT JOIN `user_register` u ON b.reported_by = u.id
            WHERE b.user_id = '$driverId'
            ORDER BY b.id DESC";
    $historyQuery = mysqli_query($con, $sql);
    if ($historyQuery && mysqli_num_rows($historyQuery) > 0) {
        $result = mysqli_fetch_all($historyQuery, MYSQLI_ASSOC);
    }
    $currentDate = date('Y-m-d');
    foreach ($result as &$row) {
        if (empty($row['rejected_by_name'])) {
            $row['rejected_by_name'] = 'Administrator';
        }
        if (!empty($row['created_at']) && $row['created_at'] !== '0000-00-00 00:00:00') {
            $row['formatted_date'] = date('d M Y', strtotime($row['created_at']));
        } elseif (!empty($row['block_date'])) {
            $row['formatted_date'] = date('d M Y', strtotime($row['block_date']));
        } else {
            $row['formatted_date'] = '-';
        }
        if ($row['deletes'] == 1 && !empty($row['updated_at']) && $row['updated_at'] !== '0000-00-00 00:00:00') {
            $row['formatted_unblock_date'] = date('d M Y, h:i a', strtotime($row['updated_at']));
        } else {
            $row['formatted_unblock_date'] = '-';
        }
        $expiryFormatted = date('d M Y', strtotime($row['expiry_date']));
        $row['enhanced_reason'] = $row['reason_for_rejection'] . " <br><small class='text-muted'>Blocked until: $expiryFormatted ({$row['day_count']} Days)</small>";
        if ($row['deletes'] == 0 && strtotime($row['expiry_date']) >= strtotime($currentDate)) {
            $row['action_status'] = 1;
        } else {
            $row['action_status'] = 0;
        }
    ...
    echo json_encode($result);
    exit;
} elseif ($method == 'admin_update_driver_permissions') {
    global $con;
    $driverId = isset($_POST['driver_id']) ? (int)$_POST['driver_id'] : 0;
    $isBidding = isset($_POST['isBidding']) ? (int)$_POST['isBidding'] : 0;
    $isOpenjob = isset($_POST['isOpenjob']) ? (int)$_POST['isOpenjob'] : 0;
    $isPostJob = isset($_POST['isPostJob']) ? (int)$_POST['isPostJob'] : 0;
    $isPostSchJob = isset($_POST['isPostSchJob']) ? (int)$_POST['isPostSchJob'] : 0;
    if ($driverId > 0) {
        $sql = "UPDATE `user_register` 
                SET `isBidding` = '$isBidding', 
                    `isOpenjob` = '$isOpenjob', 
                    `isPostJob` = '$isPostJob', 
                    `isPostSchJob` = '$isPostSchJob' 
                WHERE `id` = '$driverId'";
        if (mysqli_query($con, $sql)) {
            echo json_encode(['status' => true, 'message' => 'Driver permissions updated successfully.']);
        } else {
            echo json_encode(['status' => false, 'message' => 'Failed to update database.']);
        }
    } else {
    ...
    $result = [];
    $now = date('Y-m-d');
    $agdate = isset($_POST["agdate"]) ? BlockSQLInjection($_POST["agdate"]) : '';
    $todate = isset($_POST["todate"]) ? BlockSQLInjection($_POST["todate"]) : '';
    $status = isset($_POST["status"]) ? BlockSQLInjection($_POST["status"]) : '';
    if ($agdate != '' && $todate != '') {
        $fd = date("Y-m-d", strtotime($agdate));
        $td = date("Y-m-d", strtotime($todate));
    ...
    $contype .= ' AND push_notifications.deletes = 0';
    $sql = "
    SELECT 
    ...
    if ($pushnotifi && mysqli_num_rows($pushnotifi) > 0) {
        while ($value = mysqli_fetch_assoc($pushnotifi)) {
            $result[] = [
                ...
                "route" => htmlspecialchars($value['route']),
                "status" => $value['status'],
    ...
    echo json_encode($result);
    exit;
} elseif ($method == 'scheduled_push_report') {
    $result = [];
    $agdate = isset($_POST["agdate"]) ? BlockSQLInjection($_POST["agdate"]) : '';
    $todate = isset($_POST["todate"]) ? BlockSQLInjection($_POST["todate"]) : '';
    $status = isset($_POST["status"]) ? BlockSQLInjection($_POST["status"]) : '';
    ...
    if ($agdate != '' && $todate != '') {
        $fd = date("Y-m-d", strtotime($agdate));
        $td = date("Y-m-d", strtotime($todate));
        $contype .= " AND s.created_at BETWEEN '$fd 00:00:00' AND '$td 23:59:59'";
    }
    if ($status !== '') {
        $contype .= " AND s.status = $status";
    }
    $sql = "SELECT s.id, s.user_type, s.sent_by, s.sent_state,s.title, s.body, s.route, s.image_url, s.scheduled_at, s.status, s.created_at, s.res_json, u.name as scheduled_by_name 
            FROM scheduled_push_notifications s 
            LEFT JOIN user_register u ON s.sent_by = u.id 
            WHERE $contype ORDER BY s.id DESC";
    $query = mysqli_query($rcon, $sql);
    if ($query && mysqli_num_rows($query) > 0) {
        while ($row = mysqli_fetch_assoc($query)) {
            $success_count = 0;
            $failure_count = 0;
            if (!empty($row['res_json'])) {
                $res_data = json_decode($row['res_json'], true);
                if (isset($res_data['success_count'])) {
                    $success_count = $res_data['success_count'];
                }
                if (isset($res_data['failure_count'])) {
                    $failure_count = $res_data['failure_count'];
                }
            }
            $result[] = [
                "id" => $row['id'],
                "user_type" => $row['user_type'],
                "sent_state" => $row['sent_state'],
                "sent_by" => $row['sent_by'],
                "scheduled_by_name" => $row['scheduled_by_name'] ? $row['scheduled_by_name'] : 'Unknown',
                "title" => $row['title'],
    ...
    echo json_encode($result);
    exit;
} elseif ($method == 'reschedule_push') {
    $id = (int)$_POST['id'];
    $scheduled_at = $_POST['scheduled_at'];
    $selected_time = strtotime($scheduled_at);
    $current_time = time();
    if ($selected_time <= $current_time) {
        echo json_encode(['status' => false, 'message' => 'Time must be in the future.']);
        exit;
    }
    $mysql_time = date('Y-m-d H:i:s', $selected_time);
    $sql = "UPDATE scheduled_push_notifications SET scheduled_at = ? WHERE id = ? AND status = 0";
    $stmt = $rcon->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("si", $mysql_time, $id);
        if ($stmt->execute()) {
            echo json_encode(['status' => true, 'message' => 'Notification rescheduled to ' . date('M d, Y h:i A', $selected_time)]);
        } else {
    ...
    exit;
} elseif ($method == 'delete_scheduled_push') {
    $id = (int)$_POST['id'];
    $sql = "UPDATE scheduled_push_notifications SET deletes = 1 WHERE id = ?";
    $stmt = $rcon->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("i", $id);
        if ($stmt->execute()) {
            echo json_encode(['status' => true, 'message' => 'Scheduled notification deleted.']);
        } else {
    ...
    exit;
} elseif ($method == 'get_state_mobiles') {
    $state = $_POST['state'] ?? '';
    $user_type = $_POST['user_type'] ?? 'driver'; // Default to driver if not passed
    $result = [];
    if (!empty($state)) {
        // Dynamically choose the table and column based on target type
        if ($user_type === 'customer') {
            $query = "SELECT mobile FROM customer_register WHERE current_state = ? AND mobile IS NOT NULL AND mobile != ''";
        } else {
            $query = "SELECT mobile FROM user_register WHERE state = ? AND mobile IS NOT NULL AND mobile != ''";
        }
        ...
        if ($stmt) {
            ...
            while ($row = $res->fetch_assoc()) {
                ...
                // Ensure Indian formatting as required by UI
                if (strlen($mobile) == 10) {
                    $mobile = '91' . $mobile;
                }
                $result[] = $mobile;
    ...
    $specific_mobiles_type = $_POST['specific_mobiles_type'] ?? '';
    $sent_state = $_POST['sent_state'] ?? NULL; // Capture the sent_state variable
    $selected_time = strtotime($scheduled_at);
    $current_time = time();
    if ($selected_time <= $current_time) {
        echo json_encode(['status' => false, 'message' => 'Scheduled time must be in the future.']);
        exit;
    }
    $imageUrl = null;
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        $upload_dir = '/home2/airportrides/console.goride.run/uploads/push_notification_images';
        if (!file_exists($upload_dir)) {
            mkdir($upload_dir, 0755, true);
        }
        $imageName = time() . '_scheduled_' . uniqid() . '.jpg';
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $targetPath)) {
            ...
            if ($info) {
                if ($info['mime'] == 'image/jpeg') {
                    $imageResource = imagecreatefromjpeg($targetPath);
                } elseif ($info['mime'] == 'image/png') {
                    $imageResource = imagecreatefrompng($targetPath);
                    // Convert PNG transparency to white background for JPG compatibility
                ...
                if ($imageResource) {
                    // Compress and overwrite the file with 60% quality
                    imagejpeg($imageResource, $targetPath, 60);
                    imagedestroy($imageResource);
                }
            }
    ...
    if ($user_type === 'specific_mobiles' || $user_type === 'state') {
        $targetDesc = ($user_type === 'state' && $sent_state) ? 'Scheduled for specific State: ' . $sent_state : 'Scheduled for specific ' . $specific_mobiles_type . 's';
        $req_json_array = [
            'target' => 'specific_mobiles',
            'mobile_numbers' => array_map('trim', explode(',', $mobile_numbers)),
            'specific_type' => $specific_mobiles_type,
            'sent_state' => $sent_state
        ];
        $req_json = json_encode($req_json_array);
        // Force mapping 'state' cleanly to 'specific_mobiles' so the CRON Job handles it gracefully.
        if ($user_type === 'state') {
            $user_type = 'specific_mobiles';
        }
    } else {
        $targetDesc = 'Scheduled for all ' . ucfirst($user_type) . 's';
        $req_json = json_encode(['target' => $targetDesc]);
    }
    $res_json = json_encode(['status' => 'Pending Execution...']);
    $created_at = date('Y-m-d H:i:s');
    $mysql_scheduled_time = date('Y-m-d H:i:s', $selected_time);
    // Added sent_state securely into the INSERT logic
    $sql = "INSERT INTO scheduled_push_notifications 
            (user_type, sent_by, sent_state, title, body, image_url, route, scheduled_at, status, req_json, res_json, created_at, updated_at, deletes) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, 0)";
    $stmt = $con->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("sissssssssss", $user_type, $admin_id, $sent_state, $title, $body, $imageUrl, $route, $mysql_scheduled_time, $req_json, $res_json, $created_at, $created_at);
        if ($stmt->execute()) {
            echo json_encode([
                'status' => true,
                'message' => 'Notification successfully scheduled for ' . date('M d, Y h:i A', $selected_time)
            ]);
        } else {
            echo json_encode(['status' => false, 'message' => 'Database execution error: ' . $stmt->error]);
        }
        $stmt->close();
    } else {
        echo json_encode(['status' => false, 'message' => 'Database preparation error: ' . $con->error]);
    }
    exit;
} elseif ($method == 'get_states') {
    $user_type = $_POST['user_type'] ?? 'driver';
    $result = [];
    // Choose the correct table and column based on target user type
    if ($user_type === 'customer') {
        $query = "SELECT DISTINCT current_state AS state FROM customer_register WHERE current_state IS NOT NULL AND current_state != '' ORDER BY current_state ASC";
    } else {
        $query = "SELECT DISTINCT state FROM user_register WHERE state IS NOT NULL AND state != '' ORDER BY state ASC";
    }
    $res = mysqli_query($con, $query);
    if ($res) {
        while ($row = mysqli_fetch_assoc($res)) {
            $result[] = htmlspecialchars($row['state']);
        }
    }
    echo json_encode(['status' => true, 'data' => $result]);
    exit;
} elseif ($method == 'get_mobile_numbers') {
    $target = strtolower(isset($_POST['target']) ? $_POST['target'] : '');
    $rawIds = isset($_POST['ids']) ? $_POST['ids'] : [];
    $result = [];
    if (!empty($rawIds) && is_array($rawIds)) {
        $driverIdsToRaw = [];
        $customerIdsToRaw = [];
        // 1. Categorize each ID strictly by its prefix to prevent cross-contamination
        foreach ($rawIds as $rawId) {
            $uidStr = strtolower((string)$rawId);
            $cleanId = (int) preg_replace('/[^0-9]/', '', $uidStr);
            if ($cleanId > 0) {
                if (strpos($uidStr, 'driver') !== false || strpos($uidStr, 'rm') !== false) {
                    $driverIdsToRaw[$cleanId][] = $rawId;
                } elseif (strpos($uidStr, 'customer') !== false) {
                    $customerIdsToRaw[$cleanId][] = $rawId;
                } else {
                    // Legacy Support (No Prefix): Guess based on the target string
                    if (strpos($target, 'customer') !== false) {
                        $customerIdsToRaw[$cleanId][] = $rawId;
                    } elseif (strpos($target, 'driver') !== false || strpos($target, 'rm') !== false) {
                        $driverIdsToRaw[$cleanId][] = $rawId;
                    } else {
                        // "Both" or unknown fallback
                        $driverIdsToRaw[$cleanId][] = $rawId;
                        $customerIdsToRaw[$cleanId][] = $rawId;
                    }
                }
        ...
        // 2. Query Driver Table ONLY for Driver IDs
        if (!empty($driverIdsToRaw)) {
            $dList = implode(',', array_keys($driverIdsToRaw));
            $q = mysqli_query($rcon, "SELECT id, mobile FROM user_register WHERE id IN ($dList)");
            while ($r = mysqli_fetch_assoc($q)) {
                if (isset($driverIdsToRaw[$r['id']])) {
                    foreach ($driverIdsToRaw[$r['id']] as $origId) {
                        $result[$origId] = $r['mobile'];
                    }
                }
            }
        }
        // 3. Query Customer Table ONLY for Customer IDs
        if (!empty($customerIdsToRaw)) {
            $cList = implode(',', array_keys($customerIdsToRaw));
            $q = mysqli_query($rcon, "SELECT id, mobile FROM customer_register WHERE id IN ($cList)");
            while ($r = mysqli_fetch_assoc($q)) {
                if (isset($customerIdsToRaw[$r['id']])) {
                    foreach ($customerIdsToRaw[$r['id']] as $origId) {
                        // Assign only if not already assigned by a legacy driver overlap
                        if (!isset($result[$origId])) {
                            $result[$origId] = $r['mobile'];
                        }
                    }
                }
    ...
    try {
        ...
        if ($txt == 'DRIVING_LICENSE' || $txt == 'NAME_BOARD' || $txt == 'GST') {
            ...
            if ($txt == 'NAME_BOARD' || $txt == 'GST') {
                $stmt2 = $con->prepare("SELECT id, type, user_id FROM kyc_details WHERE id = ? ORDER BY id DESC LIMIT 1");
                $stmt2->bind_param("i", $id);
                $stmt2->execute();
                $data = $stmt2->get_result()->fetch_assoc();
                if ($data && $data['type'] == 'Driver') {
                    echo json_encode(["type" => '0', "result" => 'Driver Role Not Suitable']);
                    exit;
                }
            } elseif ($txt == 'DRIVING_LICENSE') {
                $stmt3 = $con->prepare("SELECT id, type, user_id FROM kyc_details WHERE id = ? ORDER BY id DESC LIMIT 1");
                $stmt3->bind_param("i", $id);
                $stmt3->execute();
                $data = $stmt3->get_result()->fetch_assoc();
                if ($data && $data['type'] == 'Owner') {
                    echo json_encode(["type" => '0', "result" => 'Owner Role Not Suitable']);
                    exit;
                }
            }
            // --- 2. Fetch Primary User & KYC Details (No Left Join) ---
            $get_sql = "
                SELECT 
                    cu.name, cu.mobile, cu.doc_verify, cu.company_name,
                    kd.*
                FROM kyc_details kd
                JOIN user_register cu ON kd.user_id = cu.id
                WHERE kd.id = " . intval($id) . " 
                  AND kd.user_id = " . intval($user_id) . " 
                  AND cu.deletes = '0'
                LIMIT 1
            ";
            $get_res = mysqli_query($con, $get_sql);
            $complain = mysqli_fetch_assoc($get_res);
            if ($complain) {
                // --- 3. Resolve OCR Request Fallback Logic ---
                $ocr_cond = ($txt == 'DRIVING_LICENSE')
                    ? "doc_type = 'DRIVING_LICENSE' AND (global_type IS NULL OR global_type = '')"
                    : "1=1"; // Matching your original unfiltered condition for GST/NAME_BOARD
                $ocr_sql = "SELECT * FROM ocr_request WHERE user_id = " . intval($user_id) . " AND $ocr_cond ORDER BY created_at DESC, id DESC LIMIT 1";
                // Query Main DB
                $ocr_res_main = mysqli_query($con, $ocr_sql);
                $ocr_main = $ocr_res_main ? mysqli_fetch_assoc($ocr_res_main) : null;
                // Query Log DB (Isolating connection so it doesn't slow down non-OCR requests)
                $con_log = new mysqli('localhost', 'airportrides_goriderun_log', 'Z3cR8NTpm_8*^Y3B', 'airportrides_goriderun_log');
                $ocr_log = null;
                if (!$con_log->connect_error) {
                    $ocr_res_log = mysqli_query($con_log, $ocr_sql);
                    $ocr_log = $ocr_res_log ? mysqli_fetch_assoc($ocr_res_log) : null;
                    $con_log->close(); // Close immediately to free resources
                }
                // --- 4. Compare timestamps and pick the most recent ---
                $best_ocr = null;
                if ($ocr_main && $ocr_log) {
                    $time_main = strtotime($ocr_main['created_at'] ?? '1970-01-01');
                    $time_log = strtotime($ocr_log['created_at'] ?? '1970-01-01');
                    $best_ocr = ($time_log > $time_main) ? $ocr_log : $ocr_main;
                } else {
                    $best_ocr = $ocr_main ?: $ocr_log;
                }
                // --- 5. Merge Data back into response array ---
                if ($best_ocr) {
                    $fields_to_merge = ['doc_type', 'doc_no', 'doc_expiry', 'exp', 'status', 'request_id', 'req_response', 'front', 'back'];
                    foreach ($fields_to_merge as $field) {
                        if (isset($best_ocr[$field])) {
                            $complain[$field] = $best_ocr[$field];
                        }
                    }
                }
                echo json_encode(["type" => '1', "result" => $complain]);
            } else {
                echo json_encode(["type" => '0', "result" => "KYC not found or already processed"]);
            }
            exit; // Stop execution here for these specific document types
        } elseif ($txt == 'VEHICLE') {
            $get_sql = "
                SELECT *
                FROM user_register cu
        ...
        // General execution for VEHICLE, USERVERIFY, AADHAR
        $get_res = mysqli_query($con, $get_sql);
        $complain = mysqli_fetch_assoc($get_res);
        if ($complain) {
            $result["type"] = '1';
            ...
            if ($k_type == 'Driver') {
                ...
                if ($selfie_status == 'approved' && $aadhar_status == 'approved' && $dl_status == 'approved') {
                    ...
                    if ($send_whatsapp == 'yes') {
                        // Using Meta WhatsApp Template
                        sendMetaWhatsAppTemplate($con, $mobile, 'kyc_driver_approved_', [$name]);
                        // Push Notifications
                        $apiUrl = API_DOMAIN_2 . ($template_type == 'whatsapp' ? 'send-push-notify' : 'send-template');
                        ...
                        requestAPI('POST', $apiUrl, ['Content-Type: application/json'], json_encode($payload));
                    }
                    mysqli_query($rcon, $update_user);
                } else {
                    $doc_status = "";
                    $doc_status .= ($selfie_status != 'approved') ? "❌ *Selfie*: Pending\n" : "✅ Selfie verified\n";
                    $doc_status .= ($aadhar_status != 'approved') ? "❌ *Aadhar*: Pending\n" : "✅ Aadhar verified\n";
                    $doc_status .= ($dl_status != 'approved') ? "❌ *DL*: Pending\n" : "✅ DL verified\n";
                    if ($send_whatsapp == 'yes') {
                        // Using Meta WhatsApp Template
                        sendMetaWhatsAppTemplate($con, $mobile, 'kyc_driver_pending', [$name, trim($doc_status)]);
                        // Push Notifications
                        ...
                        $payload = [
                            ...
                            'title' => 'KYC Pending',
                            'action' => 'kyc_pending',
                            'message' => "Hi $name, your GoRide profile is pending. Please complete the KYC process."
                        ];
                        requestAPI('POST', $apiUrl, ['Content-Type: application/json'], json_encode($payload));
                    }
                }
                ...
                if ($selfie_status == 'approved' && $aadhar_status == 'approved' && $gst_status == 'approved') {
                    $update_user = "
                        ...
                        WHERE id = '$user_id' AND `deletes` = '0'
                    ";
                    $gst_name = ($o_proof_type == 'gst') ? 'GST' : 'Name Board';
                    mysqli_query($rcon, $update_user);
                    if ($send_whatsapp == 'yes') {
                        // Using Meta WhatsApp Template
                        sendMetaWhatsAppTemplate($con, $mobile, 'kyc_owner_approved', [$name, $gst_name]);
                        // Push Notifications
                        $apiUrl = API_DOMAIN_2 . ($template_type == 'whatsapp' ? 'send-push-notify' : 'send-template');
                        $message_content = "Hi $name, your GoRide profile is approved. Add vehicle details to continue.";
                        $payload = [
                    ...
                    $doc_status .= ($selfie_status != 'approved') ? "❌ *Selfie*: Pending\n" : "✅ Selfie verified\n";
                    $doc_status .= ($aadhar_status != 'approved') ? "❌ *Aadhar*: Pending\n" : "✅ Aadhar verified\n";
                    $doc_status .= ($gst_status != 'approved') ? "❌ *$gst_name*: Pending\n" : "✅ $gst_name verified\n";
                    if ($send_whatsapp == 'yes') {
                        // Using Meta WhatsApp Template
                        sendMetaWhatsAppTemplate($con, $mobile, 'kyc_owner_pending', [$name, trim($doc_status)]);
                        // Push Notifications
                        $apiUrl = API_DOMAIN_2 . 'send-push-notify';
                        $payload = [
                            'method' => $template_type,
    ...
    try {
        ...
        if ($fetch_kyc['nr'] > 0) {
            ...
            if ($k_type == 'Driver') {
                ...
                if ($send_whatsapp == 'yes') {
                    // Using Meta WhatsApp Template
                    sendMetaWhatsAppTemplate($con, $mobile, 'kyc_driver_rejected', [$name, ucfirst($remark)]);
                    // Note: If you want to send media ($s_image, $a_image, $dl_image), you will need to add a media-specific template API logic, but text will send.
                }
                mysqli_query($rcon, $update_user);
                $result["type"] = '1';
                $result["result"] = "Driver Kyc Verification updated";
                ...
                $update_user = "
                    ...
                    ";
                $doc_status = "";
                if ($selfie_status == 'rejected') $doc_status .= "❌ *Selfie*: Rejected\n";
                if ($aadhar_status == 'rejected') $doc_status .= "❌ *Aadhar*: Rejected\n";
                if ($gst_status == 'rejected') $doc_status .= "❌ *$gst_name*: Rejected\n";
                if ($send_whatsapp == 'yes') {
                    // Using Meta WhatsApp Template
                    sendMetaWhatsAppTemplate($con, $mobile, 'kyc_owner_rejected', [$name, trim($doc_status), ucfirst($remark)]);
                }
                mysqli_query($rcon, $update_user);
                $result["type"] = '1';
                $result["result"] = "Driver Kyc Verification updated";
    ...
    try {
        ...
        if ($fetch_user['nr'] > 0) {
            ...
            if ($tableData) {
                ...
                if ($send_whatsapp == 'yes') {
                    // Using Meta WhatsApp Template
                    sendMetaWhatsAppTemplate($con, $mobile, 'vehicle_approved', [$name]);
                }
                goto LKJHTYUI;
            }
        } else {
    ...
    try {
        ...
        if ($fetch_user['nr'] > 0) {
            ...
            if ($tableData) {
                ...
                if ($send_whatsapp == 'yes') {
                    // Using Meta WhatsApp Template
                    sendMetaWhatsAppTemplate($con, $mobile, 'vehicle_rejected', [$name, $re_mess]);
                }
                goto LKJHTYUII;
            }
        } else {
