...
if ($method == "list_agent") {
    ...
    $now = date('Y-m-d');
    $contype = ""; // default empty (important)
    if ($datefrom != '' && $datefill != '') {
        $contype = "`created_at` BETWEEN '$datefrom' AND '$datefill' AND";
    } else {
    }
    ...
    $type = $_POST['type'];
    if ($type == 'users') {
        $roll_id1 = "(`roll_id`= 5 OR `roll_id`= 6 OR `roll_id` = 7 OR `roll_id`= 0) AND ";
    } else if ($type == 'staffs') {
    ...
    try {
        ...
        $endDate = $_POST['datefill'] ?? '';
        $dateFilter = $_POST['dateFilter'] ?? '';
        $searchTxt = $_POST['fieldname'] ?? '';
        ...
        if ($u_type == 'all') {
            $filterParts[] = "cu.roll_id = '0' AND cu.deletes = '$deletedstatus' ORDER BY kd.updated_at DESC";
        } elseif ($u_type == 'k_n_c') {
            $filterParts[] = "
            (
                cu.doc_verify = '0'
            ...
            ) AND cu.deletes = '$deletedstatus' ORDER BY kd.updated_at DESC
            ";
        } elseif ($u_type == 'k_n_v') {
            $filterParts[] = "cu.doc_verify = '0' AND kd.updated_by IS NULL AND kd.reject_reason IS NULL AND kd.id IS NOT NULL AND cu.deletes = '$deletedstatus' ORDER BY kd.updated_at DESC";
        } elseif ($u_type == 'k_r') {
            $filterParts[] = "cu.doc_verify = '0' AND kd.updated_by IS NOT NULL AND kd.reject_reason IS NOT NULL AND kd.id IS NOT NULL AND cu.deletes = '$deletedstatus' ORDER BY kd.updated_at DESC";
        } elseif ($u_type == 'k_v') {
            $filterParts[] = "
                (
                    (
            ...
            ) AND cu.deletes = '$deletedstatus' ORDER BY cu.v_updated_at DESC
            ";
        } elseif ($u_type == 'v_n_v') {
            $filterParts[] = "
            (
...
elseif ($method == 'WhatsApp_report1') {
    ...
    // var_dump($gatcon);die;
    $SMSquery = mysqli_query($con, "SELECT * FROM `smslog` WHERE $gatcon $contype `id` != '' AND  `gateway` IN ('shiwhatsapp','smsportal')");
    if (mysqli_num_rows($SMSquery) > 0) {
        $result = mysqli_fetch_all($SMSquery, MYSQLI_ASSOC);
    }
    echo json_encode($result);
