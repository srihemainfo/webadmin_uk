...
if ($method == 'fetch_selfie_data') {
    ...
    $query = "SELECT c.id as user_id, c.name, c.mobile, c.updated_at, k.selfie_status, k.selfie_reason, k.s_lat, k.s_lang, k.updated_at as kyc_updated_at 
    ...
    $query = "SELECT c.id as user_id, c.name, c.mobile, c.updated_at, c.vehicle_details, k.s_lat, k.s_lang, k.dl_status as kyc_dl_status, o_dl.doc_no as dl_no, o_dl.status as dl_status, o_dl.req_response as dl_response, o_dl.updated_at as dl_updated_at, o_dl.front as dl_front, o_dl.back as dl_back 
              FROM customer_register c 
              LEFT JOIN kyc_carpool k ON c.id = k.user_id 
    ...
    $query = "SELECT c.id as user_id, c.name, c.mobile, c.updated_at, c.vehicle_details, 
                     k.s_lat, k.s_lang, k.dl_status as kyc_dl_status, 
    ...
    if ($result) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
            ...
            if (!isset($seen[$row['user_id']])) {
                ...
                // IMPORTANT FIX:
                // OCR should NOT overwrite already approved status
                // -------------------------------------------------
