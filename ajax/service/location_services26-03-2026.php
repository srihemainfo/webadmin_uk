...
// Global Helper to convert spaces to hyphens for Database Storage
function dbFormatName($str) {
    $str = trim($str ?? '');
    // Replaces spaces (even multiple spaces) with a single hyphen
...
if($method == "fetch_states") {
    ...
    // If it's a completely new district typed in, swap spaces to hyphens
    if ($is_new) {
    ...
    if($info = mysqli_fetch_assoc($info_q)) {
        ...
        if(mysqli_query($con, $up_q)) {
            ...
            if (strtolower($old_name) != strtolower(trim($name))) {
                ...
                // Display titles should have Spaces, Slugs have hyphens
                $display_new_title = str_replace('-', ' ', $name);
                
                $q1 = mysqli_query($con, "SELECT id, to_place FROM dynamic_pages_local WHERE LOWER(TRIM(name)) = LOWER('$old_name_esc')");
                while($r = mysqli_fetch_assoc($q1)) {
                    $r_id = $r['id'];
                    $to_place_clean = trim($r['to_place']);
                    $to_place_display = str_replace('-', ' ', $to_place_clean);
                    
                    $n_slug_raw = strtolower($name . '-to-' . $to_place_clean . '-drop-taxi');
                ...
                $q2 = mysqli_query($con, "SELECT id, name as from_loc FROM dynamic_pages_local WHERE LOWER(TRIM(to_place)) = LOWER('$old_name_esc')");
                while($r = mysqli_fetch_assoc($q2)) {
                    $r_id = $r['id'];
                    $from_loc_clean = trim($r['from_loc']);
                    $from_loc_display = str_replace('-', ' ', $from_loc_clean);
                    
                    $n_slug_raw = strtolower($from_loc_clean . '-to-' . $name . '-drop-taxi');
                    $n_slug = trim(preg_replace('/[^a-z0-9]+/', '-', $n_slug_raw), '-');
    ...
    foreach ($names_to_delete as $d_name) {
        mysqli_query($con, "UPDATE dynamic_pages_local SET deletes = '1' WHERE LOWER(TRIM(name)) = LOWER('$d_name') OR LOWER(TRIM(to_place)) = LOWER('$d_name')");
    }
    ...
    $combos = [];
    $res = mysqli_query($con, "SELECT name as from_loc, to_place as to_loc FROM dynamic_pages_local WHERE LOWER(TRIM(name)) = LOWER('$loc_name') AND deletes = '0' ORDER BY id DESC");
...
// 7. ULTRA FAST Combination Generator
    ...
    while($dest = mysqli_fetch_assoc($others_q)) {
        ...
        $chk = mysqli_query($con, "SELECT id FROM dynamic_pages_local WHERE LOWER(TRIM(name)) = LOWER('$source_esc') AND LOWER(TRIM(to_place)) = LOWER('$dest_esc')");
        
        if(mysqli_num_rows($chk) == 0) {
            
            // STRICT CLEAN SLUGS - No %20 Space Errors
    ...
    $query = "
        SELECT 
            ...
            (SELECT COUNT(id) FROM dynamic_pages_local dpl WHERE LOWER(TRIM(dpl.name)) = LOWER(TRIM(t1.district_name)) AND dpl.deletes = '0') as combo_count,
