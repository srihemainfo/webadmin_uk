...
if ($method == "list_agent") {
    ...
    $query = "SELECT kt.id,u.name, u.email, u.mobile,  kt.net_total, kt.createdon, kt.kiosk_id, kt.draw_id, tl.my3number, tl.raffle_id, kt.ticket_no , kt.transaction_id, d.name AS 'drawname' FROM `kticket` AS kt
                ...
                WHERE kt.`deletes` = '0' AND ";

    $roll_id = select_top_name($con, "user_register", "roll_id", "`id`='$currentUser' and `deletes`='0'", "roll_id", "");
    if (in_array($roll_id, [9])) {
        // if ($searchTxt != '') {
        $query .= "kt.`kiosk_id` in (SELECT `kiosk_id` FROM `kiosk_machines` WHERE `ownedby` = '$currentUser') AND";
        // } else {
    ...
    $drawID = $_POST['draw_new_id'];
    if ($drawID != '' && $drawID != null) {
        $query .= " kt.`draw_id` = '$drawID' AND";
    }
    
    
     $searchTxt = $_POST['searchTxt'];    
     if ($searchTxt != '') {
            $query .=   "`ticket_id` IN (SELECT id FROM `kticket` WHERE  `kiosk_id` LIKE '%$searchTxt%' AND `deletes` = '0') AND";
        }
...
error_log_new($con, getUserIP(), 'Kiosk_Status_update', $currentUser, '', '', 'The Kiosk Machine Status Has been updated', json_encode($_REQUEST), __DIR__, basename(__FILE__), __LINE__, $dubaidate_time);
    ...
    if ($errors != "") {
        ...
        $result["result"] = "Update Failed. Kindly Contact to Admin Team!";
          ...
          goto gviRetrun12d;
              ...
              $result['type'] = 1;
        $result['result'] = 'The Kiosk machine status successfully!';
       goto gviRetrun12d;
}
    ...
    gviRetrun123:
    echo json_encode($result);
} else if ($method == "new_sign_up_list") {
    $result = [];
    $currentUser = $_SESSION['memid'];
    ...
    $query = "SELECT name, email, mobile, created_at FROM `user_register` WHERE `kiosk_id` IN (SELECT `kiosk_id` FROM `kiosk_machines` WHERE `ownedby` = $currentUser AND `deletes` = '0') AND `deletes` = '0' AND `status` = '0' AND ";

    if ($_POST['formdate'] != '' && $_POST['todate'] != '') {



        $fd = date("Y-m-d", strtotime($_POST['formdate']));

        $td = date("Y-m-d", strtotime($_POST['todate']));

        $query .= "`created_at` BETWEEN '$fd 00:00:00' AND '$td 23:59:59' AND ";
    }
