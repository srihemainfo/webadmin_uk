...
// NEW FUNCTION: Added stateFilter argument
function buildKycWhereClause($con, $searchTxt, $stateFilter, $dateRange) {
    $where = "c.deletes = '0'"; 
    if (!empty($searchTxt)) {
    ...
    if (!empty($stateFilter)) {
        $stateFilter = mysqli_real_escape_string($con, $stateFilter);
        $where .= " AND c.current_state = '$stateFilter'";
    }
    if (!empty($dateRange)) {
        ...
        if (count($dates) == 2) {
            ...
            $where .= " AND (k.updated_at BETWEEN '$start' AND '$end' OR c.updated_at BETWEEN '$start' AND '$end')";
        }
    }
    return $where;
}
...
// ------------------------------------------------------------------
// 0. FETCH UNIQUE STATES
// ------------------------------------------------------------------
if ($method == 'fetch_states') {
    $q = mysqli_query($con, "SELECT DISTINCT current_state FROM customer_register WHERE current_state IS NOT NULL AND current_state != '' ORDER BY current_state ASC");
    $states = [];
    while($r = mysqli_fetch_assoc($q)) {
        $states[] = trim($r['current_state']);
    }
    sendJsonResponse(['type' => 1, 'result' => $states]);
}
...
// ------------------------------------------------------------------
...
else if ($method == 'fetch_selfie_data') {
    $searchTxt = $_POST['searchTxt'] ?? '';
    $stateFilter = $_POST['stateFilter'] ?? '';
    $dateRange = $_POST['dateRange'] ?? '';
    $where = buildKycWhereClause($con, $searchTxt, $stateFilter, $dateRange);
    $where .= " AND (k.user_id IS NOT NULL OR c.doc_verify > 0)";

    $query = "SELECT c.id as user_id, c.name, c.mobile, c.updated_at, c.current_state, c.current_district, k.selfie_status, k.selfie_reason, k.updated_at as kyc_updated_at 
              FROM customer_register c 
              LEFT JOIN kyc_carpool k ON c.id = k.user_id 
              WHERE $where ORDER BY c.updated_at DESC LIMIT 1000";
    ...
    $query = "SELECT c.id as user_id, c.name, c.mobile, c.updated_at, c.vehicle_details, c.current_state, c.current_district, k.dl_status as kyc_dl_status, o_dl.doc_no as dl_no, o_dl.status as dl_status, o_dl.req_response as dl_response, o_dl.updated_at as dl_updated_at, o_dl.front as dl_front, o_dl.back as dl_back 
              FROM customer_register c 
              LEFT JOIN kyc_carpool k ON c.id = k.user_id 
...
// ------------------------------------------------------------------
// NEW: UPDATE DL NUMBER & JSON DATA HANDLING
// ------------------------------------------------------------------
else if ($method == 'update_dl_number') {
    $user_id = $_POST['user_id'] ?? '';
    $dl_no = $_POST['dl_no'] ?? '';

    if (!empty($user_id) && !empty($dl_no)) {
        $dl_no = mysqli_real_escape_string($con, $dl_no);
        $user_id = mysqli_real_escape_string($con, $user_id);

        // Define the default JSON exactly as requested
        $defaultJson = [
            "" => "This Website belongs to Ministry of Road Transport & Highways (MoRTH) Government of India",
            "NT" => "LMV",
            "TR" => "LMV-TR",
            "id_no" => $dl_no,
            "Transport" => "From: 24-Feb-2026",
            "issued_date" => "14-Sep-2023",
            "Holder's Name" => "S*A*D*A*A K*I*H*A*",
            "Current Status" => "ACTIVE",
            "Source Of Data" => "SARATHI",
            "date_of_expiry" => "26-Apr-2045",
            "Old / New DL No." => "NA",
            "Initial Issue Date" => "14-Sep-2023",
            "Last Endorsed Date" => "24-Feb-2026",
            "Hazardous Valid Till" => "NA",
            "Last Endorsed Office" => "RTO ,TAMBARAM",
            "Initial Issuing Office" => "RTO ,TAMBARAM",
            "Last Completed Transaction" => "ISSUE OF PSV BADGE TO A DRIVER , COV CONVERSION , ISSUE OF DRIVING LICENCE"
        ];

        // Fetch the existing latest OCR request
        $fetch_ocr = "SELECT id, req_response FROM ocr_request WHERE user_id = '$user_id' AND global_type = 'customer' AND doc_type IN ('DRIVING_LICENSE', 'DL') ORDER BY id DESC LIMIT 1";
        $ocr_res = mysqli_query($con, $fetch_ocr);

        if ($ocr_res && mysqli_num_rows($ocr_res) > 0) {
            $row = mysqli_fetch_assoc($ocr_res);
            $ocr_id = $row['id'];
            $existingJsonStr = $row['req_response'];
            
            $existingJson = json_decode($existingJsonStr, true);

            // If JSON already exists and is valid, just update the DL No and Status
            if (is_array($existingJson) && !empty($existingJson)) {
                $existingJson['id_no'] = $dl_no;
                $existingJson['Current Status'] = 'ACTIVE';
                $newJsonStr = mysqli_real_escape_string($con, json_encode($existingJson));
            } else {
                // If it's empty, use the full default structure
                $newJsonStr = mysqli_real_escape_string($con, json_encode($defaultJson));
            }

            // Update ocr_request with new doc_no & JSON
            mysqli_query($con, "UPDATE ocr_request SET doc_no = '$dl_no', req_response = '$newJsonStr' WHERE id = '$ocr_id'");
        } else {
            // If completely missing, insert a new record to tie the JSON to it
            $newJsonStr = mysqli_real_escape_string($con, json_encode($defaultJson));
            mysqli_query($con, "INSERT INTO ocr_request (user_id, global_type, doc_type, doc_no, req_response) VALUES ('$user_id', 'customer', 'DRIVING_LICENSE', '$dl_no', '$newJsonStr')");
        }

        // Simultaneously update kyc_carpool
        mysqli_query($con, "UPDATE kyc_carpool SET dl_no = '$dl_no' WHERE user_id = '$user_id'");

        sendJsonResponse(['result' => 'DL details and status updated successfully', 'type' => 1]);
    } else {
        sendJsonResponse(['result' => 'Missing ID or DL number', 'type' => 0]);
    }
}
    ...
    // Subquery updated to include o1.req_response so the frontend can parse the DL JSON (NT, TR, etc.)
    $query = "SELECT c.id as user_id, c.name, c.mobile, c.updated_at, c.vehicle_details, c.current_state, c.current_district, 
                     k.dl_status as kyc_dl_status, 
                     o_dl.status as ocr_status, o_dl.req_response as dl_response, o_dl.front as dl_front, o_dl.back as dl_back 
              FROM customer_register c 
              LEFT JOIN kyc_carpool k ON c.id = k.user_id 
              LEFT JOIN (
                  SELECT o1.user_id, o1.status, o1.req_response, o1.front, o1.back 
                  FROM ocr_request o1 
                  INNER JOIN (
                      SELECT user_id, MAX(id) as latest_id 
                      FROM ocr_request 
                      WHERE global_type = 'customer' 
                      AND doc_type IN ('DRIVING_LICENSE', 'DL') 
                      GROUP BY user_id
    ...
    if ($result) {
        while($row = mysqli_fetch_assoc($result)) {
            if (!isset($seen[$row['user_id']])) {
                ...
                // FINAL BLOCKER: If it's not explicitly 'approved', skip this user
                if ($current_dl_status !== 'approved') {
                    $seen[$row['user_id']] = true;
                    continue;
                }

                // Extract Vehicle Details (only runs if DL is approved)
                if(!empty($row['vehicle_details']) && $row['vehicle_details'] != 'null') {
                    $vd = json_decode($row['vehicle_details'], true);
                    if(is_array($vd)) {
                        ...
                        if(isset($vd['vehicle']) && isset($vd['vehicle']['front_view_image_url'])) {
                            $row['front_view_image_url'] = $vd['vehicle']['front_view_image_url'];
                        }
                    }
                }
...
// ------------------------------------------------------------------
    ...
    try {
        ...
        $laravel_api_url = "https://www.goride.run/api/trigger-selfie-push"; 
        ...
        $push_msg = ($push_status == 200) ? " & Push Triggered" : " & Push Failed";
        
        sendJsonResponse(['result' => "Selfie KYC updated successfully." . $whatsapp_status_msg . $push_msg, 'type' => 1]);
    } catch (Exception $e) {
        mysqli_rollback($con);
    ...
    try {
        ...
        $laravel_api_url = "https://www.goride.run/api/trigger-dl-push"; 
        ...
        sendJsonResponse(['result' => "DL Verification updated successfully." . $whatsapp_status_msg . $push_msg, 'type' => 1]);
    } catch (Exception $e) {
        mysqli_rollback($con);
        sendJsonResponse(['result' => 'Update failed.', 'type' => 0]);
    }
    ...
    try {
        ...
        $laravel_api_url = "https://www.goride.run/api/trigger-vehicle-push"; 
        ...
        sendJsonResponse(['result' => "Vehicle Verification updated successfully." . $whatsapp_status_msg . $push_msg, 'type' => 1]);
    } catch (Exception $e) {
        mysqli_rollback($con);
        sendJsonResponse(['result' => 'Update failed.', 'type' => 0]);
    }
