...
if ($method == "list_agent") {
    ...
    if ($user_register['nr'] > 0) {
        ...
        foreach ($user_register['result'] as $key => $value) {
            ...
            $result[] = ["buildinglocation" => utf8_encode($value['address']), "Country" => utf8_encode($value['nationality']), "City" => utf8_encode($value['city']), "residinglocation" => $value['residinglocation'], "createdat" => $value['created_at'], "action" => $action, "id" => $value['id'], "nid" => str_pad($value['id'], 7, "0", STR_PAD_LEFT), "rollType" => $value['user'], "name" => $value['name'] . ' ' . $value['lname'], "t_point" => $value['t_point'], "mobile" => $value['mobile'], "passport" => utf8_encode($value['passport']), "email" => $value['email'], "dob" => $value['dob']];
        }
    }
...
//   var_dump('hiiiiii');die;
        ...
        $NDticketquery = mysqli_query($con, "SELECT I.netTotal, I.createdon,I.ticketReferenceID,I.payment_transaction_id,T.referenceID, I.renewalStatus, I.startDate, I.endDate, T.ticketNo, T.raffleIds, CASE WHEN I.netTotal <= 7 THEN 'New Purchased' WHEN I.netTotal >= 10 AND I.netTotal <= 300 THEN 'Monthly' ELSE 'Yearly' END AS netTotalStatus FROM ndticket AS T JOIN `invoice` AS I ON I.ticketId = T.id WHERE $contype T.id = $ticketID");
