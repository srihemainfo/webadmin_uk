...
if ($method == "generatedcopoun") {
    ...
    if ($Coupon_name != '') {
        ...
        // $contype1 = '';
            $Coupon_name1 = "AND cc.c_name = '$Coupon_name' ";
    }
...
$selectQuery = "SELECT cp.*, ur.name, ur.lname, ur.mobile, ur.email, cc.c_code, cc.c_name FROM `couponcode_history` cp JOIN `user_register` ur ON cp.userid = ur.id JOIN `couponcode` cc ON cc.id = cp.couponcodeID WHERE cp.deletes='0' AND cp.type !='discount' $contype1 $draw_Con $Coupon_name1;";
...
//  var_dump("SELECT c_name, MIN(c_started_at) AS c_started_at, MIN(c_ended_at) AS c_ended_at, productid, MIN(createdon) AS first_createdon, COUNT(*) AS count, SUM(c_use_count) AS user FROM couponcode $draw_Con  GROUP BY c_name, productid $draw1 $draw2 $draw3 $creatdate12 $contype22;");die;        
...
$selectQuery = "SELECT c_name, MIN(c_started_at) AS c_started_at, MIN(c_ended_at) AS c_ended_at, productid, MIN(createdon) AS first_createdon, COUNT(*) AS count, SUM(c_use_count) AS user FROM couponcode $draw_Con  GROUP BY c_name, productid $draw1 $draw2 $draw3 $creatdate12 $contype22;";
