...
if ($method == "list_agent_customer") {
    ...
    // Safely pull global dependencies
    ...
    // Full Name Search including First Name + Last Name
    $fieldname = isset($_POST["fieldname"]) ? mysqli_real_escape_string($con, trim($_POST["fieldname"])) : '';
    if ($fieldname != '') {
        $query_parts[] = "(`email` LIKE '%$fieldname%' OR `name` LIKE '%$fieldname%' OR `lname` LIKE '%$fieldname%' OR CONCAT(`name`, ' ', `lname`) LIKE '%$fieldname%' OR `mobile` LIKE '%$fieldname%')";
    }

    // Default to '0' (not deleted)
    $deletedstatus = isset($_POST['deletedstatus']) ? mysqli_real_escape_string($con, trim($_POST['deletedstatus'])) : '';
    if ($deletedstatus !== '') {
        $query_parts[] = "`deletes`='$deletedstatus'";
    } else {
    ...
    // Default to today if no search filters are applied
    if ($datefrom == '' && $datefill == '' && $fieldname == '' && $deletedstatus == '') {
        $query_parts[] = "`created_at` LIKE '%$now%'";
    }

    // Safely apply global role variables if they exist
    if (!empty($role)) {
        $clean_role = trim(preg_replace('/AND\s*$/i', '', trim($role)));
        if (!empty($clean_role)) $query_parts[] = $clean_role;
    }
    ...
    // --- OPTIMIZATION 1: Fetch ONLY required columns instead of SELECT * ---
    // Added current_state, current_district, and current_address to the query
    ...
    if (isset($customer_register['nr']) && $customer_register['nr'] > 0) {
        foreach ($customer_register['result'] as $value) {
            // --- OPTIMIZATION 2: Cache array keys to variables to avoid repeated memory lookups ---
            $id = $value['id'];
    ...
    // --- OPTIMIZATION 3: Free up RAM before JSON encoding ---
