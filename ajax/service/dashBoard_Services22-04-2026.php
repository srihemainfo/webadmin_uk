...
// elseif ($method == 'customer_bookings_chart') {
//     $startDate = $_POST['startDate'] ?? date('Y-m-d', strtotime('-7 days'));
//     $endDate   = $_POST['endDate'] ?? date('Y-m-d');
//     $start = $startDate . ' 00:00:00';
//     $end   = $endDate . ' 23:59:59';
//     $now   = date('Y-m-d H:i:s');
//     // Fetch all customer jobs matching the pickup date range
//     $sql = "SELECT DATE(pickup_date) as pdate, job_status, pickup_date 
//             FROM `cus_job_temp` 
//             WHERE deletes = '0' 
//             AND job_no LIKE 'GRC-%' 
//             AND pickup_date BETWEEN '$start' AND '$end'";
...
//     }
//     $assigned   = array_fill_keys($dates, 0);
//     $unassigned = array_fill_keys($dates, 0);
//     $completed  = array_fill_keys($dates, 0);
//     $expired    = array_fill_keys($dates, 0);
//     $cancelled  = array_fill_keys($dates, 0);
//     // Populate data based on the same logic used in job_details_summary
//     if ($res && mysqli_num_rows($res) > 0) {
//         while ($row = mysqli_fetch_assoc($res)) {
//             $d = $row['pdate'];
//             if (!isset($assigned[$d])) continue;
//             $status = strtolower($row['job_status']);
//             $pTime  = $row['pickup_date'];
//             $isPast = ($pTime < $now);
//             if ($status === 'cancelled') {
//                 $cancelled[$d]++;
//             } elseif ($status === 'completed') {
//                 $completed[$d]++;
//             } elseif (in_array($status, ['accept', 'accepted'])) {
//                 if ($isPast) {
//                     $completed[$d]++; // Count as completed if past pickup time
//                 } else {
//                     $assigned[$d]++;
//                 }
//             } else {
//                 // Not accepted, completed, or cancelled
//                 if ($isPast) {
//                     $expired[$d]++;
//                 } else {
//                     $unassigned[$d]++;
...
//         }
//     }
//     // Format dates for display (e.g., "15 Oct")
//     $formattedDates = array_map(function($d) { return date('d M', strtotime($d)); }, $dates);
//     echo json_encode([
//         'type'       => 1,
...
// }
    ...
    // Added: AND c.pickup_date >= NOW() to exclude expired jobs globally
    $sql = "SELECT c.*, d.name as driver_name, d.mobile as driver_mobile
            ...
            WHERE c.global_type = 'carpool' AND c.deletes = '0' AND c.pickup_date >= NOW()";
    // Apply Search
    if (!empty($search)) {
