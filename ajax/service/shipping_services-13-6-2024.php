...
$selectedProductId = $_POST['selectedProductId']; 
    ...
    //   var_dump("SELECT  G.id,G.grams,G.delivery_status, G.ticketReferenceID, G.goldValue, G.orderClosedBy, G.createdon, G.updatedon, ur.name, ur.lname, ur.mobile, ur.email
    //     FROM `gold_request` G
    //     JOIN `user_register` ur ON G.userID = ur.id
    //     WHERE $contype $draw_Con G.deletes = '0'

    //     ");die;
        
        $GoldRequest = mysqli_query($con, "SELECT  G.id,G.grams,G.delivery_status, G.ticketReferenceID, G.goldValue, G.orderClosedBy, G.createdon, G.updatedon, ur.name, ur.lname, ur.mobile, ur.email
        FROM `gold_request` G
        JOIN `user_register` ur ON G.userID = ur.id
        WHERE $contype $draw_Con G.deletes = '0';
        ");
...
// var_dump($rowId,$newStatus);die;

$updateQuery = "UPDATE gold_request SET delivery_status = '$newStatus', updatedon = '$dubaidate_time' WHERE id = '$rowId'";
