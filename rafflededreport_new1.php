...
// var_dump('kjdzfjdfdjhfdjfdhsf');die;
...
if ($_REQUEST['reportType'] == 'pdf') {
    ...
    try {
        ...
        $encodedDrawIdFromURL1 = $_GET['drawid'];
        $winnerid = decodeValue($encodedDrawIdFromURL1);
        $pid = $_GET['proid'];
        ...
        $encodedDrawIdFromURL = $_GET['drawid'];
        $draw_new_id = decodeValue($encodedDrawIdFromURL);
        ...
        $draw = select_query($con, "superraffledraw", "", "`draw_no`= '$id' AND `deletes` = '0' ", "", "");
        ...
        if ($winnerid != '') {
            ...
            $raffle_id = "`tl`.`raffle_id` = '$winnerid' AND";
        }
        ...
        // var_dump($ticket_name ,$ticket_name ,$agdate,$permitno,);die;
        ...
        $queryNew = "SELECT tl.type, tl.id, tl.user_id, tl.product_id, tl.raffle_id, tl.ticket_id, p.rate, p.prize_one, ur.name, ur.mobile, ur.email
                        ...
                        WHERE $raffle_id tl.deletes = '0' AND ur.deletes = '0' order by rand();";
        //exit;
            ...
            if ($numRows > 0) {
                ...
                if ($ld->checkRole($pid)) {
                    ...
                    if ($pid == 'G') {
                        ...
                        $namefff = 'Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . str_replace("Draw", "", $naeme[0]) . '' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD';
                    } else if ($pid == '1') {
                        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                        $namefff = 'Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . str_replace("Draw", "", $naeme[0]) . '' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD Category-10AED';
                    } else if ($pid == '2') {
                        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                        $namefff = 'Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . str_replace("Draw", "", $naeme[0]) . '' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD Category-20AED';
                    } else if ($pid == '3') {
                        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                        $namefff = 'Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . str_replace("Draw", "", $naeme[0]) . '' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD Category-50AED';
                    } else if ($pid == '4') {
                        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                        $namefff = 'Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . str_replace("Draw", "", $naeme[0]) . '' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD Category-100AED';
                    }
                } else {
                    $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                    $namefff =  'Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . str_replace("Draw", "", $naeme[0]) . '' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . $tna . '-DET';
  ...
  $draw_new_id = $_GET['drawid'];
    ...
    $whereWin = [];
    if ($draw['result'][0]['win_raffle_no'] != null && $draw['result'][0]['win_raffle_no'] != '') {
        $whereWin[] = "'" . strval($draw['result'][0]['win_raffle_no']) . "'";
    }
