...
if ($winnerid != '' && $pid != '') {
    ...
    $draw = select_query($con, "raffledraw", "", "`id`= '$winnerid' AND `deletes` = '0' ", "", "");

    // $first = $draw['result'][0]['win_raffle_no'];

    // $lines11 = select_query($con, "ticket_lines", "", "`raffle_id` = '$first' AND `deletes` = '0'", "", "");



    $draw_no = '#' . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT);   //==================================draw_no
                                    ...
                                    <table  width="470px"  id="date-table"  align="right"  style="   border-collapse: collapse !important; ">
                                            <tbody >
                                                <tr style="border-top:1px solid #000!important; border-left:1px solid #000; border-right:1px solid #000;">
                                                    ...
                                                    <tr  style="border-right:1px solid #000; border-left:1px solid #000;"><td class="fontCom" >Grand Raffle Draw No</td>
                    ...
                    <table  style="width: 100%;  border-collapse: collapse !important;" id="winnerTable">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th class="subTitle fontCom"><strong>PARTICIPATED CATEGORY</strong></th>
    ...
    $wonprize = $draw['result'][0]['wonprize'];

    // var_dump($first);die;

    $second = $draw['result'][0]['win_raffle_no'];
    $third = $draw['result'][0]['win_raffle_no'];
    ...
    if ($first != '' && $first != '-') {
        ...
        $lines1 = select_query($con, "ticket_lines", "", "`raffle_id` = '$first' AND `deletes` = '0'", "", "");
        ...
        if ($lines1['nr'] > 0) {
            ...
            for ($i = 0; $i < $lines1['nr']; $i++) {
                ...
                $GetWinneruser = select_query($con, "raffledraw", "", "`userid`= '$uid' AND `raffle_id` = '$first' AND `my3number` IS NULL ORDER BY `id` DESC LIMIT 1", "", "");
                if ($GetWinneruser['nr'] > 0) {
                    $userName = $GetWinneruser['result'][0]['name'];
                } else {
                    $userName = $user['result'][0]['name'] . ' ' . $user['result'][0]['lname'];
                }

                // var_dump($user['result'][0]['id']);die;
                ...
                $pdfContent .= '<tr>
                                            ...
                                            <td class="content fontCom">' . 'AED' . ' ' . number_format($product_rate, 2) . '</td>
                                            <td class="content fontCom">' . number_format($wonprize, 2) . '</td>
                                           ';
    ...
    if ($ld->checkRole($pid)) {
        if ($pid == 'G') {
            $namefff = 'Grand Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Just3 raffle' . ' Winners List-LD';
        } else if ($pid == '1') {
            $namefff = 'Grand Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' .  ' Just3 raffle' . ' Winners List-Category-10AED LD';
        } else if ($pid == '2') {
            $namefff = 'Grand Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Just3 raffle' . ' Winners List-Category-20AED LD';
        } else if ($pid == '3') {
            $namefff = 'Grand Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Just3 raffle' . ' Winners List-Category-50AED LD';
        } else if ($pid == '4') {
            $namefff = 'Grand Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Just3 raffle' . ' Winners List-Category-100AED LD';
        }
    } else {
        $namefff = 'Grand Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Just3 raffle' . ' Winners List ALL DET';
    }
