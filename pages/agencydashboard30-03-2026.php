...
===================================================== */
        ...
        function loadDriversOnMap() {
    // 1. Use window.isLoadingDrivers to prevent ReferenceErrors
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            $('#mapLoader').addClass('hide');
            window.isLoadingDrivers = false; // Reset to false on success
        },
        error: function() {
            setTimeout(function() { 
                window.isLoadingDrivers = false; // Reset to false on error before retrying
                loadDriversOnMap(); 
            }, 3000);
        }
    });
...
<script>
    ...
    $(document).ready(function() {
        ...
        // 1. Ensure dates are NULL by default so nothing loads until requested
        historyStartDateTime = null;
        historyEndDateTime = null;

        // 2. Force the input to be completely empty on page load
        $('#historyDateRange').val('');

        // 3. Inject a custom "Clear (X)" button into the input group dynamically
        ...
        // 4. Initialize DateRangePicker
        if($.fn.daterangepicker) {
            $('#historyDateRange').daterangepicker({
                timePicker: true,
                timePicker24Hour: false,
                timePickerIncrement: 15,
                autoUpdateInput: false,      // KEEPS THE INPUT BOX EMPTY ON LOAD
                alwaysShowCalendars: false,  // Hides calendar grids until "Custom Range" is clicked
                maxDate: moment(),           // Prevents selecting future dates
                locale: {
            ...
            // Handle the internal DateRangePicker Clear button (if they use it)
            $('#historyDateRange').on('cancel.daterangepicker', function(ev, picker) {
                resetHistoryMap();
            });
        }

        // 5. Handle our Custom "X" Clear Button Click
        $(document).on('click', '#clearHistoryBtn', function(e) {
            e.stopPropagation(); // Prevents the calendar from opening when clicking the X
            resetHistoryMap();
        });

        // 6. The Master Reset Function (Acts like a Refresh)
        function resetHistoryMap() {
            // Clear inputs and variables
            $('#historyDateRange').val('');
            $('#clearHistoryBtn').addClass('d-none');
            historyStartDateTime = null;
            historyEndDateTime = null;
            
            // Wipe Map and Deselect Drivers
            clearMapOverlays(true); 
            window.currentDriverId = null;
            $('#mapDriverSearch').val(''); 

            // Restore calendar icon if it was stuck spinning
            let $spinIcon = $('#historyControls .fa-spinner');
            if ($spinIcon.length) {
                $spinIcon.removeClass('fa-spinner fa-spin').addClass('fa-calendar-alt');
            }

            // Reload all live drivers like a fresh start
            loadDriversOnMap(); 
        }

        // 7. Show History Controls whenever a driver is selected
    ...
    // 2. Render all drivers safely
    // 2. Render ALL drivers safely with RICH POPUPS
    function plotAllDriversHistoryOnMap(locations) {
        ...
        locations.forEach(loc => {
            ...
            // 🔥 USE EXISTING RICH POPUP FUNCTION!
            // Passing the historical time (loc.formatted_time) so it shows correctly in the popup
    ...
    // 3. Render SINGLE driver historical map with RICH POPUPS
    function plotDriverHistoryOnMap(locations, data) {
        ...
        sampled.forEach((loc, index) => {
            ...
            // 🔥 USE EXISTING RICH POPUP FUNCTION HERE TOO!
    ...
    // 3. Render SINGLE driver historical map (This is the one that went missing!)
    function plotDriverHistoryOnMap(locations, data) {
        if (typeof driversMap === 'undefined' || !driversMap) return;
    ...
    let cancelDocViewer = null; // Store viewer instance globally
    ...
    $(document).on('click', '.view-cancel-reason-btn', function(e) {
        ...
        const NO_DOC_SVG = "data:image/svg+xml;charset=UTF-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='100%25' height='100%25'%3E%3Crect width='100%25' height='100%25' fill='%23f8f9fa'/%3E%3Ctext x='50%25' y='50%25' dominant-baseline='middle' text-anchor='middle' font-family='sans-serif' font-size='12px' fill='%236c757d'%3ENo Image Uploaded%3C/text%3E%3C/svg%3E";
        ...
        if (docUrl && docUrl.trim() !== '' && docUrl !== 'null' && docUrl !== 'undefined') {
            ...
            $img.on('error.cancel', function() {
                ...
                // If it fails to load, destroy viewer to prevent opening broken image
                if (cancelDocViewer) {
            ...
            // Initialize Viewer.js for this specific image
            if (cancelDocViewer) {
                cancelDocViewer.destroy();
            }
