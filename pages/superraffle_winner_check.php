...
$pageTitle = 'Super Raffle Preliminary Winner Check';
							...
							<table class="table table-bordered display" id="drawtable" style="width:100%;">
								...
								<thead>
									...
									<tr>
										...
										<th class="wd-25p border-bottom-0">PARTICIPATED CATEGORY</th>
						...
						<div class="row">
							...
							<div class="col-md-12 ">
								...
								<div class="justify-content-center">
									...
									<h3 class="">Enter Super RaffleID</h3>
...
<script>
	var url = window.location.origin + "/ajax/service/super_raffle_services.php";
	...
	function checkwinner() {
		...
		do_ajax_call(post_data, onsuccess, window.location.origin + '/ajax/service/super_raffle_services.php');
		...
		var onsuccess = function(data) {
			...
			if (response != "") {
				...
				if (response.type == 1) {
					...
					// window.location.herf = "https://production.nationaldraw.ae/winningnumber/list/90";
					document.getElementById('winnerBtn').innerHTML = btn;

					// viewtable(response.id);

					getTabel(response.rafflePrizeOne, response.id);
	...
	function getTabel(rafflePrizeOne, id) {
	...
	function getWinnerList(pid, drawid, rafflePrizeOne, title, liveurl) {
		// document.getElementById('ldbtnnew').innerHTML = '';
		// if (pid != 0) {
		// 	var buildUrl = liveurl + 'rbddedreport1.php?drawid=' + drawid + '&proid=' + pid;
		...
		table = $("#drawtable").DataTable({
			...
			ajax: {
				...
				dataSrc: "",
				data: {
					method: 'list_draw_winner_new1',
					pid: pid,
					drawid: drawid,
					raffle_id: rafflePrizeOne
				}
			},
			dom: 'Bfrtip',
