...
<div class="main-content app-content mt-0">
    ...
    <div class="col-lg-3 col-md-6">
        ...
        <input class="form-control" type="text" id="datefilter" name="datefilter" value="<?= date('m/d/Y') . ' - ' . date('m/d/Y'); ?>" placeholder="MM/DD/YYYY - MM/DD/YYYY" autocomplete="off" oninput="this.value = this.value.replace(/[^0-9- \/]/g, '');">
    </div>
    <div class="col-lg-3 col-md-6">
        <label class="form-label fw-semibold mb-1">Search</label>
        <input type="text" id="fieldname" oninput="viewtable($(this).val())" class="form-control" placeholder="Phone / Name / Email">
    </div>
    <div class="col-lg-2 col-md-4">
        <label class="form-label fw-semibold mb-1">State</label>
        <select id="state_filter" class="form-select" onchange="viewtable()">
            <option value="">All States</option>
            <option value="N/A">N/A</option>
            <?php
            $state_query = mysqli_query($con, "SELECT DISTINCT current_state FROM customer_register WHERE current_state IS NOT NULL AND current_state != '' ORDER BY current_state ASC");
            while ($state_row = mysqli_fetch_assoc($state_query)) {
                echo '<option value="' . htmlspecialchars($state_row['current_state']) . '">' . htmlspecialchars($state_row['current_state']) . '</option>';
            }
            ?>
        </select>
    </div>
    <div class="col-lg-2 col-md-4">
        ...
        <select id="deletedstatus" class="form-select" onchange="viewtable()">
    ...
    <thead>
        <tr>
            ...
            <th class="wd-10p border-bottom-0">Reg Thru</th>
            <th class="wd-15p border-bottom-0">Full Name</th>
            <th class="wd-10p border-bottom-0">Phone No.</th>
            <th class="wd-15p border-bottom-0">E-mail</th>
            <th class="wd-10p border-bottom-0">Current State</th>
            <th class="wd-10p border-bottom-0">Current District</th>
            <th class="wd-20p border-bottom-0">Current Address</th>
            <th class="wd-10p border-bottom-0">Wallet Balance</th>
            <th class="wd-10p border-bottom-0">Bonus Points</th>
...
<style>
    /* Kill hover color changes for these specific tabs */
    #jobTabs .nav-link:not(.active):hover {
        background-color: transparent !important;
        color: inherit !important;
    }
</style>

<div class="modal fade" id="customerJobsModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-light">
                <h5 class="modal-title fw-semibold text-dark">
                    <i class="fa fa-briefcase text-primary me-2"></i> Customer Jobs Overview
                </h5>
            ...
            <div class="modal-body p-4">
                ...
                <div class="d-flex justify-content-center mb-4">
                    <ul class="nav nav-pills p-2 rounded-pill" id="jobTabs" role="tablist" style="background-color: #f1f5f9;">
                        <li class="nav-item me-3" role="presentation">
                            <button class="nav-link active rounded-pill px-4 py-2 fw-semibold text-dark shadow-none border-0" id="regular-tab" data-bs-toggle="tab" data-bs-target="#regular-jobs" type="button" role="tab" aria-selected="true" style="font-size: 14px;">
                                Regular Bookings
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link rounded-pill px-4 py-2 fw-semibold text-dark shadow-none border-0" id="carpool-tab" data-bs-toggle="tab" data-bs-target="#carpool-jobs" type="button" role="tab" aria-selected="false" style="font-size: 14px;">
                                Car Pools
                            </button>
                        </li>
                    </ul>
                ...
                <div class="tab-content" id="jobTabsContent">
                    <div class="tab-pane fade show active" id="regular-jobs" role="tabpanel" aria-labelledby="regular-tab">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover w-100 align-middle" id="regularJobsTable">
                    ...
                    <div class="tab-pane fade" id="carpool-jobs" role="tabpanel" aria-labelledby="carpool-tab">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover w-100 align-middle" id="carpoolJobsTable">
                                <thead class="table-light">
                                    <tr style="font-size: 13px; white-space: nowrap;">
                                        <th>Job No</th>
                                        <th>Created At</th>
...
// Function to fetch and display the user's jobs
function viewCustomerJobs(userId) {
    let regTbody = $('#regularJobsTable tbody');
    let poolTbody = $('#carpoolJobsTable tbody');
    
    // Clear out tables and show a loading state
    regTbody.html('<tr><td colspan="8" class="text-center py-4"><span class="spinner-border spinner-border-sm text-primary"></span> Loading...</td></tr>');
    // Adjusted colspan to 7 since carpool doesn't have a status column
    poolTbody.html('<tr><td colspan="7" class="text-center py-4"><span class="spinner-border spinner-border-sm text-primary"></span> Loading...</td></tr>');
    
    // Open Modal Instantly
    $('#customerJobsModal').modal('show');

    $.ajax({
        type: 'POST',
        url: url, // Assuming 'url' points to datatable_services.php 
        data: {
            method: 'get_customer_jobs',
            user_id: userId
        },
        success: function(response) {
            ...
            if (response.status) {
                // Populate Regular Jobs
                if (response.regular.length > 0) {
                    response.regular.forEach(job => {
                        regTbody.append(buildJobRow(job, false)); // false = not a carpool job
                    });
                } else {
                    regTbody.append('<tr><td colspan="8" class="text-center text-muted py-3">No regular bookings found.</td></tr>');
                }

                // Populate Carpool Jobs
                if (response.carpool.length > 0) {
                    response.carpool.forEach(job => {
                        poolTbody.append(buildJobRow(job, true)); // true = is a carpool job
                    });
                } else {
                    poolTbody.append('<tr><td colspan="7" class="text-center text-muted py-3">No car pool jobs found.</td></tr>');
                }
            } else {
                regTbody.append('<tr><td colspan="8" class="text-center text-muted py-3">No jobs found for this customer.</td></tr>');
                poolTbody.append('<tr><td colspan="7" class="text-center text-muted py-3">No jobs found for this customer.</td></tr>');
            }
        },
        error: function() {
            regTbody.html('<tr><td colspan="8" class="text-center text-danger py-3">Failed to fetch data.</td></tr>');
            poolTbody.html('<tr><td colspan="7" class="text-center text-danger py-3">Failed to fetch data.</td></tr>');
            toast('error', 'Failed to fetch customer jobs.');
        }
    });
}

// Updated Helper function to allow text wrapping for addresses and hide status for carpool
function buildJobRow(job, isCarpool) {
    let statusColumn = '';
    
    // Only generate the status column if it is NOT a carpool job
    if (!isCarpool) {
        statusColumn = `
            <td style="white-space: nowrap;">
                <span class="badge bg-${getBadgeColor(job.status)} px-2 py-1" style="font-size: 11px;">${job.status}</span>
            </td>
        `;
    }
    ...
    return `
        <tr>
            <td class="" style="font-size: 13px; white-space: nowrap;">${job.job_no}</td>
            <td style="font-size: 13px; white-space: nowrap;">${job.created_at}</td>
            <td style="font-size: 13px; white-space: nowrap;">${job.pickup_date}</td>
            <td class="text-center fw-bold" style="font-size: 13px;">${job.distance}</td>
            <td style="font-size: 13px; white-space: normal; min-width: 200px; line-height: 1.4;">${job.from_place}</td>
            <td style="font-size: 13px; white-space: normal; min-width: 200px; line-height: 1.4;">${job.to_place}</td>
            <td class="fw-bold" style="font-size: 13px; white-space: nowrap;">${job.fare}</td>
            ${statusColumn}
        </tr>
    `;
}

// Helper function to color code the statuses beautifully
function getBadgeColor(status) {
    if (!status) return 'secondary'; // Added safety fallback
    let st = status.toLowerCase();
    if (st === 'completed') return 'success';
    if (st === 'cancelled' || st === 'expired') return 'danger';
    if (st === 'started' || st === 'assigned') return 'primary';
    if (st === 'incomplete') return 'warning text-dark';
    return 'secondary'; // Default for Unassigned
}
    ...
    function submitPushNotification() {
        ...
        $.ajax({
            type: 'POST',
            url: 'https://mapi.goride.run/api/send-customer-push',
            data: formdata,
            headers: {
    ...
    if (datefrom == '' || datefrom != '' || datefill != '' || datefill == '' || fieldname != '' || deletedstatus != '' || deletedstatus == '' || state_filter != '') {
        var table = $('#exampleTable').DataTable();
        table.destroy();
        
        var excelTitle = 'My Users List Reports';
        if (fieldname != '') { excelTitle += ' : ' + fieldname; }
        if (state_filter != '') { excelTitle += ' - ' + state_filter; }
        ...
        table = $("#exampleTable").DataTable({
            ...
            columns: [{
                ...
                },
                {
                    data: "status",
                    render: function(data, type, row) {
                        // 0 = Active (Checked/Purple), 1 = Inactive (Unchecked/Grey)
                        let isChecked = (data == 0) ? 'checked' : '';
                        let bgColor = (data == 0) ? '#49117cc9' : '#ccc';
                        let transform = (data == 0) ? 'translateX(22px)' : 'translateX(0)';

                        return `<label class="custom-switch form-switch mb-0" style="cursor: pointer; position: relative; display: inline-block; width: 44px; height: 24px;">
                                    <input type="checkbox" class="toggle-profile" data-id="${row.id}" ${isChecked} style="opacity: 0; width: 0; height: 0;">
                                    <span style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background-color: ${bgColor}; border-radius: 24px; transition: .4s;">
                                        <span class="slider-inner" style="position: absolute; height: 18px; width: 18px; left: 3px; bottom: 3px; background-color: white; border-radius: 50%; transition: .4s; transform: ${transform};"></span>
                                    </span>
                                </label>`;
                    }
                },
