<?php
/**
 * 16/04/2023   Devanathan     THE WORD UNDEFINED IS OCCURING title of excel download name
 */
$DIR = dirname(__DIR__);
...
if ($tabID == 'customer') {
    ...
    $idname = 'Field Officer ID';
    $addName = 'Field Officer';
...
<div class="main-content app-content mt-0">
        ...
        <div class="main-container container-fluid">
            <div class="page-header">
                <h1 class="page-title"><a href="javascript:void(0)" class="back-arrow-btn"><i class="fa fa-chevron-left"
                            onclick="history.go(-1)" aria-hidden="true"></i></a><?= $title; ?></h1>
                <div>
                    <ol class="breadcrumb">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            ...
                                            <div>
                                                <input class="form-control" type="text" id="datefilter"
                                                    name="datefilter" value="" placeholder="YYYY-MM-DD"
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <input type="text" name="fieldname" id="fieldname"
                                                oninput="viewtable($(this).val())" class="form-control"
                                        ...
                                        <?php if ($tabID != 'staffs' && $tabID != 'agents' && $tabID != 'field' &&  $tabID = 'customer') { ?>
                                            <div class="col-md-3">
                                                <span>KYC filter</span>
                                                <select class="form-control" id="kyc_filter">
                                                    <option value="all">All</option>
                                                    <option value="k_n_c">KYC not completed</option>
                                                    <option value="k_n_v">KYC not verify</option>
                                                    <option value="k_r">KYC rejected</option>
                                                    <option value="k_v">KYC verifed</option>
                                                    <option value="v_n_c">Vehicle not completed</option>
                                                    <option value="v_n_v">Vehicle not verify</option>
                                                    <option value="v_r">Vehicle rejected</option>
                                                    <option value="v_v">Vehicle verifed</option>
                                                </select>
                                            </div>
                                        ...
                                        <?php if ($tabID != 'support') { ?>
                                            <div class="col-md-3 text-center">
                                                <br>
                                                <button style="margin-top: 0px;" onclick="viewtable()">GO</button>
                                            </div>
                                        </div>
                                    <?php } ?>
                                </div>
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <?php if ($roll_id != '0' && strtolower(trim($addName)) != 'customer') { ?>
            <button class="btn btn-success" type="button" onclick="$('#addagent').modal('show')">
                <i class="fa fa-plus me-1"></i> Create <?= $addName; ?>
            </button>
        <?php } ?>
    </div>
    <div class="card-body">
                ...
                <thead>
                    <tr>
                        ...
                        <?php if ($tabID == 'customer') { ?>
                            ...
                            <th class="wd-15p border-bottom-0">Earning Balance</th>
                        <?php } ?>
                        ...
                        <?php if ($tabID == 'field') { ?>
                            <th class="wd-25p border-bottom-0">Owners Count</th>
                        <?php } ?>

                        <?php if ($tabID == 'customer') { ?>
                            <th class="wd-25p border-bottom-0">Kyc Status</th>
                            <th class="wd-25p border-bottom-0">Vehicle Status</th>
                        <?php } ?>
            ...
            <div class="cls_form_div">
                <form class="login100-form validate-form cls_booking_form">
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <label class="lb-text">First Name&nbsp;<sup
                                        style="color: red;weight: bolder;">*</sup></label><br>
                                <div class="wrap-input100 validate-input input-group">
                                    <a href="javascript:void(0)" class="input-group-text bg-white text-muted"><i
                                            class="mdi mdi-account" aria-hidden="true"></i></a>
                                    <input class="input100 border-start-0 ms-0 form-control req-empty" id="name"
                                        name="name" oninput="this.value = this.value.replace(/[^A-Za-z. ]/g, '');"
                                        type="text" placeholder="First Name" required>
                                </div>
                                <label class="lb-text">Last Name&nbsp;<sup
                                        style="color: red; weight: bolder;">*</sup></label><br>
                                <div class="wrap-input100 validate-input input-group">
                                    <a href="javascript:void(0)" class="input-group-text bg-white text-muted"><i
                                            class="mdi mdi-account" aria-hidden="true"></i></a>
                                    <input class="input100 border-start-0 ms-0 form-control req-empty" name="lname"
                                        id="lname" oninput="this.value = this.value.replace(/[^A-Za-z. ]/g, '');"
                                        type="text" placeholder="Last Name" required>
                                </div>
                                <label class="lb-text">Date of Birth&nbsp;<sup
                                        style="color: red; weight: bolder;">*</sup></label><br>
                                <div class="wrap-input100 validate-input input-group">
                                    <a href="javascript:void(0)" class="input-group-text bg-white text-muted">
                                        <i class="fa fa-calendar" aria-hidden="true"></i>
                                    </a>
                                    <input type="date" value="<?= $row['date_of_birth']; ?>" name="dob"
                                        class="form-control" id="dob" placeholder="Date of Birth">
                                </div>
                                <label class="lb-text">Mobile No&nbsp;<sup
                                        style="color: red;weight: bolder;">*</sup></label><br>
                                <div class="wrap-input100 validate-input input-group">
                                    <input id="phone" type="tel" oninput="mobile_number_validation($(this).val())"
                                        onfocusout="Email_check()"
                                        oninput="this.value = this.value.replace(/[^0-9]/g, '');" maxlength="15"
                                        required>
                                    <span id="valid-msg" class="hide">Valid</span>
                                    <span id="error-msg" class="hide">Invalid number</span>
                                </div>
                                <label class="lb-text">Email&nbsp;<sup
                                        style="color: red;weight: bolder;">*</sup></label><br>
                                <div class="wrap-input100 validate-input input-group">
                                    <a href="javascript:void(0)" class="input-group-text bg-white text-muted"><i
                                            class="zmdi zmdi-email" aria-hidden="true"></i></a>
                                    <input class="input100 border-start-0 ms-0 form-control req-empty" id="email"
                                        name="email" type="email" placeholder="Email">
                                </div>
                                <div class="wrap-input100 validate-input input-group">
                                    <div class="form-group" style="width: 100%;" id="gfhdjkdk">
                                        <label class="lb-text">Country&nbsp;<sup
                                                style="color: red;weight: bolder;">*</sup></label>
                                        <div class="disc-pot">
                                            ...
                                            <select class="form-control form-select selectpicker"
                                                data-live-search="true" id="nationlaity" name="nationlaity" required>
                                                ...
                                                while ($value = mysqli_fetch_array($countries)): ?>
                                                    <option value="<?= $value['id']; ?>"
                                                        <?= (strtolower(utf8_encode($value['name'])) == strtolower(utf8_encode($row['nationality']))) ? 'selected' : ''; ?>><?= utf8_encode($value['name']); ?></option>
                                                <?php endwhile; ?>
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="wrap-input100 validate-input input-group">
                                    <div class="form-group" style="width: 100%;" id="gfhdjkdk">
                                        <label class="lb-text">Country of Residence&nbsp;<sup
                                                style="color: red;weight: bolder;">*</sup></label>
                                        <div class="disc-pot">
                                            <select class="form-control form-select selectpicker"
                                                data-live-search="true" id="residinglocation" name="residinglocation"
                                                required>
                                                <option value="">Select Country</option>
                                                <?php
                                                $countries = mysqli_query($con, "SELECT `countries`.`id` AS `id`, `countries`.`name` AS `name`, COUNT(states.id) AS `statecount` FROM `countries` INNER JOIN `states` ON `countries`.`id` = `states`.`country_id` WHERE `countries`.`flag` = '1' GROUP BY countries.id HAVING `statecount` > 0 ORDER BY name ASC;");
                                ...
                                <?php //if ($tabID != 'customer') { ?>
                                <label class="lb-text">Password&nbsp;<sup
                                        style="color: red; font-weight: bolder;">*</sup></label><br>
                                <div class="wrap-input100 validate-input input-group">
                                    <a href="javascript:void(0)"
                                        class="input-group-text bg-white text-muted toggle-password">
                                        <i class="fa fa-eye" toggle="#password" aria-hidden="true"></i>
                                    </a>
                                    <input class="input100 border-start-0 ms-0 form-control" id="password"
                                        name="Password" type="password" placeholder="Password" minlength="6"
                                        maxlength="20">&nbsp;
                                    <i onclick="generatePassword(7)"
                                        style="font-size: 20px!important;color: lime;cursor: pointer;margin: auto 0;"
                    ...
                    <div class="modal-footer">
                        <div id="rebtn">
                            <button class="btn ripple btn-success" id="smallmodal1" type="button"
                                onclick="sendotp()">Create <?= $addName; ?></button>
                        </div>
                    </div>
                </form>
            </div>
                    ...
                    <form action="#" class="otp_form">
                        <div class="form-group">
                            <div class="model-text">
                                ...
                                <p class="text-center">Enter the code we just send on your <b id="title"></b> <b
                                        id="mno"></b></p>
                                <br>
                            </div>
                            <div class="row">
                                <div class="col-3">
                                    <input type="text" maxlength="1"
                                        oninput="this.value=this.value.replace(/[^0-9]/g,'');"
                                        onkeyup="onKeyUpEvent(1, event)" onfocus="onFocusEvent(1)" class="form-control"
                                        id="otp1" name="otp1">
                                </div>
                                <div class="col-3">
                                    <input type="text" maxlength="1"
                                        oninput="this.value=this.value.replace(/[^0-9]/g,'');"
                                        onkeyup="onKeyUpEvent(2, event)" onfocus="onFocusEvent(2)" class="form-control"
                                        id="otp2" name="otp2">
                                </div>
                                <div class="col-3">
                                    <input type="text" maxlength="1"
                                        oninput="this.value=this.value.replace(/[^0-9]/g,'');"
                                        onkeyup="onKeyUpEvent(3, event)" onfocus="onFocusEvent(3)" class="form-control"
                                        id="otp3" name="otp3">
                                </div>
                                <div class="col-3">
                                    <input type="text" maxlength="1"
                                        oninput="this.value=this.value.replace(/[^0-9]/g,'');"
                                        onkeyup="onKeyUpEvent(4, event)" onfocus="onFocusEvent(4)" class="form-control"
                                        id="otp4" name="otp4">
                                </div>
                            </div>
                        </div>
...
<div class="modal fade" id="showimgmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
    aria-hidden="true">
...
<div class="modal fade" id="deleteinfo" tabindex="-1" role="dialog" aria-labelledby="customModalLabel"
    aria-modal="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="text-center">
                    <p class="mb-3 modalques">Are you sure to Delete Driver/Owner?</p>
                </div>
            </div>
            <div class="modal-footer custom">
                ...
                <div class="right-side">
                    <button aria-label="Close" class="btn btn-danger pd-x-25 success"
...
<script id="rendered-js">
    ...
    //     let kyc_filter = $('#kyc_filter').val();
    //     let user_type_filter = $('#user_type_filter').val(); // Capture new filter
    ...
    //                 buttons: [
    //                     'pageLength', 'copy',
    //                     { extend: 'excelHtml5', title: excelTitle }
    //                 ],
    //                 columns: [
    //                     { data: "nid" },
    //                     { data: "rollType" },
    ...
    //                     { data: "email" },
    //                     { data: "o_count" },
    //                     { data: null, render: function (data, type, row, meta) { return moment(data.createdat).format("DD MMM YYYY hh:mm a") } },
    //                     { data: "action" }
    //                 ]
    //             });
    //         } else if (tabID == 'customer') {
    //             if ((datefrom != '' || datefill != '') && (deletedstatus == 1 && deletedstatus != "")) {
    //                 var excelTitle = 'My Customer List Reports : (' + fieldname + ' Deleted ' + datefrom + ' to ' + datefill + ')';
    //             } else if ((datefrom != '' || datefill != '') && (deletedstatus == 0 && deletedstatus != "")) {
    //                 var excelTitle = 'My Customer List Reports : (' + fieldname + ' Active ' + datefrom + ' to ' + datefill + ')';
    //             } else if (datefrom != '' || datefill != '') {
    //                 var excelTitle = 'My Customer List Reports : (' + datefrom + ' to ' + datefill + ')';
    //             } else if (deletedstatus == 1) {
    //                 var excelTitle = 'My Customer List Reports : (' + fieldname + ' Deleted )';
    //             } else if (deletedstatus == "") {
    //                 var excelTitle = 'My Customer List Reports : (all data)';
    //             } else if (deletedstatus == 0) {
    //                 var excelTitle = 'My Customer List Reports: (' + fieldname + ' Active )';
    //             }
    //             table = $("#exampleTable").DataTable({
    //                 pageLength: 10,
    //                 order: [[4, 'desc']],
    ...
    //                             if (data.type == 'Driver' || data.type == 'Owner') {
    //                                 return data.type;
    //                             } else {
    //                                 return 'N/A';
    //                             }
    ...
    //                         data: null,
    //                         render: function (data, type, row, meta) {
    //                             if (data.doc_verify == 0 && data.id == null) {
    //                                 return 'Not complete';
    //                             }
    //                             else if (data.doc_verify == 0 && data.id != null && data.o_status != 3) {
    //                                 return 'Pending';
    //                             }
    //                             else if (data.doc_verify == 0 && data.id != null && data.o_status == 3) {
    //                                 return 'Not Verify';
    //                             }
    //                             else if (data.doc_verify == 1) {
    //                                 return 'Verified';
    //                             }
    //                             else {
    //                                 return 'N/A';
    ...
    //                         data: null,
    //                         render: function (data, type, row, meta) {
    //                             if (data.type == 'Driver' || data.type == null) {
    //                                 if (data.vehicle_verify == 0 && data.vehicle_details == null) {
    //                                     return 'Not complete';
    //                                 } else if (data.vehicle_verify == 1 && data.vehicle_details != null) {
    //                                     return 'Not verify';
    //                                 } else if (data.vehicle_verify == 2 && data.vehicle_details != null) {
    //                                     return 'Verified';
    //                                 } else {
    //                                     return 'N/A';
    //                                 }
    ...
    //                     data: null,
    //                     render: function (data, type, row, meta) {
    //                         // Always show the KYC button. 
    //                         // If data.id (kyc_details id) is null, default it to 0 so the URL still works.
    //                         let kycId = data.id || data.kyc_id || 0;
                            
    //                         let kycButton = `&nbsp;&nbsp;<a href="https://console.goride.run/kyc-verify/verify/${data.nid}/${kycId}" target="_blank" style="cursor: pointer;" title="KYC Verification"><span class="fa fa-id-card" style="color: #198754;font-size: 16px;font-weight: bold;">&nbsp;Kyc</span></a>`;

    //                         return `
    //                             <a class="btn text-danger btn-sm" data-bs-target="#deleteinfo" data-bs-toggle="modal" data-bs-toggle="tooltip" style="cursor: pointer;" data-bs-original-title="Delete">
    //                                 <span class="fe fe-trash-2" style="color: #ff0000;font-size: 16px;font-weight: bold;" onclick=deletelist('${data.nid}')></span>
    //                             </a>&nbsp;&nbsp;
    //                             <a href="/profile/edit/${data.nid}" style="cursor: pointer;">
    //                                 <span class="fa fa-eye" style="color: #49117cc9;font-size: 16px;font-weight: bold;">&nbsp;Profile</span>
    //                             </a>
    //                             ${kycButton}
    //                         `;
    //                     }
    ...
    //         } else if (tabID == 'kioskcreater') {
    //             if ((datefrom != '' || datefill != '') && (deletedstatus == 1 && deletedstatus != "")) {
    //                 var excelTitle = 'My kiosk List Reports : (' + fieldname + ' Deleted ' + datefrom + ' to ' + datefill + ')';
    //             } else if ((datefrom != '' || datefill != '') && (deletedstatus == 0 && deletedstatus != "")) {
    //                 var excelTitle = 'My kiosk List Reports : (' + fieldname + ' Active ' + datefrom + ' to ' + datefill + ')';
    //             } else if (datefrom != '' || datefill != '') {
    //                 var excelTitle = 'My kiosk List Reports : (' + fieldname + ' ' + datefrom + ' to ' + datefill + ')';
    //             } else if (deletedstatus == 1) {
    //                 var excelTitle = 'My kiosk List Reports : (' + fieldname + ' Deleted )';
    //             } else if (deletedstatus == "") {
    //                 var excelTitle = 'My kiosk List Reports : (all data)';
    //             } else if (deletedstatus == 0) {
    //                 var excelTitle = 'My kiosk List Reports : (' + fieldname + ' Active )';
    //             }
    //             table = $("#exampleTable").DataTable({
    //                 pageLength: 10,
    //                 order: [[6, 'desc']],
    ...
    //                 lengthMenu: [[10, 25, 50, 100, 200, 500, -1], [10, 25, 50, 100, 200, 500, 'All']],
    //                 buttons: [
    //                     'pageLength', 'copy', { extend: 'excelHtml5', title: excelTitle }
    //                 ],
    //                 columns: [
    //                     { data: "nid" },
    //                     { data: "rollType" },
    ...
    //             if ((datefrom != '' || datefill != '') && (deletedstatus == 1 && deletedstatus != "")) {
    //                 var excelTitle = 'My Staffs List Reports : (' + fieldname + ' Deleted ' + datefrom + ' to ' + datefill + ')';
    //             } else if ((datefrom != '' || datefill != '') && (deletedstatus == 0 && deletedstatus != "")) {
    //                 var excelTitle = 'My Staffs List Reports : (' + fieldname + ' Active ' + datefrom + ' to ' + datefill + ')';
    //             } else if (datefrom != '' || datefill != '') {
    //                 var excelTitle = 'My Staffs List Reports : (' + fieldname + ' ' + datefrom + ' to ' + datefill + ')';
    //             } else if (deletedstatus == 1) {
    //                 var excelTitle = 'My Staffs List Reports : (' + fieldname + ' Deleted )';
    //             } else if (deletedstatus == "") {
    //                 var excelTitle = 'My Staffs List Reports : (all data)';
    //             } else if (deletedstatus == 0) {
    //                 var excelTitle = 'My Staffs List Reports : (' + fieldname + ' Active )';
    //             }
    //             table = $("#exampleTable").DataTable({
    //                 pageLength: 10,
    //                 order: [[6, 'desc']],
    ...
    //         } else if (tabID == 'affiliate') {
    //             if ((datefrom != '' || datefill != '') && (deletedstatus == 1 && deletedstatus != "")) {
    //                 var excelTitle = 'Affiliate User List : (' + fieldname + ' Deleted ' + datefrom + ' to ' + datefill + ')';
    //             } else if ((datefrom != '' || datefill != '') && (deletedstatus == 0 && deletedstatus != "")) {
    //                 var excelTitle = 'Affiliate User List : (' + fieldname + ' Active ' + datefrom + ' to ' + datefill + ')';
    //             } else if (datefrom != '' || datefill != '') {
    //                 var excelTitle = 'Affiliate User List : (' + fieldname + ' ' + datefrom + ' to ' + datefill + ')';
    //             } else if (deletedstatus == 1) {
    //                 var excelTitle = 'Affiliate User List : (' + fieldname + ' Deleted )';
    //             } else if (deletedstatus == "") {
    //                 var excelTitle = 'Affiliate User List : (all data)';
    //             } else if (deletedstatus == 0) {
    //                 var excelTitle = 'Affiliate User List : (' + fieldname + ' Active )';
    //             }
    //             table = $("#exampleTable").DataTable({
    //                 pageLength: 10,
    //                 order: [[6, 'desc']],
    ...
    //         } else if (tabID == 'field') {
    //             if ((datefrom != '' || datefill != '') && (deletedstatus == 1 && deletedstatus != "")) {
    //                 var excelTitle = 'Field Officers User List : (' + fieldname + ' Deleted ' + datefrom + ' to ' + datefill + ')';
    //             } else if ((datefrom != '' || datefill != '') && (deletedstatus == 0 && deletedstatus != "")) {
    //                 var excelTitle = 'Field Officers List : (' + fieldname + ' Active ' + datefrom + ' to ' + datefill + ')';
    //             } else if (datefrom != '' || datefill != '') {
    //                 var excelTitle = 'Field Officers List : (' + fieldname + ' ' + datefrom + ' to ' + datefill + ')';
    //             } else if (deletedstatus == 1) {
    //                 var excelTitle = 'Field Officers List : (' + fieldname + ' Deleted )';
    //             } else if (deletedstatus == "") {
    //                 var excelTitle = 'Field Officers List : (all data)';
    //             } else if (deletedstatus == 0) {
    //                 var excelTitle = 'Field Officers List : (' + fieldname + ' Active )';
    //             }
    //             table = $("#exampleTable").DataTable({
    //                 pageLength: 10,
    //                 order: [[6, 'desc']],
    ...
    //         } else if (tabID == 'support') {
    //             if ((datefrom != '' || datefill != '') && (deletedstatus == 1 && deletedstatus != "")) {
    //                 var excelTitle = 'Customer support List Reports : (' + fieldname + ' Deleted ' + datefrom + ' to ' + datefill + ')';
    //             } else if ((datefrom != '' || datefill != '') && (deletedstatus == 0 && deletedstatus != "")) {
    //                 var excelTitle = 'Customer support List Reports : (' + fieldname + ' Active ' + datefrom + ' to ' + datefill + ')';
    //             } else if (datefrom != '' || datefill != '') {
    //                 var excelTitle = 'Customer support List Reports : (' + fieldname + ' ' + datefrom + ' to ' + datefill + ')';
    //             } else if (deletedstatus == 1) {
    //                 var excelTitle = 'Customer support List Reports : (' + fieldname + ' Deleted )';
    //             } else if (deletedstatus == "") {
    //                 var excelTitle = 'Customer support List Reports : (all data)';
    //             } else if (deletedstatus == 0) {
    //                 var excelTitle = 'Customer support List Reports : (' + fieldname + ' Active )';
    //             }
    //             table = $("#exampleTable").DataTable({
    //                 pageLength: 10,
    //                 order: [[0, 'desc']],
    ...
    // }
    function viewtable() {
    const tabID = $('#tabID').val();
    const newdate = $('#datefilter').val();
    const fieldname = $('#fieldname').val();
    const deletedstatus = $('#deletedstatus').val();
    const kyc_filter = $('#kyc_filter').val();
    const user_type_filter = $('#user_type_filter').val();

    // 1. Resolve dates using Moment.js
    let datefrom = '';
    let datefill = '';

    if (newdate && $('#datefilter').data('daterangepicker')) {
        const picker = $('#datefilter').data('daterangepicker');
        datefrom = moment(picker.startDate).format("YYYY-MM-DD HH:mm:ss");
        datefill = moment(picker.endDate).format("YYYY-MM-DD HH:mm:ss");
    } else {
        datefrom = moment().startOf('day').format("YYYY-MM-DD HH:mm:ss");
        datefill = moment().endOf('day').format("YYYY-MM-DD HH:mm:ss");
    }

    // Standard column layout: ID, Type, Name, Phone, Email, Created On, Action (7 cols)
    const standardColumns = [
        ...
        { 
            data: null, 
            render: data => data.createdat ? moment(data.createdat).format("DD MMM YYYY hh:mm a") : "-" 
        },
        { data: "action", defaultContent: "-" }
    ];

    // 2. Comprehensive Tab Configuration Map (Fixes "Invalid tab selected")
    const tabConfigs = {
        // AGENTS (8 columns: ID, Type, Name, Phone, Email, Country, Created On, Action)
        agents: {
            role: '3',
            type: 'agent',
            titlePrefix: 'My Agent List Reports',
            dateTargetCol: 6,
            order: [[6, 'desc']],
            columns: [
                { data: "nid" },
                ...
                { data: null, render: data => moment(data.createdat).format("DD MMM YYYY hh:mm a") },
                { data: "action", defaultContent: "-" }
            ]
        },

        // CUSTOMERS / DRIVERS & OWNERS (12 columns: State visible here)
        customer: {
            role: '0',
            type: 'customer',
            titlePrefix: 'My Customer List Reports',
            dateTargetCol: 10,
            order: [[10, 'desc']],
            columns: [
                { data: "nid" },
                { data: null, render: data => ['Driver', 'Owner'].includes(data.type) ? data.type : 'N/A' },
                { data: "name" },
                { data: "mobile" },
                { data: "email" },
                { data: "state", defaultContent: "N/A" },
                { data: "walletBalance", render: data => data ? parseFloat(data).toFixed(2) : "0.00" },
                { data: "winningBalance", render: data => data ? parseFloat(data).toFixed(2) : "0.00" },
                {
                    data: null,
                    render: data => {
                        if (data.doc_verify == 0) {
                            if (data.id == null) return 'Not complete';
                            return data.o_status == 3 ? 'Not Verify' : 'Pending';
                        }
                        return data.doc_verify == 1 ? 'Verified' : 'N/A';
                    }
                },
                {
                    data: null,
                    render: data => {
                        if (data.type === 'Driver' || data.type === null) {
                            if (data.vehicle_verify == 0 && data.vehicle_details == null) return 'Not complete';
                            if (data.vehicle_verify == 1 && data.vehicle_details != null) return 'Not verify';
                            if (data.vehicle_verify == 2 && data.vehicle_details != null) return 'Verified';
                        }
                        return '-';
                    }
                },
                { data: null, render: data => moment(data.createdat).format("DD MMM YYYY hh:mm a") },
                {
                    data: null,
                    render: data => {
                        const kycId = data.id || data.kyc_id || 0;
                        return `
                            <a class="btn text-danger btn-sm" data-bs-target="#deleteinfo" data-bs-toggle="modal" title="Delete" style="cursor: pointer;">
                                <span class="fe fe-trash-2" style="color: #ff0000;font-size: 16px;font-weight: bold;" onclick="deletelist('${data.nid}')"></span>
                            </a>&nbsp;&nbsp;
                            <a href="/profile/edit/${data.nid}" style="cursor: pointer;">
                                <span class="fa fa-eye" style="color: #49117cc9;font-size: 16px;font-weight: bold;">&nbsp;Profile</span>
                            </a>&nbsp;&nbsp;
                            <a href="https://console.goride.run/kyc-verify/verify/${data.nid}/${kycId}" target="_blank" style="cursor: pointer;" title="KYC Verification">
                                <span class="fa fa-id-card" style="color: #198754;font-size: 16px;font-weight: bold;">&nbsp;Kyc</span>
                            </a>
                        `;
                    }
                }
        ...
        // FIELD OFFICERS (8 columns: ID, Type, Name, Phone, Email, Owners Count, Created On, Action)
        field: {
            role: '8',
            titlePrefix: 'Field Officers List',
            dateTargetCol: 6,
            order: [[6, 'desc']],
            columns: [
                { data: "nid" },
                ...
                { data: "mobile" },
                { data: "email" },
                { data: "o_count", defaultContent: "0" },
                { data: null, render: data => moment(data.createdat).format("DD MMM YYYY hh:mm a") },
                { data: "action", defaultContent: "-" }
            ]
        },

        // STAFFS (7 columns: ID, Type, Name, Phone, Email, Created On, Action)
        staffs: {
            role: '4',
            type: 'staffs',
            titlePrefix: 'My Staffs List Reports',
            dateTargetCol: 5,
            order: [[5, 'desc']],
            columns: standardColumns
        },
        staff: { // Fallback alias
            role: '4',
            type: 'staffs',
            titlePrefix: 'My Staffs List Reports',
            dateTargetCol: 5,
        ...
        // KIOSK CREATORS
        kioskcreater: { role: '9', titlePrefix: 'My Kiosk List Reports', dateTargetCol: 5, order: [[5, 'desc']], columns: standardColumns },
        
        // AFFILIATES
        affiliate: { role: '7', titlePrefix: 'Affiliate User List', dateTargetCol: 5, order: [[5, 'desc']], columns: standardColumns },
        
        // SUPPORT
        support: { role: '11', titlePrefix: 'Customer Support List Reports', dateTargetCol: 5, order: [[0, 'desc']], columns: standardColumns }
    };

    // Fallback key lookup if tabID includes special characters or alternate naming
    let currentConfig = tabConfigs[tabID];
    if (!currentConfig && tabID) {
        const cleanedTabID = tabID.toLowerCase().trim();
        currentConfig = tabConfigs[cleanedTabID];
    }

    if (!currentConfig) {
        console.error("Unrecognized tabID:", tabID);
        return;
    }

    // 3. Dynamic Excel Export Title
    let statusText = '';
    if (deletedstatus === '1' || deletedstatus === 1) statusText = ' Deleted';
    else if (deletedstatus === '0' || deletedstatus === 0) statusText = ' Active';

    let dateText = (datefrom && datefill) ? ` ${datefrom} to ${datefill}` : '';
    let excelTitle = (deletedstatus === "" && !dateText) 
        ? `${currentConfig.titlePrefix} : (all data)` 
        : `${currentConfig.titlePrefix} : (${fieldname}${statusText}${dateText})`;

    // 4. Safely Destroy Table instance
    if ($.fn.DataTable.isDataTable('#exampleTable')) {
        $('#exampleTable').DataTable().clear().destroy();
    }

    // 5. Build Data Payload
    const payload = {
        method: tabID === 'customer' ? 'list_agent2' : 'list_agent',
    ...
    if (currentConfig.type) payload.type = currentConfig.type;
    if (tabID === 'customer') {
        payload.kyc_filter = kyc_filter;
        payload.user_type = user_type_filter;
    }

    // 6. Initialize DataTable
    $('#exampleTable').DataTable({
        pageLength: 10,
        order: currentConfig.order || [[5, 'desc']],
        columnDefs: [{ type: 'date', targets: [currentConfig.dateTargetCol || 5] }],
        paging: true,
        searching: true,
        info: true,
        ajax: {
            ...
            type: "POST",
            data: payload,
            dataSrc: tabID === 'customer' ? response => response.result : ""
        },
        dom: 'Bfrtip',
        lengthMenu: [[10, 25, 50, 100, 200, 500, -1], [10, 25, 50, 100, 200, 500, 'All']],
        buttons: [
    ...
    function deletelist(str) {
        var tabID = $('#tabID').val();
        var heading = "Delete";
        var Question = "Are you sure to Delete Field Officer?";
        if (tabID == 'customer') {
            heading = 'Delete Customer';
            Question = "Are you sure to Delete Driver/Owner?";
        }
        var post_data = {
            deleteid: str,
            method: 'deleteagent'
