...
<div class="main-content app-content mt-0">
    <div class="side-app">
        ...
        <?php if ($role_id != 8) { ?>
            <div class="row g-3 <?php echo ($role_id == 8) ? 'mt-5' : ''; ?>">
                <style>
    /* Adds a smooth, professional hover effect */
    .job-card-clickable {
    ...
    /* Solid & Bold Styling for the numbers */
    .job-stat-item {
        font-size: 13px;
        color: #6c757d;
        white-space: nowrap; /* 🔥 FIX: Forces text to stay on one line, preventing word breaks */
    }
    .job-stat-val {
        font-size: 16px;
        font-weight: 900; /* Extra bold/solid */
        color: #212529; /* Solid dark color */
        margin-left: 4px;
    }
</style>
    ...
    <div class="card shadow-sm border-start border-4 h-100" style="border-color: #20c997;">
        ...
        <div class="card-body p-3">
            ...
            <div class="d-flex flex-wrap gap-3">
                ...
                <!--        <div class="card-body d-flex justify-content-between align-items-center">-->
                <!--            <div>-->
                <!--                <h6 class="fw-bold mb-1" style="color:#ff9800;">Payment History<small>(Today)</small></h6>-->
                ...
                <!--                </div>-->
                <!--            </div>-->
                <!--            <i class="bi bi-receipt fs-1" style="color:#ff9800; opacity: 0.7;"></i>-->
                <!--        </div>-->
                <!--    </div>-->
                <!--</div>-->
                <div class="col-md-6 col-lg-4">
    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content shadow border-0">
            ...
            <div class="modal-body pt-2">
                ...
                <!-- OTP Section -->
                <!--<div id="otpSection" class="mt-4 pt-3 border-top" >-->
                <!--    <label class="form-label small fw-semibold">Enter OTP</label>-->
                <!--    <div class="input-group input-group-sm">-->
                <!--        <input type="text"-->
                <!--            id="otp_code"-->
                <!--            class="form-control"-->
                <!--            placeholder="6-digit code" oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,6)">-->
...
function paymentCounts(startDate, endDate) {
    $.ajax({
        ...
        // 1. Initialize the Date Picker specifically for the Job Details Card
function initJobDetailsDatePicker() {
    ...
    function cb(start, end) {
        ...
        fetchJobDetailsSummary(start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD'));
    }
...
function fetchJobDetailsSummary(startDate, endDate) {
    $.ajax({
        url: origin + "/ajax/service/dashBoard_Services.php",
        type: "POST",
        dataType: "json",
        ...
        success: function(res) {
            ...
            // 🔥 This will log the exact MySQL query in your browser console!
            console.log("SQL Query Executed:\n", res.query);

            if (res.type == 1) {
                $('#jd_total').text(res.data.total_jobs);
                $('#jd_unassigned').text(res.data.unassigned_jobs);
                $('#jd_assigned').text(res.data.assigned_jobs);
                $('#jd_website').text(res.data.website_jobs);
                $('#jd_cancelled').text(res.data.cancelled_jobs);
                $('#jd_expired').text(res.data.expired_jobs);
            } else {
        ...
        error: function(xhr, status, error) {
            ...
            console.log(xhr.responseText); // In case of a 500 error
        }
    });
}

// 3. Start the function on page load
$(function() {
