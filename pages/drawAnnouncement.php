<?php
$pageTitle = ucwords(strtolower('Draw Announcement'));
                ...
                success: function(response) {
                    ...
                    if (response != "") {
                        ...
                        response.result.forEach(element => {
                            ...
                            html += `<br><label>Gold Rate Per Gram (AED)</label><input type="text" id="todayGoldPrize${element.id}" class="form-control" value="${todayGoldPrize}" style="pointer-events: none;" oninput="this.value = this.value.replace(/[^0-9.]/g, '');" maxlength="10" >`;
                            ...
                            if ((element.dailyThirllStatus && element.dailyThirllStatus === 'Active' && element.winThirllRaffleIds != null && element.winThirllRaffleIds != undefined && element.winThirllRaffleIds != '' && element.winThirllRaffleIds != 'NULL') ||
                                (element.weeklyBoosterStatus && element.weeklyBoosterStatus === 'Active' && element.winweeklyBoosterIds != null && element.winweeklyBoosterIds != undefined && element.winweeklyBoosterIds != '' && element.winweeklyBoosterIds != 'NULL') ||
                                (element.monthlyBumperStatus && element.monthlyBumperStatus === 'Active' && element.winBumperRaffleIds != null && element.winBumperRaffleIds != undefined && element.winBumperRaffleIds != '' && element.winBumperRaffleIds != 'NULL') ||
                                (element.dailyThirllStatus === 'Completed' || element.weeklyBoosterStatus === 'Completed' || element.monthlyBumperStatus === 'Completed')) {
                                // const jsonArray = JSON.parse(data.winweeklyBoosterIds);
                                html += `<button id="updateRate${element.id}" class="btn btn-info mt-3" onclick="winnerPreview(${element.id}, '${(element.dailyThirllStatus != 'Active' && element.weeklyBoosterStatus != 'Active' && element.monthlyBumperStatus != 'Active') ? 'WinnerList' : 'WinnerPreview'}')"><span class="fa fa-eye" style="font-size: 16px;font-weight: bold;">&nbsp;${(element.dailyThirllStatus != 'Active' && element.weeklyBoosterStatus != 'Active' && element.monthlyBumperStatus != 'Active') ? 'Winner List' : 'Winner Preview'}</span></button>`;
                                // html += `<a class="mt-3" style="cursor: pointer;" ></a>&nbsp;&nbsp;`;
                            }
                    ...
                    if (response != "") {
                        if (response.type == '1') {
                            ...
                            if (response.winnerData?.winThirllRaffleIds?.RaffleID != null) {
                                ...
                                var thrillDrawPre = '';
                                thrillDrawPre += `<h4 class="text-white text-center">Thrill Draw Prize GOLD (KG)</h4>`;
                            ...
                            if (response != "") {
                                if (response.type == '1') {
                                    ...
                                    if (response.result != '' && response.result != null && response.result != undefined) {
                                        ...
                                        setTimeout(function() {
                                            $(`[data-date-val="${moment(activeDate).format('MM/DD/YYYY')}"]`).trigger('click');
                                            // calendar.evoCalendar('selectDate', moment(activeDate, 'YYYY-MM-DD'));
                                        }, 5000);
