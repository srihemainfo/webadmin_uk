                                ...
                                <div class="card-header bg-white pb-3 border-bottom">
                                    ...
                                    <div class="row g-2 mb-2">
                                        ...
                                        <div class="col-6">
                                            <select class="form-select form-select-sm shadow-none" id="gdFilterCab">
                                                ...
                                                <option value="go 6seater">Go 6Seater</option>
                                                <option value="go 7seater">Go 7Seater</option>
                                            </select>
                                        </div>
                                        <div class="col-6">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center pt-1">
                                        ...
                                        <button class="btn btn-sm btn-success px-3 py-1 shadow-sm" id="gdSendPushBtn" disabled>
                                            <i class="fa fa-paper-plane me-1"></i> Send Push (<span id="gdSelectedCount">0</span>)
                                        </button>
                                    </div>
                                </div>
                                <div class="card-body p-0">
          ...
          <div class="col-6">
           ...
           <div class="col-6">
            ...
            ${(job.feedback_rating || (isExpired === true && (job.job_status === 'accept' || job.job_status === 'accepted'))) ? `
                <div class="col-12 mt-2 text-start">
                    <button type="button" class="btn btn-sm btn-outline-warning py-1 px-2 view-feedback-btn text-dark fw-semibold" 
                            data-rating="${job.feedback_rating || 0}" 
                            data-review="${String(job.feedback_review || 'No text review provided.').replace(/"/g, '&quot;')}">
        ...
        $(document).on('click', '#getDistanceBtn', function() {
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    // Optional: Clear previous values while loading
                    $('#distance, #duration, #baseFare, #tollFare, #taxFare').val('');
                    $('#totalFare').text('₹0');
                },
                success: function(response) {
                    if (response.status && response.data) {
                        // 1. Determine which car type is currently selected in the dropdown
                        let selectedCarKey = $('#carType').val(); // e.g., 'four_seater'
                        let cab = response.data[selectedCarKey];
                        // Safe Fallback: if the specific key doesn't exist, grab the first available
                        if (!cab) {
                            const availableKeys = Object.keys(response.data);
                            cab = response.data[availableKeys[0]];
                        }
                        // 2. Fill the standard inputs
                        $('#distance').val(cab.distance);
                        $('#duration').val(cab.duration);
                        $('#baseFare').val(cab.fare);
                        $('#tollFare').val(cab.toll_fare);
                        // 3. FILL THE TAX FIELD 
                        // We use cab.tax_fare based on the JSON you provided
                        let taxValue = cab.tax_fare || 0;
                        $('#taxFare').val(taxValue);
                        // 4. CALCULATE AND FILL THE TOTAL
                        // Sum: fare (1814) + toll_fare (55) + tax_fare (86) = 1955
...
<div class="border p-3 mb-3">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-2">
            ...
            <input type="number" id="taxFare" class="form-control" value="${job.tax || 0}">
        </div>
        <div class="col-md-2">
    ...
    $(document).off('click', '#updateJobBtn').on('click', '#updateJobBtn', function() {
        ...
        const tollFare = parseFloat($('#tollFare').val()) || 0;
        const taxFare = parseFloat($('#taxFare').val()) || 0;
        // Failsafe: if the old script messed up the UI total, override it mathematically
        let totalFare = parseFloat($('#editTotalFare').val()) || 0;
        if (totalFare === (baseFare + tollFare) && taxFare > 0) {
            totalFare = baseFare + tollFare + taxFare;
        } else if (totalFare <= 0) {
            totalFare = baseFare + tollFare + taxFare;
        }
        ...
        const payload = {
            ...
            base_fare: baseFare,
            toll_fare: tollFare,
            tax: taxFare, // 🔥 Perfect Tax Payload
            fare: totalFare,
...
<script>
    ...
    $(document).on('click', '#sendPushBtn', function() {
        ...
        $.ajax({
            ...
            data: {
                "user_ids": userId.includes(',') ? userId.split(',') : [userId], // Handles single or multiple group IDs safely
                "title": title,
                "body": body,
                "admin_id": "<?= $_SESSION['memid'] ?? 1 ?>"
            },
