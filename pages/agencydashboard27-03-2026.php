...
// --- Lively Animated Car & Route Line ---
function animateCar(coords) {
    ...
    // Create an empty blue line that we will fill dynamically
    animatedLineObj = L.polyline([], { color: '#0d6efd', weight: 5, opacity: 0.8 }).addTo(driversMap);
    ...
    animatedCarMarker = L.marker([coords[0].lat, coords[0].lng], { icon: carIcon, zIndexOffset: 1000 }).addTo(driversMap);
    ...
    carAnimationTimer = setInterval(() => {
        if (i < coords.length) {
            ...
            // Draw line up to the car's current position
            ...
            ${job.assigned_driver ? `
                <div class="col-12">
                    <div class="d-flex align-items-center mb-1">
                        <i class="fa fa-user text-primary" style="width:16px;"></i>
                        <span class="d-inline-flex align-items-center gap-1">
                            <a href="#" class="driver-link fw-bold" data-driver="${job.assigned_driver.driver_id}">${job.assigned_driver.name}</a>
                            ${!isInactive ? `<i class="fa fa-times-circle text-danger cursor-pointer remove-driver d-none" title="Remove Driver" data-driver="1" data-job="${jobId}"></i>` : ''}
                        </span>
                    </div>
                </div>
                ` : ''}
