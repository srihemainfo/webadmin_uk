...
<div class="main-content app-content mt-0">
    <div class="side-app">
        <div class="main-container container-fluid mt-4">
            ...
            <div class="d-flex flex-wrap gap-2 mb-4" id="carpoolRoleTabs">
                <button class="btn bg-white shadow-sm fw-bold carpool-role-tab active px-4 py-3"
    data-role="customer"
    data-color="#fd7e14"
    style="color: #fd7e14; border-radius: 6px; border: 1px solid #fd7e14; border-bottom: 3px solid #fd7e14;">
    Customer Poolings
</button>
                <button class="btn bg-white shadow-sm border-0 fw-bold text-muted carpool-role-tab px-4 py-3" data-role="driver" data-color="#198754" style="border-radius: 6px;">
                    Driver Poolings
                </button>
            </div>

            <div class="d-flex flex-wrap gap-2 mb-3" id="carpoolStatusTabs">
                <button class="btn bg-white shadow-sm fw-bold carpool-status-tab active px-4 py-3" data-tab="current" data-color="#0d6efd" style="color: #0d6efd; border-radius: 6px; border: 1px solid #0d6efd; border-bottom: 3px solid #0d6efd;">
                    Current Pools
                </button>
                <button class="btn bg-white shadow-sm border-0 fw-bold text-muted carpool-status-tab px-4 py-3" data-tab="scheduled" data-color="#6f42c1" style="border-radius: 6px;">
                    Scheduled Pools
                </button>
                <button class="btn bg-white shadow-sm border-0 fw-bold text-muted carpool-status-tab px-4 py-3 driver-only-tab" data-tab="started" data-color="#0dcaf0" style="border-radius: 6px; display: none;">
                    Started Pools
                </button>
                <button class="btn bg-white shadow-sm border-0 fw-bold text-muted carpool-status-tab px-4 py-3" data-tab="completed" data-color="#198754" style="border-radius: 6px;">
                    Completed Pools
                </button>
                <button class="btn bg-white shadow-sm border-0 fw-bold text-muted carpool-status-tab px-4 py-3" data-tab="expired" data-color="#fd7e14" style="border-radius: 6px;">
                    Expired Pools
                </button>
                <button class="btn bg-white shadow-sm border-0 fw-bold text-muted carpool-status-tab px-4 py-3" data-tab="cancelled" data-color="#dc3545" style="border-radius: 6px;">
                    Cancelled Pools
                </button>
            </div>
                    ...
                    <div class="row g-3 align-items-center">
                        ...
                        <div class="col-md-10">
                            <div class="row g-2 justify-content-end align-items-center">
                                ...
                                <div class="col-md-2" id="scheduleStatusWrapper" style="display:none;">
                                    <select id="carpoolScheduleStatusFilter" class="form-select form-select-sm shadow-none fw-semibold text-primary border-light" style="background-color: #f8f9fa;">
                                ...
                                <div class="col-md-3">
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="text" id="searchCarpool" class="form-control border-start-0 shadow-none bg-white" placeholder="Search job...">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    $(document).ready(function() {
        ...
        if ($.fn.daterangepicker) {
            ...
            $('#carpoolDateRange').on('apply.daterangepicker', function(ev, picker) {
                ...
                loadCarpoolJobs(carpoolTabType);
            });
        ...
        $('.carpool-role-tab').on('click', function(e) {
            e.preventDefault();
            
            $('.carpool-role-tab')
                .removeClass('active')
                .addClass('text-muted border-0')
                .css({
            ...
            let tabColor = $(this).data('color');
            
            $(this)
                .addClass('active')
                .removeClass('text-muted border-0')
                .css({
            ...
            carpoolRoleType = $(this).data('role');
            
            if (carpoolRoleType === 'driver') {
                $('.driver-only-tab').show();
            } else {
                $('.driver-only-tab').hide();
                if (carpoolTabType === 'started') {
                    $('.carpool-status-tab[data-tab="current"]').trigger('click');
                    return;
                }
            }
        ...
        $('.carpool-status-tab').on('click', function(e) {
            ...
            if (carpoolTabType === 'scheduled') {
                $('#scheduleStatusWrapper').show();
                $('#carpoolFilterType').val('public').prop('disabled', true);
                $('#carpoolDateFilterType').val('created_at').prop('disabled', true);
            } else {
                $('#scheduleStatusWrapper').hide();
                $('#carpoolFilterType').prop('disabled', false);
                $('#carpoolDateFilterType').prop('disabled', false);
            }
        ...
        function loadCarpoolJobs(tabType) {
            if (carpoolXHR && carpoolXHR.readyState !== 4) {
                carpoolXHR.abort();
            }
            ...
            const requestMethod = (tabType === 'scheduled') ? 'carpoolScheduledList' : 'carpoolJobList';
            ...
            carpoolXHR = $.ajax({
                ...
                success: function(res) {
                    if (res.status && res.data) {
                        ...
                        renderCarpoolList(typeFilter, tabType);
                    } else {
        ...
        function renderCarpoolList(typeFilter, tabType) {
            ...
            if (tabType === 'scheduled') {
                const statusFilter = parseInt($('#carpoolScheduleStatusFilter').val());
                jobsToRender = jobsToRender.filter(job => parseInt(job.status) === statusFilter);
            }

            const publicCount = jobsToRender.filter(job => job.poolType === 'public').length;
            const privateCount = jobsToRender.filter(job => job.poolType === 'private').length;
            
            $('#publicPoolCount').text(publicCount + ' Public Pools');
            $('#privatePoolCount').text(privateCount + ' Private Pools');
            $('#totalPoolCount').text('Total Jobs: ' + jobsToRender.length);
            ...
            jobsToRender.forEach(job => {
                html += generateCarpoolCard(job, tabType);
            });

            $list.html(html);
        }

        $(document).on('change', '#carpoolScheduleStatusFilter', function() {
            renderCarpoolList($('#carpoolFilterType').val(), carpoolTabType);
        });

        function generateCarpoolCard(job, tabType) {
            const isScheduled = (tabType === 'scheduled');
            ...
            const userRoleBadge = (job.global_type === 'dr_carpool') 
                ? '<span class="badge shadow-sm ms-2" style="background:#198754;color:#fff;font-size:9px;padding:3px 6px;">Driver</span>' 
                : '<span class="badge shadow-sm ms-2" style="background:#0d6efd;color:#fff;font-size:9px;padding:3px 6px;">Customer</span>';
            ...
            if (tabType === 'cancelled' && job.cancellation_reason && job.cancellation_reason.trim() !== '') {
                cancellationReasonHtml = `
                <div class="mt-3 p-2 bg-white shadow-sm w-100">
                    <div class="text-danger fw-bold mb-1" style="font-size: 10px; text-transform: uppercase;">
                        <i class="fa fa-times-circle me-1"></i> Cancellation Reason
                    </div>
                    <div class="text-dark fw-semibold" style="font-size: 12px; word-break: break-word;">
            ...
            if (isScheduled) {
                const freqType = job.frequency_type ? job.frequency_type.toUpperCase() : 'N/A';
                const lastGenAt = job.last_generated_at ? new Date(job.last_generated_at.replace(' ', 'T')).toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true }) : 'Not Generated Yet';
                const lastGenJobNo = job.last_generated_job_no || 'None';
                ...
                <div class="col-md-4 p-3 border-end bg-light d-flex flex-column" style="max-height: 280px;">
                    <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
                        <h6 class="text-dark fw-bold mb-0" style="font-size: 12px; text-transform: uppercase;">
                            <i class="fa fa-sync-alt me-1 text-primary"></i> Routine Details
                        </h6>
                    </div>
                    <div class="flex-grow-1" style="overflow-y: auto; padding-right: 5px;">
                        <div class="mb-3 p-2 bg-white rounded-3 border border-light shadow-sm">
                            <div class="text-muted mb-1" style="font-size: 10px; text-transform: uppercase; font-weight: 700;">Frequency Type</div>
                            <div class="fw-bold text-primary" style="font-size: 13px;">${freqType}</div>
                        </div>
                    </div>
                </div>`;
            } else {
            ...
            <div class="col-12 mb-3">
                <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
                    <div class="row g-0">
                        <div class="col-md-3 bg-light p-3 border-end d-flex flex-column justify-content-center align-items-start">
                            ...
                            <div class="text-muted w-100" style="font-size: 11px;">
