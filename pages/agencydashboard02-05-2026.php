...
<script>
   ...
   function highlightBiddersOnMap(jobNo, skipZoom = false) {
    ...
    // FIX 1: Remind the global tracker what job we are looking at!
    window.currentRouteJobNo = jobNo;
    
    // Show the filter dropdown when a route is being viewed
    ...
    // Check what the user wants to see from the dropdown
    ...
    db.collection('<?= rtrim(FIREBASE_COLLECTION, '/') ?>').doc(jobNo).get().then(doc => {
        ...
        // Track which bidders we successfully found on the map
        let foundBidderIds = new Set();

        // Loop 1: Handle existing markers in memory
        Object.values(driverMarkers).forEach(m => {
            ...
            if (!isBidder) {
                ...
                // IT IS A BIDDER - Track that we found them
        ...
        // FIX 2: LOOP TO FETCH ANY MISSING BIDDERS!
                    ...
                    success: function(res) {
                        if (res.status && res.data) {
                            ...
                            if (lat && lng && typeof driversMap !== 'undefined') {
                                ...
                                // Wait a moment for popup HTML to load, then append bid text
                                setTimeout(() => {
                                ...
                                // Re-adjust map to fit the newly found missing driver
                                if (!skipZoom && bounds.isValid()) {
                                    driversMap.fitBounds(bounds, { padding: [50, 50], maxZoom: 13 });
                                }
                            }
                        }
                    }
        ...
        setTimeout(() => {
            ...
            const executeRouteDrawing = (fLat, fLng, tLat, tLng) => {
                ...
                if (typeof L.Routing === 'undefined') {
                    ...
                    // NOTE: The logic to only show bidders needs to happen inside this function
...
`;
    ...
    <div class="card mb-2 shadow-sm border-0">
        ...
        function loadWebsiteBookingsFinal(isSilent = false) {
            if (!isSilent) {
        $('#unassignedJobs').empty();
    ...
    `);
        ...
        setTimeout(() => {
                reloadJobsWithFilter(); // Loads Unassigned, Assigned, Website, Cancelled, Expired, Completed
                loadDriversList();      // Loads Drivers count
            }, 300);
        loadUnassignedJobsFinal();
...
<div class="bid-item mb-2 p-2">
    <div class="d-flex align-items-center gap-2">
        ...
        function reloadJobsWithFilter(isSilent = false) {
            loadWebsiteBookingsFinal(isSilent);
    ...
    $(document).on('click', '.view-cancelled-bids-btn', function(e) {
        ...
        if (bids && Object.keys(bids).length > 0) {
            Object.entries(bids).forEach(([driverId, bidData]) => {
                ...
                // 1. Determine the status and the badge color
            ...
            // 2. Generate the HTML with the dynamic badgeClass
            html += `
            <div class="list-group-item d-flex justify-content-between align-items-center p-3">
                <div>
                    <div class="fw-bold text-dark mb-0" style="font-size: 14px;">
