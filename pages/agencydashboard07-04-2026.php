                                ...
                                <div class="card-body p-2">
                                    <div class="row align-items-center mb-2">
                                        ...
                                        <div class="col-2 px-1">
                                            <select id="appSourceFilter_current" class="form-select form-select-sm text-secondary" style="font-size:12px;">
        ...
        <div class="d-flex justify-content-between align-items-start mb-2">
            <div class="d-flex align-items-center gap-2">
                ...
                <h6 class="mb-0 fw-bold d-flex align-items-center gap-1">
                    <a href="https://console.goride.run/job-bidding?job_id=${job.id}" class="text-dark text-decoration-none" style="font-size:13px;" target="_blank">${job.title}</a>
                    ${sourceBadge}
                ...
                ${job.global_type !== 'schedule' ? `<a href="javascript:void(0)" class="text-info viewPushStatusBtn" title="View Push Delivery Status" data-jobid="${job.id}" data-jobno="${job.job_no || job.title}"><i class="fa fa-bell text-warning" style="font-size: 14px;"></i></a>` : ''}
            </div>
        ...
        <div class="mt-2 row small text-muted">
            <div class="col-6">
                <div class="d-flex align-items-center mb-1">
                    ...
                    ${job.assigned_driver && !isInactive ? `<i class="fa fa-comment-dots text-primary cursor-pointer send-push-icon me-2" title="Send Push Notification" data-userid="${job.user_id}" data-name="${job.name}" style="font-size: 13px;"></i>` : ''}
            ...
            ${(!isInactive && job.source && job.source.toLowerCase() === 'posted' && ['created', 'bidding'].includes(job.job_status)) ? `<div class="col-12"><span class="badge bg-secondary cursor-pointer unassigned-driver-btn" data-job="${job.id}" data-jobno="${job.job_no}">Make a Bid</span></div>` : ''}
            <div class="col-6">
            ...
            ${(job.job_status || '').toLowerCase() === 'cancelled' ? `<div class="col-12 mt-2 text-start"><button type="button" class="btn btn-sm btn-outline-danger py-1 px-2 view-cancel-reason-btn" data-reason="${String(job.cancel_reason || 'No reason provided.').replace(/" /g, '&quot;' )}" data-doc="${job.cancel_doc ? job.cancel_doc : ''}"><i class="fa fa-info-circle me-1"></i> View Reason</button></div>` : ''}
            ${(job.feedback_rating || (isExpired === true && (job.job_status === 'accept' || job.job_status === 'accepted'))) ? `<div class="col-12 mt-2 text-start"><button type="button" class="btn btn-sm btn-outline-warning py-1 px-2 view-feedback-btn text-dark fw-semibold" data-rating="${job.feedback_rating || 0}" data-review="${String(job.feedback_review || 'No text review provided.').replace(/" /g, '&quot;' )}"><i class="fa fa-star text-warning me-1"></i> View Feedback</button></div>` : ''}
            <div class="col-6">
...
<div class="bid-item mb-2 p-2">
    ...
    console.log(jobStatus); // <-- Added to get the job status
    ...
    // --- EXISTING CONDITION FOR ACCEPTED JOBS ---
            ...
            success: function(res) {
                if (res.status) {
                    Swal.fire('Cancelled!', '', 'success').then(() => {
                        $('#cancelJobModal').modal('hide');
                        reloadJobsWithFilter(); // Assuming this is defined globally
                    });
                } else {
                    Swal.fire('Error', res.message || 'Cancel failed', 'error');
                }
    ...
    // --- EXISTING LOGIC FOR SCHEDULED JOBS ---
