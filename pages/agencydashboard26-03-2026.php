                                    ...
                                    <div class="card h-100" id="mapCard">
                                        <div class="card-body p-0">
                                            <div id="driversMapWrapper" style="position:relative; height:600px;">
                                                ...
                                                <div style="position: absolute; top: 15px; left: 55px; z-index: 1000; width: 320px;">
                                                    <div class="input-group bg-white" style="border-radius: 24px; box-shadow: 0 2px 6px rgba(0,0,0,0.2); overflow: hidden;">
                                                        <span class="input-group-text bg-white border-0 text-secondary" style="padding-left: 15px; padding-right: 5px;">
                                                            <i class="fa fa-search"></i>
                                                        </span>
                                                        <input type="text" id="mapDriverSearch" class="form-control border-0 shadow-none" placeholder="Search Driver by Name or Phone" autocomplete="off" style="font-size: 13px; border-radius: 0 24px 24px 0; padding-left: 5px;">
                                                    </div>

                                                    <div id="mapDriverSearchResults" class="list-group position-absolute w-100 d-none" style="max-height: 350px; overflow-y: auto; top: 100%; left: 0; background: white; border-radius: 12px; margin-top: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.15); border: 1px solid #f0f0f0;"></div>
                                                </div>
                                                <div id="mapLoader">
...
let routePins = []; 
let animatedLineObj = null; // NEW: Tracks the lively drawn route line
            ...
            if (res.status && res.data) {
                ...
                if (lat && lng && typeof driversMap !== 'undefined' && driversMap) {
                    ...
                    setTimeout(() => {
                        ...
                        mapSearchTargetMarker.bindPopup(`
                            <div style="min-width:210px; font-family: sans-serif;">
                                <div class="d-flex justify-content-between align-items-start mb-1">
                                    <strong style="font-size: 14px; color: #333;">${driver.name}</strong>
                                    <span class="badge bg-success" style="font-size:10px;">${cabType}</span>
                                </div>
                                <div class="d-flex align-items-center justify-content-between mb-2">
                                    <span class="small text-muted"><i class="fa fa-phone text-primary me-1"></i>${driver.mobile || 'N/A'}</span>
                                    ${cleanMobile ? `
                                ...
                                <div class="text-center text-muted mt-2" style="font-size: 10px;">Last seen: ${lastUpdated}</div>
                                <a href="javascript:void(0)" class="btn btn-primary btn-sm w-100 mt-2 py-1 driver-link" data-driver="${driver.user_id || driver.id}" style="font-size:12px; font-weight: 500;">View Profile</a>
                            </div>
                        `).openPopup();
                    }, 300);
...
===================================================== */
    ...
    $(document).off('click', '.jobEdit').on('click', '.jobEdit', function(e) {
        ...
        for (let i = 1; i <= 10; i++) {
            let isSelected = (job.day == i || job.dropoff_date == i) ? 'selected' : '';
