...
`;
    ...
    $(document).ready(function() {
        ...
        function normalizeJob(job, source) {
            ...
            if (job.job_no && String(job.job_no).startsWith('GRD-')) {
                appSourceText = "Driver App";
            } else if (job.user_details && job.user_details !== 'null' && job.user_details !== '') {
        ...
        function getUnassignedActionButton(job) {
            ...
            if (job.source === 'posted' && ['created', 'bidding', 'schedule'].includes(job.job_status)) {
                let buttons = '';
                if (job.job_status !== 'schedule') {
                    buttons += `<button class="btn btn-sm btn-warning d-flex justify-content-center align-items-center bidBtn text-dark ms-2 px-2 position-relative" data-job="${job.id}"> <i class="fa fa-gavel me-1 text-dark"></i> Bids ${job.bid_count > 0 ? `<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">${job.bid_count}</span>` : ''} </button>`;
                }
                if (job.sch_status_data && job.sch_status_data.length > 0) {
                    buttons += `
    <button class="btn btn-sm btn-info viewScheduledRequestsBtn d-flex justify-content-center align-items-center ms-2 text-white px-2 py-0 ${job.job_status !== 'schedule' ? 'ms-2' : ''} position-relative" 
      ...
      <div class="card-body p-3 ${job.global_type == 'open' && !isInactive ? 'card-open' : ''}">
        <div class="d-flex justify-content-between align-items-start mb-2">
         ...
         <h6 class="mb-0 fw-bold d-flex align-items-center gap-1">
            <a href="https://console.goride.run/job-bidding?job_id=${job.id}" class="text-dark text-decoration-none" style="font-size:13px;" target="_blank">
                ${job.title}
            </a>
            ${job.job_status === 'schedule' 
                    ? `<span class="badge bg-primary text-white" style="font-size: 10px;">Scheduled</span>` 
                    : `<span class="badge bg-success text-white" style="font-size: 10px;">${job.app_source}</span>`
                }
        ...
        <a href="javascript:void(0)" 
           class="text-success viewRouteBtn" 
        ...
        <a href="javascript:void(0)" 
           class="text-info viewPushStatusBtn" 
           title="View Push Delivery Status"
           data-jobid="${job.id}"
           data-jobno="${job.job_no || job.title}">
        ...
        <div class="mt-2 row small text-muted">
          <div class="col-6">
            <div class="d-flex align-items-center mb-1">
                ...
                ` : ''}
                ${(showEdit && !isInactive && job.global_type !== 'schedule') ? `
                    ...
                    let match = selectedFields.every(field => {
                        ...
                        const isFilled = (val) => {
                            ...
                            if (typeof val === "string") {
                                val = val.trim();
                                if (val === "" || val.toLowerCase() === "null" || val === "N/A" || val === "-") return false;
                            }
                            return true;
                        };
...
<div class="bid-item mb-2 p-2">
    <div class="d-flex align-items-center gap-2">
        ...
        $(document).on('click', '#confirmCancelJobBtn', function() {
            ...
            if (jobType === 'schedule' || jobType === 'scheduled') {
                $.ajax({
    ...
    $(document).off('click', '.jobEdit').on('click', '.jobEdit', function(e) {
        ...
        const availableCabTypes = <?php
                                    $cabArr = [];
                                    $cab_query = mysqli_query($con, "SELECT name FROM cab_types ORDER BY name ASC");
                                    if ($cab_query) {
                                        while ($cab = mysqli_fetch_assoc($cab_query)) {
                                            $cabArr[] = $cab['name'];
                                        }
                                    }
                                    echo json_encode($cabArr);
                                    ?>;
        let cabTypeOptions = '<option value="">Select Car</option>';
        let currentCarType = String(job.car_type || '').toLowerCase().trim();
        availableCabTypes.forEach(function(cabName) {
            let isSelected = (cabName.toLowerCase() === currentCarType) ? 'selected' : '';
            if (!isSelected && currentCarType !== '') {
                if (currentCarType.includes('mini') && cabName.toLowerCase().includes('mini')) isSelected = 'selected';
                else if (currentCarType.includes('four') && cabName.toLowerCase().includes('sedan')) isSelected = 'selected';
                else if (currentCarType.includes('6') && cabName.toLowerCase().includes('6')) isSelected = 'selected';
                else if (currentCarType.includes('7') && cabName.toLowerCase().includes('7')) isSelected = 'selected';
            }
            ...
            <div class="border p-3 mb-3">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" id="editTotalFare" class="form-control fw-bold" style="color: #14b8a6;" value="${job.fare || ''}">
                    </div>
