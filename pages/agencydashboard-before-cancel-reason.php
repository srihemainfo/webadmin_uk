...
if (isset($_POST['method']) && $_POST['method'] === 'fetch_job_logs') {
    if (ob_get_length()) ob_clean();
    header('Content-Type: application/json');
    $jobId = (int)$_POST['job_id'];

    $logs = [];
    // Joins with user_register to get the Admin's name
...
.car-3d-sprite {
    position: relative;
    z-index: 2;
    filter: drop-shadow(0px 4px 4px rgba(0,0,0,0.3)); /* Adds 3D depth */
    display: block;
}

/* Subtle Shadow that stays on the road */
.car-shadow {
    position: absolute;
    width: 20px;
    height: 35px;
...
<div class="main-content app-content mt-0">
    ...
    <div id="historyControls" class="d-none bg-white shadow-sm" style="border-radius: 6px; box-shadow: 0 2px 6px rgba(0,0,0,0.15); overflow: hidden; display: flex; align-items: center;">
        <div class="input-group" style="height: 38px;">
            <span class="input-group-text bg-white border-0 text-primary">
                <i class="fa fa-calendar-alt"></i>
            </span>
            <input type="text" id="historyDateRange" class="form-control border-0 shadow-none fw-semibold h-100" placeholder="Select Date & Time Range" style="font-size: 12px; width: 250px; cursor: text; background: #fff; padding-left: 0; outline: none; box-shadow: none;">
        </div>
    </div>
...
let historyPolyline = null; // NEW: For historical path drawing
let historyMarkers = [];    // NEW: For historical pins

// --- Master Map Cleaner (Runs on tab change or new search) ---
function clearMapOverlays(fullClear = false) {
    if (typeof driversMap === 'undefined' || !driversMap) return;
    
    if (currentRouteControl) {
        try { driversMap.removeControl(currentRouteControl); } catch(err){}
    ...
    // Only hide the date picker if we are fully wiping the map (e.g. changing tabs)
    if (fullClear) {
        $('#historyControls').addClass('d-none');
        window.currentDriverId = null;
    }
    
    resetDriverIcons();
}
// Wipe map clean when switching tabs
$(document).on('click', '.nav-link', function() {
    clearMapOverlays();
});

// Automatically wipe the map clean when switching tabs
$(document).on('click', '.nav-link', function() {
    clearMapOverlays();
});

// --- 2. Helper to reset driver pins back to default blue ---
    ...
    Object.values(driverMarkers).forEach(marker => {
        if (marker) {
            ...
            if (typeof content === 'string') {
                ...
                if ($name.length) {
                    $name.css({color: '#333', fontWeight: 'bold'});
...
// --- 3. Helper to fetch bids and auto-zoom ---
function highlightBiddersOnMap(jobNo, skipZoom = false) {
    ...
    db.collection('<?= rtrim(FIREBASE_COLLECTION, '/') ?>').doc(jobNo).get().then(doc => {
        if (doc.exists) {
                ...
                if (targetMarker) {
                    ...
                    if (typeof popupContent === 'string') {
                        ...
                        if ($name.length) {
                            ...
                            let cleanText = $name.text().replace(/\s*-\s*Bid:\s*₹\d+/g, '').trim();
                            $name.text(cleanText); 
                            $name.css({color: '#000', fontWeight: '900'})
            ...
            // ONLY auto-zoom if we aren't currently drawing a route
            if (!skipZoom && hasBidders && typeof driversMap !== 'undefined' && bounds.isValid()) {
                setTimeout(() => { driversMap.fitBounds(bounds, { padding: [50, 50], maxZoom: 13 }); }, 500); 
            }
        }
    }).catch(err => console.error("Error fetching map bids:", err));
}

// --- 4. Main Core: Load Drivers (RESTORES WhatsApp & Dynamic Updates) ---
function loadDriversOnMap() {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if (typeof driversAll === 'undefined') window.driversAll = {};

            res.data.forEach(driver => {
                let status = driver.rm_status == 1 ? 'online' : (driver.rm_status == 2 ? 'busy' : 'offline');
                driversAll[driver.id] = {
                    id: driver.id, user_id: driver.user_id, name: driver.name,
                    mobile: driver.mobile, status: status, fullData: driver
                };
                ...
                let vModel = 'N/A', vSeats = 'N/A', vFuel = 'N/A';
                // Add Cab Type variable
                ...
                // Updated popupContent with WhatsApp icon and Green Cab Badge
                let popupContent = `
                    <div style="min-width:210px; font-family: sans-serif;">
                        <div class="d-flex justify-content-between align-items-start mb-1">
                            <strong style="font-size: 14px; color: #333;" class="driver-name-head">${driver.name}</strong>
                ...
                if (!driverMarkers[driver.id]) {
                    const pulseIcon = L.divIcon({
                        className: '', html: `<div class="pulse-marker"></div>`,
                    ...
                    // Dynamically refresh popup data WITHOUT erasing the bidder red highlight if active
        ...
        error: function() {
            setTimeout(function() { isLoadingDrivers = false; loadDriversOnMap(); }, 3000);
        }
    });
}

// --- 5. Locate Driver (Also clears map now) ---
function showRichDriverPopup(marker, driverId, fallbackName, lastUpdated, fallbackMobile = '') {
    // 1. Immediately show loading spinner inside the popup
    ...
    // 2. ALWAYS fetch full details from DB to prevent N/A (bypassing shallow cache)
    $.ajax({
        url: window.location.origin + "/ajax/service/driverServices.php",
        type: "POST",
        dataType: "json",
        data: { method: 'get_driver_details', driver_id: driverId },
        success: function(res) {
            let data = null;
            
            // If API returns rich data, use it
            if (res.status && res.data && res.data.driver) {
                data = res.data.driver;
            } else {
                // Fallback to active map cache if API fails
            ...
            if (!data) data = { name: fallbackName, mobile: fallbackMobile };

            // Extract variables safely (checking direct table rows first)
            ...
            // Try to parse deep vehicle_details if they exist
        ...
        error: function() {
            // If API completely fails, render basic popup
...
function pinDriverOnMap(driverId, driverName) {
    clearMapOverlays(); // Wipes previous highlights and routes

    $.ajax({
        url: window.location.origin + "/ajax/service/driverServices.php",
        type: "POST", dataType: "json",
        data: { method: 'get_driver_current_location', driver_id: driverId },
        success: function(res) {
            if (res.status && res.data) {
                ...
                if (lat && lng && typeof driversMap !== 'undefined') {
                    setTimeout(() => {
                        ...
                        driversMap.flyTo([lat, lng], 16, { animate: true, duration: 1.5 });
...
$(document).on('click', '.viewRouteBtn', function(e) {
    ...
    setTimeout(() => {
        ...
        clearMapOverlays(); // Wipe previous routes/bidders clean

        // This function handles the actual drawing once coordinates are confirmed
        const executeRouteDrawing = (fLat, fLng, tLat, tLng) => {
            ...
            const createRoutePins = () => {
                const createPin = (lat, lng, color, label) => {
                    ...
                    let pin = L.marker([lat, lng], {icon: icon}).addTo(driversMap).bindPopup(`
            ...
            if (typeof L.Routing === 'undefined') {
                let fallbackBounds = L.latLngBounds([[fLat, fLng], [tLat, tLng]]);
                driversMap.fitBounds(fallbackBounds, { padding: [50, 50] });
                createRoutePins();
                highlightBiddersOnMap(jobNo, true); 
                
                $('#mapLoader').addClass('hide');
                $('#mapLoader .mt-2').text('Loading Live Drivers...'); 
                animateCar([ {lat: fLat, lng: fLng}, {lat: tLat, lng: tLng} ]);
                return;
            }
        ...
        // Ultimate Fallback: OpenStreetMap Geocoding via City Name
        ...
        if (!fromLat || !fromLng || !toLat || !toLng || isNaN(fromLat) || isNaN(toLat)) {
            ...
            if (fromPlaceId && toPlaceId && fromPlaceId !== 'undefined' && toPlaceId !== 'undefined') {
                ...
                $.ajax({
                    url: window.location.origin + "/ajax/service/driverServices.php",
                    type: "POST", dataType: "json",
                    data: { method: 'get_coords_by_place_id', from_place_id: fromPlaceId, to_place_id: toPlaceId },
                    success: function(res) {
                        if (res.status && res.data && res.data.from_lat && res.data.to_lat) {
                            ...
                            // DB Failed -> Ask OpenStreetMap!
                            fallbackToOSMGeocoding();
                        }
                    },
                    error: function() { fallbackToOSMGeocoding(); }
                });
            } else {
                // No IDs -> Ask OpenStreetMap!
                fallbackToOSMGeocoding();
            }
        } else {
            // Valid coordinates found right away in HTML
...
// Helper: Calculate angle between two points to rotate the car
...
function animateCar(coords) {
    ...
    // 1. Professional Blue Route Line
    ...
    // 2. High-Quality 3D Top-Down Car Icon
    // Note: I'm using a professional car sprite URL. 
    // You can replace this src with your own 'car.png' if you have one.
    const carIcon = L.divIcon({
        className: 'moving-car-container',
        html: `
            <div class="car-shadow"></div>
            <img src="https://www.goride.run/assets/images/top-car.png" 
                 class="car-3d-sprite" 
                 id="animatedCarImg" 
                 style="width:40px; transform: rotate(0deg); transition: transform 0.2s linear;">
        `,
        iconSize: [40, 40],
        iconAnchor: [20, 20]
    });
    ...
    let i = 0;
    const stepSize = Math.max(1, Math.floor(coords.length / 150)); // Smoother steps

    carAnimationTimer = setInterval(() => {
        if (i < coords.length - stepSize) {
            const currentPos = coords[i];
            const nextPos = coords[i + stepSize];

            // 3. Calculate Rotation
            const angle = calculateHeading(currentPos, nextPos);
            
            // 4. Update Marker Position and Rotation
            animatedCarMarker.setLatLng([currentPos.lat, currentPos.lng]);
            
            // Apply rotation directly to the image inside the marker
            const carImg = document.getElementById('animatedCarImg');
            if (carImg) {
                carImg.style.transform = `rotate(${angle}deg)`;
            }

            // 5. Extend the path line
            animatedLineObj.setLatLngs(coords.slice(0, i + 1));
            
            i += stepSize;
        } else {
            // Animation Finished: Set to exact final coordinate
            const finalPos = coords[coords.length - 1];
            animatedCarMarker.setLatLng([finalPos.lat, finalPos.lng]);
            animatedLineObj.setLatLngs(coords);
            clearInterval(carAnimationTimer);
        }
    }, 40); // 40ms for smooth 25fps feel
}
    ...
    function safeRefreshScheduleUI(driverId) {
        ...
        // 1. Re-fetch Schedule manually to avoid scope issues
        $.ajax({
        ...
        // 2. Trigger Score Update in background
        $.ajax({
    ...
    $(document).off('click', '#confirmAutoScheduleBtn').on('click', '#confirmAutoScheduleBtn', function(e) {
        ...
        // ... (inside your #confirmAutoScheduleBtn click event) ...
    ...
    /* =====================================================
       SAVE FROM MAIN EDIT JOBS PANEL
    ===================================================== */
    ...
    let allMapDriversCache = null; // Caches the drivers so it loads instantly

    // Render the dropdown list safely (No Images, Just Name & Mobile)
    function renderMapDriverResults(drivers, searchVal, resultsBox) {
        ...
        if (searchVal) {
            ...
            filtered = drivers.filter(d =>
                ...
                (d.mobile && String(d.mobile).toLowerCase().includes(lowerSearch)) // <-- FIXED: Cast to String
            );
        }

        // Limit to 20 items to keep the dropdown rendering fast
    ...
    function fetchMapDrivers(searchVal) {
        ...
        // Use cached list if we already hit the API
        if (allMapDriversCache) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.status && res.data) {
                        allMapDriversCache = res.data; // Cache it
                        renderMapDriverResults(allMapDriversCache, searchVal, resultsBox);
                    } else {
    ...
    // 1. TRIGGER ON CLICK (Shows default list instantly)
    $(document).on('click focus', '#mapDriverSearch', function(e) {
        e.stopPropagation();
        $('#mapDriverSearchResults').removeClass('d-none');
        if ($(this).val().trim() === '') {
            fetchMapDrivers(''); // Fetch initial batch if empty
        }
    });
    ...
    $(document).on('keyup', '#mapDriverSearch', function() {
        ...
        // Slight delay so it doesn't query the database on every single letter
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.status && res.data) {
                ...
                // Grab full details from cache
                let driver = (typeof driversAll !== 'undefined' && driversAll[driverId]) ? driversAll[driverId].fullData : { name: driverName, mobile: res.data.mobile || '' };
                ...
                if (lat && lng && typeof driversMap !== 'undefined' && driversMap) {
                    ...
                    setTimeout(() => {
                        driversMap.flyTo([lat, lng], 16, { duration: 1.5 });
    ...
    /* =====================================================
   UNIFIED TAB NAVIGATION & AUTO-LOAD LISTENER
===================================================== */
...
`;
    ...
    $(document).ready(function() {
        ...
        // REMOVE 'let driversMap;' from here! It is now global.
...
===================================================== */
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status && res.data) {
                            ...
                            // Replaced basic HTML with rich popup
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            // 🔥 FIX: Initialize global cache so bidders can find them instantly
            if (typeof driversAll === 'undefined') window.driversAll = {};

            res.data.forEach(driver => {
                // Populate global array for other functions
                let status = driver.rm_status == 1 ? 'online' : (driver.rm_status == 2 ? 'busy' : 'offline');
                driversAll[driver.id] = {
                    id: driver.id, user_id: driver.user_id, name: driver.name,
                    mobile: driver.mobile, status: status, fullData: driver
...
// Assuming your API returns 'updated_at' or 's_time'. Fallback to empty string if not available.
...
`);
                    ...
                    // 🔥 FIX: Attach IDs directly to the marker so they are never lost!
                    marker.driver_table_id = driver.id;
                    marker.driver_user_id = driver.user_id;
        ...
        function normalizeJob(job, source) {
            ...
            return {
                ...
                from_place_id: job.from_place_id || '', // ADDED
                to_place_id: job.to_place_id || '',     // ADDED
    ...
    <div class="card mb-2 shadow-sm border-0">
      <div class="card-body p-3 ${job.global_type == 'open' && !isInactive ? 'card-open' : ''}">
        <div class="d-flex justify-content-between align-items-start mb-2">
          <div class="d-flex align-items-center gap-2">
           ${!isInactive ? `
                <span class="text-muted cursor-pointer cancelJobBtn" style="font-size:14px" data-jobtype="${job.global_type}" data-user-id="${job.user_id}" data-job="${job.id}" data-job-no="${job.job_no}">
                    <i class="fa fa-times-circle text-danger"></i>
                </span>
            ` : `<span class="text-danger"><i class="fa fa-times-circle"></i></span>`}
                <span class="text-muted mx-1">|</span>
         <h6 class="mb-0 fw-bold d-flex align-items-center gap-2">
            <a href="https://console.goride.run/job-bidding?job_id=${job.id}" class="text-dark text-decoration-none" target="_blank">
                ${job.title}
            </a>
            ${job.job_no && String(job.job_no).startsWith('GRD') && !isInactive ? `
                ${job.confirm_status == 0 || !job.confirm_status ? `
                    <button class="btn btn-sm btn-info confirmGrdBtn" data-jobid="${job.id}" data-jobno="${job.job_no}" data-userid="${job.user_id}" data-jobtype="${job.global_type}" style="font-size: 11px; padding: 2px 6px;">
                        <i class=""></i> Confirm
                    </button>
                ` : ''}
        ...
        <a href="javascript:void(0)" 
           class="text-success ms-2 viewRouteBtn" 
        ...
        <div class="mt-2 row small text-muted">
          <div class="col-6">
            <div class="d-flex align-items-center mb-1">
                <i class="fa fa-user-tie text-dark" style="width:16px;"></i>
                <a href="javascript:void(0)" class="${job.source != 'website' ? 'owner-link' : ''} fw-bold text-dark text-decoration-none me-2" data-owner="${job.user_id}" data-jobtype="${job.global_type}">
                    ${job.name}
                </a>
                
                ${job.assigned_driver && !isInactive ? `
                <i class="fa fa-comment-dots text-primary cursor-pointer send-push-icon me-2" title="Send Push Notification" data-userid="${job.user_id}" data-name="${job.name}" style="font-size: 13px;"></i>
                ` : ''}
                
                ${(showEdit && !isInactive) ? `
                <i class="fa fa-pencil-alt text-warning cursor-pointer jobEdit" data-job="${jobId}" data-fulljob="${safeJobData}" title="Edit Job Details" style="font-size: 13px;"></i>
                <i class="fa fa-history text-info cursor-pointer ms-2 viewJobLogs" data-jobid="${jobId}" data-jobno="${job.job_no}" title="View Edit History" style="font-size: 13px;"></i>
                ` : ''}
            </div>
            <div class="d-flex align-items-center gap-2 mt-1" style="padding-left: 16px;">
                <span class=" fw-bold" style="font-size: 13px;">${job.mobile || 'No Number'}</span>
                ${!isInactive ? `
                <a href="tel:${job.mobile}" class="call-con" title="Call Owner" style="width: 25px; height: 25px; font-size: 12px;"><i class="fa fa-phone"></i></a>
                <a href="https://wa.me/${job.mobile}" target="_blank" class="whatsapp-icon" title="WhatsApp Owner" style="width: 25px; height: 25px; font-size: 14px;"><i class="fab fa-whatsapp"></i></a>
                ` : `
                <span class="call-con bg-light text-secondary border" title="Call Disabled" style="width: 25px; height: 25px; font-size: 12px; cursor: not-allowed; opacity: 0.6;"><i class="fa fa-phone"></i></span>
                <span class="whatsapp-icon bg-light text-secondary border" title="WhatsApp Disabled" style="width: 25px; height: 25px; font-size: 14px; cursor: not-allowed; opacity: 0.6;"><i class="fab fa-whatsapp"></i></span>
                `}
            </div>
          </div>
           <div class="col-6">
            ...
            <div class="col-12">
                <div class="d-flex align-items-center justify-content-between gap-4 mb-1">
                    ...
                    <span><strong>${job.day ? (job.day == 1 ? 'Upto 24 hours' : (String(job.day).match(/day|hour/i) ? job.day : job.day + ' Days')) : (job.dropoff_date == 1 ? 'Upto 24 hours' : (String(job.dropoff_date).match(/day|hour/i) ? job.dropoff_date : job.dropoff_date + ' Days'))}</strong></span>
                ` : ''}
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="d-flex align-items-center"><i class="fa fa-users text-success me-1" style="width:16px;"></i><strong>${job.pass_count}</strong></div>
            ...
            ${job.assigned_driver ? `
                <div class="col-12">
                    <div class="d-flex align-items-center mb-1">
                        <i class="fa fa-user text-primary" style="width:16px;"></i>
                        <span class="d-inline-flex align-items-center gap-2">
                            <a href="#" class="driver-link fw-bold text-dark text-decoration-none" data-driver="${job.assigned_driver.driver_id}">${job.assigned_driver.name}</a>
                            
                            ${!isInactive ? `
                            <i class="fa fa-comment-dots text-primary cursor-pointer send-push-icon" title="Send Push Notification" data-userid="${job.assigned_driver.driver_id}" data-name="${job.assigned_driver.name}" style="font-size: 13px;"></i>
                            
                            <i class="fa fa-times-circle text-danger cursor-pointer remove-driver d-none" title="Remove Driver" data-driver="${job.assigned_driver.driver_id}" data-job="${jobId}"></i>
                            ` : ''}
                        </span>
                    </div>
                </div>
                ` : ''}
            ${(!isInactive && job.source && job.source.toLowerCase() === 'posted' && ['created', 'bidding'].includes(job.job_status)) ? `
                <div class="col-12">
                    <span class="badge bg-secondary cursor-pointer unassigned-driver-btn" data-job="${job.id}" data-jobno="${job.job_no}">Make a Bid</span>
                </div>
            ` : ''}
            <div class="col-6">
                <p class="mb-0"> <i class="fa fa-calendar text-danger me-2"></i>${job.created_date}</p>
                <p class="mb-0">
                    <a href="javascript:void(0)" class="${job.source != 'website' ? 'owner-link' : ''} fw-bold" data-owner="${job.user_id}" data-jobtype="${job.global_type}"><i class="fa fa-user-tie text-info"></i> Booked By ${job.name}</a>
                </p>
            </div>
            <div class="col-6">
                <div class="d-flex align-items-center justify-content-end">
                    <div class="text-end d-flex gap-2 align-items-center">
                        ${(job.job_no && String(job.job_no).startsWith('GRD') && (!job.base_fare || Number(job.base_fare) === 0)) ? '' : `<div class="text-muted">Base: ${job.base_fare === 'Included' ? 'Included' : '₹' + (job.base_fare || 0)}</div>`}
                        ${(job.job_no && String(job.job_no).startsWith('GRD') && (!job.toll_fare || Number(job.toll_fare) === 0)) ? '' : `<div class="text-muted">Toll: ${job.toll_fare === 'Included' ? 'Included' : '₹' + (job.toll_fare || 0)}</div>`}
        ...
        function loadWebsiteBookingsFinal() {
            ...
            function render() {
                ...
                if (jobsArray.length === 0) {
                    $('#websiteJobs').html(`<div class="alert alert-info text-center mt-3"><i class="fa fa-info-circle me-2"></i>No data found</div>`);
                } else {
                    jobsArray.forEach(job => {
                        $('#websiteJobs').append(renderJobCard(job, job.id, false, true)); // true = showEdit
                    });
                }
        ...
        function loadUnassignedJobsFinal() {
            ...
            function render() {
                $('#unassignedJobs').empty();
                const sortedJobs = Object.entries(unassignedJobs).sort(([idA], [idB]) => Number(idB) - Number(idA));
                if (sortedJobs.length === 0) {
                    $('#unassignedJobs').html(`<div class="alert alert-info text-center mt-3"><i class="fa fa-info-circle me-2"></i>No data found</div>`);
                } else {
        ...
        /* =====================================================
   CANCELLED JOBS - DATE FILTER LOGIC
===================================================== */
        // Global variables for Cancelled Jobs date filter (Default to Today)
        let cancelledStartDate = moment().format('YYYY-MM-DD');
        let cancelledEndDate = moment().format('YYYY-MM-DD');

        $(document).ready(function() {
            // Initialize Date Picker for Cancelled Jobs
            $('#cancelledDateRange').daterangepicker({
                ...
                ranges: {
                    ...
                    'All Time': [moment('2020-01-01'), moment().add(10, 'years')] // No limit for custom ranges
                }
            }, function(start, end, label) {
                cancelledStartDate = start.format('YYYY-MM-DD');
                cancelledEndDate = end.format('YYYY-MM-DD');
                loadCancelledJobsFinal(); // Trigger API fetch on date change
            });

            // Trigger reload when filter type (Created vs Pickup) changes
...
===================================================== */
        ...
        $(document).ready(function() {
            ...
            $('#expiredDateRange').daterangepicker({
                ...
                loadExpiredJobsFinal(); // Trigger API fetch on date change
            });
            ...
            // Initialize Date Picker for Completed Jobs
        ...
        function loadExpiredJobsFinal() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    <div class="alert alert text-center mt-3">
                        <i class="fa fa-info-circle me-2"></i>No expired jobs available for selected date
                    </div>
                `);
                    } else {
                        $('#expiredJobsList').empty();
        ...
        function loadCompletedJobsFinal() {
            ...
            $.ajax({
                url: window.location.origin + "/ajax/service/dashBoard_Services.php", // Adjust API route if needed
                type: "POST",
                dataType: "json",
                data: {
                    method: 'jobList_New', // Reusing your existing list method
                    jobStatus: 'completed', // New custom status identifier
                ...
                success: function(res) {
                    ...
                    const jobsArray = Object.values(completedJobsObj).sort((a, b) => b.id - a.id);

                    if (jobsArray.length === 0) {
                        $('#completedJobsList').html(`
                    <div class="alert alert-success text-center mt-3">
                        <i class="fa fa-info-circle me-2"></i>No completed jobs available for the selected dates
                    </div>
                `);
                    } else {
                        $('#completedJobsList').empty();
                        jobsArray.forEach(job => {
                            // Pass true to `isExpired/isInactive` to hide Edit/Cancel buttons for finished jobs
...
===================================================== */
        $(document).on('click', '.createJobBtn', function() {
            ...
            // Toggle Buttons (Show Create, Hide Edit)
        ...
        $(document).on('click', '#getDistanceBtn', function() {
            ...
            // ✅ FIXED: Match the actual dropdown values
            let apiCarKey = 'four_seater'; // Default
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // ✅ FIXED: Safe Fallback. 
                    // If the DB doesn't have a 7-seater price for this specific route, 
                    // grab the 4-seater data so the distance and duration still load perfectly!
                    if (!cab) {
                        const availableKeys = Object.keys(response.data);
                        if (availableKeys.length > 0) {
                            cab = response.data[availableKeys[0]]; // Grab whatever car is available
                        } else {
                    ...
                    // Add both Base Fare and Toll Fare together safely
                    const baseAmount = parseInt(cab.fare) || 0;
                    const tollAmount = parseInt(cab.toll_fare) || 0;
                    const total = baseAmount + tollAmount;
        ...
        $(document).on('click', '#confirmBookingBtn', function() {
            ...
            // ✅ FIXED: Pass the exact value from the dropdown ('seven_seater', 'six_seater', etc.)
            let apiCabType = carType;
            // ================= CONFIRMATION POPUP =================
            Swal.fire({
...
<div class="bid-item mb-2 p-2">
    <div class="d-flex align-items-center gap-2">
        ...
        $(document).on('click', '.manageBidBtn', function() {
            const $btn = $(this);
            const driverId = $btn.data('driver'); // driver.user_id
            const jobId = $btn.data('job'); // job ID

            // Get Job No depending on which panel we are viewing
            ...
            // Handle visual loading state (Pencil icon vs Full button)
            ...
            db.collection('<?= rtrim(FIREBASE_COLLECTION, '/') ?>')
                ...
                .then(doc => {
                    ...
                    // Update Modal Titles for Editing
        ...
        /* =====================================================
           MAKE A NEW BID (Show Both Amount & Remark)
        ===================================================== */
        /* =====================================================
   MAKE A NEW BID (Show Both Amount & Remark)
===================================================== */
        $(document).on('click', '.makeBidBtn', function() {
            ...
            // 1. ENSURE Amount Field is Visible for NEW bids
            $('#bidAmount').closest('.mb-3').removeClass('d-none');

            // 2. Set Modal Titles for New Bid
            $('#createBidModal .modal-title').html('<i class="fa fa-gavel me-2 text-warning"></i> Create Bid');
            $('.bid-btn-text').html('<i class="fa fa-paper-plane me-1"></i> Bid');

            $('#createBidModal').modal('show');
        });

        /* =====================================================
           MANAGE BID / EDIT REMARK (Dynamic Fields & Fix Loading)
        ===================================================== */
        $(document).on('click', '.manageBidBtn', function() {
            const $btn = $(this);
            ...
            // The Pencil is an <i> tag, the Manage Bid is a <button> tag. This is bulletproof.
            ...
            db.collection('<?= rtrim(FIREBASE_COLLECTION, '/') ?>')
                ...
                .then(doc => {
                    if (doc.exists) {
                        ...
                        if (bid) {
                            // Pre-fill hidden amount so it saves correctly even if hidden
                            $('#bidAmount').val(bid.amount);
                            $('#bidRemark').val(bid.remark || '');
                        } else {
                            $('#bidAmount').val('');
                    ...
                    // --- DYNAMIC UI LOGIC based on Trigger ---
                    if (isPencilIcon) {
                        // Pencil Clicked -> Hide the Bid Amount container
                        ...
                        // Manage Bid Button Clicked -> Show the Bid Amount container
                        $('#bidAmount').closest('.mb-3').removeClass('d-none');
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.status) {
                        ...
                        // Change wording based on whether it was a new bid or an update
                        ...
                        // Update the button on the Available Drivers tab (if visible)
                        ...
                        // We don't need to manually reload the Incoming Bids panel because your 
                        // Firebase onSnapshot listener will automatically detect the change and re-render the HTML!
                    } else {
        ...
        function reloadJobsWithFilter() {
            ...
            loadUnassignedJobsFinal();
            loadAssignedJobsFinal();
            loadCancelledJobsFinal(); // ✅ Fixed: Added Cancelled Jobs!
            loadExpiredJobsFinal();
            loadCompletedJobsFinal();
        }
    ...
    <div class="schedule-route-box border  p-2 mb-3 bg-white shadow-sm">
        ...
        // Helper to get the exact current local time in "YYYY-MM-DDTHH:MM" format
    ...
    <div class="row g-2 mb-2 align-items-center schedule-date-row">
        ...
        // Ensure strict validation even if they bypass the min attribute in the picker
        ...
        // --- REMOVE ENTIRE ROUTE BOX WITH SWEETALERT ---
        $(document).off('click', '.remove-route-box').on('click', '.remove-route-box', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Remove instantly to avoid DOM state glitches
                    $box.remove();
                    toast1('Route removed successfully.', 'success');
                    autoSaveSchedulesSilently(); // Sync with backend
                }
            });
        });

        // --- REMOVE SPECIFIC DATE ROW WITH SWEETALERT ---
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.status) {
                        ...
                        // Do NOT call loadDriverSchedule() here, because it will wipe the DOM while the user is still editing!
    ...
    ===================================================== */
    /* =====================================================
   EDIT JOB CLICK (Dynamic Prefill)
===================================================== */
    /* =====================================================
       EDIT JOB CLICK (Dynamic Prefill)
    ===================================================== */
    /* =====================================================
       EDIT JOB CLICK (Dynamic Prefill - FIX DURATION)
    ===================================================== */
    /* =====================================================
       EDIT JOB CLICK (Dynamic Prefill)
    ===================================================== */
    $(document).off('click', '.jobEdit').on('click', '.jobEdit', function(e) {
        ...
        // Pass the job id securely via data attribute AND attr
        $('#updateJobBtn').removeClass('d-none').data('jobid', job.id).attr('data-jobid', job.id);

        let formattedPickup = job.pickup_date ? job.pickup_date.replace(' ', 'T').substring(0, 16) : "";
        let isRound = job.job_type === 'roundtrip' || job.job_type === 'round_trip';

        let carVal = 'four_seater';
        if (job.car_type) {
            let ct = String(job.car_type).toLowerCase();
            if (ct.includes('mini')) carVal = 'mini';
            else if (ct.includes('6')) carVal = 'six_seater';
            else if (ct.includes('7')) carVal = 'seven_seater';
        }

        // Safely extract the exact number even if the database contains text like "1 Day Upto 24 hours"
        ...
        <div class="border p-3 mb-3">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="email" id="custEmail" class="form-control" value="${(job.email && job.email !== 'null') ? job.email : ''}">
                </div>
                <div class="col-md-4">
                    <label class="form-label text-muted small fw-bold">Mobile *</label>
                    <input type="text" id="custMobile" class="form-control" value="${job.mobile || ''}">
                </div>
            </div>
        </div>
        ...
        <div class="border p-3 mb-3">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="carType">
                        <option value="mini" ${carVal === 'mini' ? 'selected' : ''}>Go Mini</option>
                        <option value="four_seater" ${carVal === 'four_seater' ? 'selected' : ''}>Go Sedan</option>
                        <option value="six_seater" ${carVal === 'six_seater' ? 'selected' : ''}>Go 6Seaters</option>
                        <option value="seven_seater" ${carVal === 'seven_seater' ? 'selected' : ''}>Go 7Seaters</option>
                    </select>
                </div>
        ...
        <div class="border p-3 mb-3">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control" id="pickupLocation" value="${job.from_place || ''}">
                </div>
                <div class="col-md-6">
                    <label class="form-label text-muted small fw-bold">To *</label>
                    <input type="text" class="form-control" id="dropLocation" value="${job.to_place || ''}">
                </div>
                <div class="col-md-6">
                    <label class="form-label text-muted small fw-bold">Pickup Date & Time *</label>
                    <input type="datetime-local" class="form-control" id="pickupDateTime" value="${formattedPickup}">
                </div>
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold">Passengers *</label>
                    <input type="number" id="passengers" class="form-control" value="${job.pass_count || 1}">
                </div>
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold">Luggage</label>
                    <input type="number" id="luggage" class="form-control" value="${job.luggage || 0}">
                </div>
            </div>
        </div>

        <div class="border p-3 mb-3">
            <h6 class="text-muted mb-3"><i class="fa fa-rupee-sign me-2"></i>Fare & Distance</h6>
            <div class="row g-3">
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold">Distance (kms) *</label>
                    <input type="number" id="distance" class="form-control" value="${job.distance || ''}">
                </div>
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold">Duration</label>
                    <input type="text" id="editDuration" class="form-control" value="${job.duration || ''}">
                </div>
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold">Base Fare *</label>
                    <input type="number" id="baseFare" class="form-control" value="${job.base_fare && job.base_fare !== 'Included' ? job.base_fare : ''}">
                </div>
                <div class="col-md-3">
                    <label class="form-label text-muted small fw-bold">Toll Fare</label>
                    <input type="number" id="tollFare" class="form-control" value="${job.toll_fare && job.toll_fare !== 'Included' ? job.toll_fare : ''}">
                </div>
                <div class="col-md-12 mt-3">
                    <label class="form-label" style="color: #14b8a6; font-weight: 600;">Total Fare *</label>
                    <input type="number" id="editTotalFare" class="form-control fw-bold" style="color: #14b8a6; width: 200px;" value="${job.fare || ''}">
                </div>
            </div>
        </div>
    </div>
    ...
    $(document).off('click', '#updateJobBtn').on('click', '#updateJobBtn', function() {
        ...
        const finalFare = $('#editTotalFare').val().trim();
        if (!finalFare || finalFare <= 0) {
            markErr('#editTotalFare');
            toast('error', 'Total Fare is mandatory.');
            return;
        }
        ...
        const payload = {
            ...
            base_fare: $('#baseFare').val() || '',
            toll_fare: $('#tollFare').val() || '',
            fare: finalFare,
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status) {
                    ...
                    // 1. Close edit panel immediately
                    $('.backToMap').trigger('click');

                    // 2. Add a tiny delay to ensure the Database has fully committed the save
                    setTimeout(() => {
                        // 3. BULLETPROOF FIX: Trigger the exact event that your tabs already listen to!
                        // This perfectly refreshes whichever sub-tab (Unassigned, Assigned, etc.) is currently open.
            ...
            complete: function() {
                // Ensures the button never gets stuck loading!
                $btn.prop('disabled', false).html('<i class="fa fa-save me-1"></i> Update Job');
            }
        });
    });
    /* =====================================================
       VIEW JOB LOGS (HISTORY ICON CLICK) - FORMAL & PROFESSIONAL
    ===================================================== */
    /* =====================================================
       VIEW JOB LOGS (HISTORY ICON CLICK) - FORMAL UI
    ===================================================== */
    $(document).on('click', '.viewJobLogs', function(e) {
        ...
        $.ajax({
            url: window.location.href.split('?')[0], // Safely post to current URL
            type: "POST",
            dataType: "json",
            data: {
                ...
                if (res.status && res.data && res.data.length > 0) {
                    ...
                    res.data.forEach(log => {
                        ...
                        for (const [field, changes] of Object.entries(details)) {
                            ...
                            // Red text for old value (no strikethrough), Teal/Cyan for new value
                            detailsHtml += `<tr>
    ...
    // Auto-calculate Total Fare if Base/Toll are changed manually
...
<script>
    ...
    // 1. Helper to get the exact local time + 2 HOURS in "YYYY-MM-DDTHH:MM" format
    function getMinDateTime() {
        const now = new Date();
        // Add exactly 2 hours to the current time
        now.setHours(now.getHours() + 2);

        // Adjust for local timezone offset so it outputs correctly in the HTML input
        now.setMinutes(now.getMinutes() - now.getTimezoneOffset());
        return now.toISOString().slice(0, 16);
    }
    ...
    $(document).on('change', '#pickupDateTime', function() {
        ...
        if (selectedTime && selectedTime < getMinDateTime()) {
            $(this).val(""); // Clear the invalid input so the form cannot be submitted
...
<script>
    ...
    window.recalculateDriverScore = function(driverId) { // 1. Trigger the score update in the backend
...
<script>
    // Handle Map Icon click in the Drivers Tab list
    $(document).on('click', '.view-driver-location-tab', function(e) {
        ...
        // 1. Switch to the main Jobs Tab where the map exists
        $('button[data-bs-target="#jobsTab"]').tab('show');

        // 2. Ensure the map UI is showing (closes side panels if open)
        $('.backToMap').trigger('click');

        // 3. Call the existing map pinning function we fixed earlier
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status && res.data) {
                    ...
                    // 3. STRICT CHECK: Ensure it's the Leaflet map, not the HTML element
    ...
    /* =====================================================
       URL HASH TAB NAVIGATION LISTENER
    ===================================================== */
    $(document).ready(function() {
        function activateTabFromHash() {
            let hash = window.location.hash;
            if (hash) {
                // Map the URL hashes to the data-bs-target attributes of your buttons
                const tabMap = {
                    '#unassigned': {
                        mainTab: '#jobsTab',
                        subTab: '#unassigned'
                ...
                if (target) {
                    // 1. Click the main top-level tab first
                    $('button[data-bs-target="' + target.mainTab + '"]').trigger('click');

                    // 2. If it's a nested sub-tab (Unassigned, Assigned, Website), click it after a tiny delay
                    if (target.subTab) {
                        setTimeout(() => {
        ...
        // Run when the page Ffirst loads
        activateTabFromHash();

        // Run if the user clicks a link while already on the Agency Dashboard page
...
<script>
    /* =====================================================
       NEW FEATURE: DRIVER HISTORICAL TRACKING ENGINE
    ===================================================== */
    // Declare globally, but do not initialize until document is ready!
    let historyStartDateTime = null;
    let historyEndDateTime = null;

    $(document).ready(function() {
        // 1. Safe Initialization of Dates
        historyStartDateTime = moment().startOf('day').format('YYYY-MM-DD HH:mm:ss');
        historyEndDateTime = moment().format('YYYY-MM-DD HH:mm:ss');

        // 2. Initialize DateRangePicker with Predefined Ranges
        if($.fn.daterangepicker) {
            $('#historyDateRange').daterangepicker({
                ...
                endDate: moment(),
                maxDate: moment(), // 🔥 PREVENTS SELECTING ANY FUTURE DATES
                locale: {
                    format: 'DD/MM/YY hh:mm A'
                },
                // 🔥 ADDED PREDEFINED FILTERS HERE
                ranges: {
            ...
            // Trigger automatically when user selects a range or clicks "Apply"
            $('#historyDateRange').on('apply.daterangepicker', function(ev, picker) {
                // Instantly update UI Input Text
        ...
        // 3. Show History Controls whenever a driver is selected
        ...
        // Failsafe: Show controls if a map popup is opened
        let mapCheckTimer = setInterval(() => {
             if (typeof driversMap !== 'undefined' && driversMap !== null) {
                 driversMap.on('popupopen', function() {
                     $('#historyControls').removeClass('d-none');
                 });
                 // WE REMOVED popupclose HERE SO IT DOESN'T DISAPPEAR
                 clearInterval(mapCheckTimer);
             }
        }, 1000);
    });

    // 4. The Unified Fetch Function (Runs automatically when Apply/Range is clicked)
    function fetchDriverHistoryAutomatically() {
        ...
        if (!driverId) {
            toast('error', 'Please select a driver from the map or list first.');
            return;
        }

        let fallbackName = $('.leaflet-popup-content .driver-name-head').text() || $('#mapDriverSearch').val() || 'Unknown Driver';

        // Turn the calendar icon into a spinner so we don't erase the date text!
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.status && res.data && res.data.length > 0) {
                    ...
                    $.ajax({
                        ...
                        success: function(driverRes) {
                            ...
                            if (driverRes.status && driverRes.data && driverRes.data.driver) {
                                ...
                                driverData = { name: fallbackName, user_id: driverId, mobile: '' };
                            }
                        ...
                        error: function() {
                            plotDriverHistoryOnMap(res.data, { name: fallbackName, user_id: driverId, mobile: '' });
                        }
                    });
                    ...
                    clearMapOverlays();
                    // Restore original driver pin if history is empty
                    if (window.currentDriverId) {
                        pinDriverOnMap(window.currentDriverId, fallbackName);
                    }
                }
            },
            error: function() {
    ...
    function plotDriverHistoryOnMap(locations, data) {
        ...
        // 1. Clear ALL existing map elements
        clearMapOverlays();
        ...
        // 3. Initialize ALL variables safely outside the loop to prevent ReferenceErrors
        ...
        // Extract Driver and Vehicle Info Securely
        ...
        // 4. Drop the Numbered Markers (No clustering, No lines)
        sampled.forEach((loc, index) => {
            let lat = parseFloat(loc.lat);
            let lng = parseFloat(loc.lng);
            
            // 🔥 OFFSET JITTER: If points share exact coords (e.g. driver parked), fan them out slightly
            let overlapCount = 0;
            ...
            // Green for Start, Red for End, Blue for middle waypoints
            ...
            // zIndex ensures newer points stay on top of older points
            let marker = L.marker([lat, lng], { icon: numIcon, zIndexOffset: 1000 + index });

            // Small Card - No Waypoint labels, just the date/time
            let popupHtml = `
            ...
            // Add DIRECTLY to map, bypassing the cluster group so they never collapse into a bubble
            marker.addTo(driversMap);
            historyMarkers.push(marker);
            
            // Open popup automatically for the last point
            if (isEnd || sampled.length === 1) {
                setTimeout(() => marker.openPopup(), 500);
            }
        });

        // 5. Zoom map so all points are visible
        if(latLngs.length > 0) {
            let bounds = L.latLngBounds(latLngs);
            // Added maxZoom to prevent zooming in too far if the points are heavily clustered
            driversMap.fitBounds(bounds, { padding: [50, 50], maxZoom: 14 });
            
            // 🔥 Ensure controls remain visible after zoom redraw
            $('#historyControls').removeClass('d-none');
        }
    }
    /* =====================================================
    ...
    $(document).on('click', '#sendPushBtn', function() {
        ...
        $.ajax({
            ...
            data: {
                "user_ids": [userId], // API expects an array
                "title": title,
                "body": body,
                "admin_id": "<?= $_SESSION['memid'] ?? 1 ?>"
            },
            ...
            complete: function() {
                $btn.prop('disabled', false).html('<i class="fa fa-paper-plane me-1"></i> Send');
            }
        });
    });
</script>