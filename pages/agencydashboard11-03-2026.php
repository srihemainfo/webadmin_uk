                ...
                <div class="card shadow-sm border-0 mb-3">
                    <div class="card-body p-2">
                        <div class="d-flex justify-content-between align-items-center mb-2 border-bottom pb-1">
                            ...
                            <button type="button" class="btn btn-info btn-sm" onclick="loadEditLogs()" style="margin-left: 332px;">
                            ...
                            <div class="row gx-2 align-items-center">
                                <div class="col-auto">
                                    <div class="position-relative" style="width: 110px;">
                                        <div id="vehicle-gallery-container" class="shadow-sm border" style="width: 100%; height: 80px; overflow: hidden; background: #f8f9fa; border-radius: 6px;">
                                ...
                                <div class="col" style="min-width: 0;">
                                    <div class="mb-1 text-truncate" style="line-height: 1.2;">
                                        ...
                                        <i class="fa fa-check-circle text-success ms-1" style="font-size: 12px;"></i>
                                    </div>
                    ...
                    <div class="w-100">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center gap-2" style="font-size:14px">
                                ...
                                <div class="d-flex align-items-center">
                                    <i id="sidebar_remarks_icon" class="fa-solid fa-message cursor-pointer me-4" style="color: #6c757d; font-size: 1.4rem;" title="Add/View Remarks"></i>
                                </div>
                            </div>
                            <div class="d-inline-flex align-items-center gap-2">
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (d.user_id) {
                        ...
                        // --- FIXED: Pass dynamic ID and Name directly to the modal ---
...
$('#preview_license').html(`<i class="fa fa-id-badge text-primary me-1"></i> <strong>${d.dl_no || 'N/A'}</strong>${dlTypeHtml} | <strong>${dlExpiryStr}</strong>`);
                    ...
                    if (d.vehicle_details) {
                        try {
                            // Safe parse: handles both stringified JSON or already decoded objects
                            const vehicleData = typeof d.vehicle_details === 'string' ? JSON.parse(d.vehicle_details) : d.vehicle_details;
                            ...
                            // ---> FIXED: Fetch fuel from rc_details first, fallback to vehicle_questions, then root DB column <---
                            // ---> Fetch fuel and keep it uppercase to match your system <---
                            let rawFuel = (vehicleData.rc_details?.response?.vehicle_details?.fuel_type ||
                                vehicleData.vehicle_questions?.fuel_type ||
                                d.fuel_type ||
                                'N/A').toString().trim().toUpperCase();
        ...
        function openDriverEditDrawer(id) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    setText('[data-field="mobile"]', d.mobile);
                    setVal('#edit_mobile', d.mobile);
                    // ---> ADD THIS NEW BLOCK: Update Call & WhatsApp links dynamically
                    if (d.mobile) {
                        let cleanMobile = String(d.mobile).replace(/\D/g, ''); // Remove non-numeric characters
                        // If it's exactly 10 digits, prepend '91' for WhatsApp (India country code)
                    ...
                    // Pre-fill the district dropdown based on the ID from the database
                    if (d.districts_id) {
                        $('#edit_districts_id').val(d.districts_id);
                        // Get the text of the selected option to display in view mode
            ...
            // ---> ADDED: Prefill Edit Mode <---
            setImage('#edit-licenseFront img', d.license_front_image);
            setImage('#edit-licenseBack img', d.license_back_image);
            ...
            // ---> NEW: Set License Type <---
            let licType = d.license_type || 'LMV'; // Fallback to LMV if empty
            ...
            setVal('#edit_license_dob', d.dob);
                    ...
                    // --- STRICT FUEL TYPE MAPPING (ALL CAPS) ---
                    let rawFuel = (vehicleData.rc_details?.response?.vehicle_details?.fuel_type ||
                        vehicleData.vehicle_questions?.fuel_type ||
                        d.fuel_type ||
                        '').toString().trim().toUpperCase();
                    // Map exactly to your new <option value="..."> 
                    let mappedFuel = '';
                    if (rawFuel.includes('PETROL/CNG') || rawFuel.includes('PETROL / CNG')) {
                        mappedFuel = 'PETROL/CNG';
                    } else if (rawFuel === 'PETROL') {
                        mappedFuel = 'PETROL';
                    } else if (rawFuel === 'DIESEL') {
                        mappedFuel = 'DIESEL';
                    } else if (rawFuel === 'CNG') {
                        mappedFuel = 'CNG';
                    }
                    // Set the display text and pre-fill the dropdown perfectly
                    ...
                    /* =========================
                       VEHICLE 6 IMAGES MAPPING
                    ========================== */
                    const vImages = vehicleData.vehicle || {};
                    const placeholder = '/assets/images/placeholder.png'; // Make sure this path exists
                    // Define the 6 image bindings
                    const imgMapping = [{
                    ...
                    // Loop and populate modal
                    imgMapping.forEach(item => {
                        if (item.url) {
                            $(item.preview).attr('src', item.url);
                            $(item.input).val(item.url);
                        } else {
                            $(item.preview).attr('src', placeholder);
                            $(item.input).val('');
                        }
                    });
                    // Set main drawer vehicle preview
                    if (vImages.front_view_image_url) {
                        $('#main_vehicle_preview').attr('src', vImages.front_view_image_url);
                        setImage('.vehicle-photo', vImages.front_view_image_url);
                    } else {
                        $('#main_vehicle_preview').attr('src', '/assets/images/carimage3.png');
                    }
                    // ---> ADDED: Populate the hidden images so the gallery can swipe them <---
                    $('#gallery_boot_image').attr('src', vImages.boot_image_url || '');
                    $('#gallery_extra_image_1').attr('src', vImages.extra_image_1_url || '');
                    $('#gallery_car_top_view_image').attr('src', vImages.car_top_view_image_url || '');
                    $('#gallery_interior_front_image').attr('src', vImages.interior_front_image_url || '');
                    $('#gallery_special_features_image').attr('src', vImages.special_features_image_url || '');
                    // REBUILD the gallery so it catches the new DB images
                    refreshVehicleGallery();
                    /* =========================
                       VEHICLE DOCUMENTS
                    ========================== */
...
<div class="bid-item mb-2 p-2">
    <div class="d-flex align-items-center gap-2">
        ...
        function updateSectionDisplay(section) {
            switch (section) {
                case 'driver-profile':
                    ...
                    if (newMobile) {
                        let cleanNewMobile = String(newMobile).replace(/\D/g, '');
        ...
        $(document).on('click', '.save-section-img[data-section="documents"]', async function() {
            ...
            // ---> FIXED: Helper to check if an image is real (not a default placeholder)
            const getValidUrl = (selector) => {
                let src = $(selector).attr('src');
                if (src && !src.includes('assets/images/') && !src.includes('placeholder')) {
                    return src;
                }
                return null;
            };

            // Grab the newly uploaded URL OR the existing image URL from the screen
            const finalRcFront = rcFrontUrl || getValidUrl('#edit-rcDocumentFront img');
            const finalRcBack = rcBackUrl || getValidUrl('#edit-rcDocumentBack img');
            const finalPuc = pucUrl || getValidUrl('#edit-pucDocument img');
            const finalInsurance = insuranceUrl || getValidUrl('#edit-insuranceDocument img');

            if (!rcNumber || !rcExpiry || !finalRcFront || !finalRcBack) {
                showNotification("RC Front and Back images, RC Number and Expiry are required", 'danger');
                return;
            }
            ...
            try {
                ...
                const rcResponse = await $.ajax({
                    ...
                    data: {
                        user_id: driverId,
                        type: "RC",
                        front_url: finalRcFront, // Use the guaranteed valid URL
                        back_url: finalRcBack,   // Use the guaranteed valid URL
                        expiry: rcExpiry,
                        rc_no: rcNumber
                    }
                });
                ...
                await $.ajax({
                    ...
                    data: {
                        ...
                        rc_back: finalRcBack,
                        puc_image: finalPuc,             // Will save existing or new image perfectly
                        puc_expiry: pucExpiry,
                        insurance_image: finalInsurance, // Will save existing or new image perfectly
                ...
                if (finalRcFront) $('#rcDocumentFront img').attr('src', finalRcFront);
                if (finalRcBack) $('#rcDocumentBack img').attr('src', finalRcBack);
                if (finalPuc) $('#pucDocument img').attr('src', finalPuc);
                if (finalInsurance) $('#insuranceDocument img').attr('src', finalInsurance);
  ...
  <div class="border  p-3 ">
    ...
    //     // Replace your existing vehicleViewer with this:
    // const vehicleViewer = new Viewer(
    //     document.getElementById('vehicle-gallery-container'), {
    //         toolbar: true,
    //         navbar: true, // Shows the thumbnails at the bottom for easy swiping
    //         fullscreen: true,
    //         zIndex: 9999,
    //         filter(image) {
    //             // Only show images in the gallery that actually have a URL
    //             return image.src && image.src.indexOf('placeholder') === -1 && image.src !== window.location.href;
    //         }
    //     }
    // );
    // Function to safely destroy and re-initialize the vehicle gallery
    function refreshVehicleGallery() {
        ...
        // Re-initialize with the newly updated image URLs
        window.vehicleViewer = new Viewer(container, {
            ...
            zIndex: 9999,
            filter(image) {
                // Only show valid images, ignore placeholders and empty srcs
                return image.src && image.src.indexOf('placeholder') === -1 && image.src !== window.location.href;
            }
        });
    }
