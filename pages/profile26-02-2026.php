...
// =======================================================================
// 1. AJAX HANDLER FOR EDIT LOGS (Must be at the very top)
// =======================================================================
if (isset($_POST['method']) && $_POST['method'] === 'fetch_edit_logs') {
    ...
    $editor_ids = [];
    foreach ($edit_log_data as $log) { if (!empty($log['edited_by'])) $editor_ids[] = (int)$log['edited_by']; }
    ...
    if (!empty($editor_ids)) {
        ...
        while ($name_row = mysqli_fetch_assoc($name_query)) { $editor_names[$name_row['id']] = $name_row['name']; }
    }
    // Helper: Friendly Names
    function getFriendlyName($key) {
        $mapping = [
            'name' => 'Full Name', 'mobile' => 'Mobile Number', 'email' => 'Email Address', 'exp' => 'Driver Experience',
            'billing_address' => 'State', 'districts_id' => 'District', 'address_us' => 'Address', 'language' => 'Language',
            'user_role' => 'User Role', 'li_upto' => 'License Expiry', 'dl_expiry' => 'License Expiry',
            'license_type' => 'License Type', 'cab_type' => 'Vehicle Model', 'fuel_types' => 'Fuel Type',
            'seat' => 'Seat Capacity', 'Luggage' => 'Luggage Capacity', 'insurance_upto' => 'Insurance Expiry',
            'rc_upto' => 'RC Expiry', 'puc_upto' => 'PUC Expiry', 'price_per_km' => 'Price/KM',
            'extra_price_per_km' => 'Extra Price/KM', 'price_per_hour' => 'Price/Hour', 'price_per_day' => 'Price/Day',
            'upi_id' => 'UPI ID', 'reviews' => 'Feedback Review', 'per_km' => 'Price Per KM',
            ...
            'remarks' => 'Remarks',
            'agent_id' => 'Assigned Agent', // NEW: Added Agent ID mapping
...
if ($subid3 != 'permission') {
   ...
   // Parse Vehicle Dates safely
   $RcUpto = $vehicleDetails['rc_details']['response']['vehicle_details']['fit_up_to'] ?? '';
   if($RcUpto && strtotime($RcUpto) > 0) $RcUpto = date('Y-m-d', strtotime($RcUpto));
   $pucUpto = $vehicleDetails['rc_details']['response']['permit_details']['pucc_upto'] ?? '';
   if($pucUpto && strtotime($pucUpto) > 0) $pucUpto = date('Y-m-d', strtotime($pucUpto));
   $insuranceUpto = $vehicleDetails['rc_details']['response']['finance_details']['insurance_upto'] ?? '';
   if($insuranceUpto && strtotime($insuranceUpto) > 0) $insuranceUpto = date('Y-m-d', strtotime($insuranceUpto));
   $SeatCapacity = $vehicleDetails['rc_details']['response']['vehicle_details']['seat_capacity'] ?? '';
   $fuelType = $vehicleDetails['rc_details']['response']['vehicle_details']['fuel_type'] ?? '';
   $cabType = $vehicleDetails['rc_details']['response']['vehicle_details']['body_type'] ?? '';
   $makerModel = $vehicleDetails['rc_details']['response']['vehicle_details']['maker_model'] ?? '';
   $carColour = $vehicleDetails['rc_details']['response']['vehicle_details']['color'] ?? '';
   $userInfo = $vehicleDetails['user_info'] ?? [];
   $selectedLanguages = !empty($userInfo['language']) ? explode(',', $userInfo['language']) : [];
   $luggageValue = $userInfo['luggage'] ?? '';
   $roll_id =  select_top_name($con, "user_register", "roll_id", "`id`='$_SESSION[memid]' and `deletes`='0'", "roll_id", "");
   $acc_type = ['savings', 'current'];
   $bank_list = [
      'First Abu Dhabi Bank (FAB)', 'Emirates NBD', 'Abu Dhabi Commercial Bank', 'Dubai Islamic Bank', 'MashreqBank',
      'Abu Dhabi Islamic Bank (ADIB)', 'HSBC Bank Middle East - UAE Operations', 'Union National Bank', 'Commercial Bank of Dubai (CBD)',
      'Emirates Islamic Bank', 'National Bank of Ras Al Khaimah (RAKBANK)', 'Al Hilal Bank', 'Noor Bank', 'Sharjah Islamic Bank',
                  ...
                  <form id="editprofileform">
                     ...
                     <div class="card ">
                        <div class="card-header">
                           ...
                           <button type="button" class="btn btn-info btn-sm" onclick="loadEditLogs()" style="margin-left: 556px; display: inline;
    margin-top: -42px;">
                        ...
                        <div class="card-body">
                           <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-select" id="cab_types" name="cab_type" required title="Please select vehicle type">
                                        ...
                                        <option value="Mini" <?= ($cabType === 'Mini') ? 'selected' : '' ?>>Go Mini</option>
                                        <option value="Go Sedan" <?= ($cabType === 'Go Sedan' || $cabType === 'CAR SEDAN') ? 'selected' : '' ?>>Go Sedan</option>
                                        <option value="Go SUV" <?= ($cabType === 'Go SUV') ? 'selected' : '' ?>>Go SUV</option>
                                        <option value="Go SUV PLUS" <?= ($cabType === 'Go SUV PLUS') ? 'selected' : '' ?>>Go SUV+</option>
                                        <option value="Go Executive" <?= ($cabType === 'Go Executive') ? 'selected' : '' ?>>Go Executive</option>
                                        <option value="Go Tourister" <?= ($cabType === 'Go Tourister') ? 'selected' : '' ?>>Go Tourister</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" value="<?= htmlspecialchars($SeatCapacity ?? '') ?>" name="seat" class="form-control" placeholder="Seat Capacity" required maxlength="2" minlength="1" pattern="^[0-9]{1,2}$" title="Seat capacity must be 1 to 2 digits (max 99)" oninput="validateSeat(this)">
                            </div>
                        </div>
                            <div class="col-md-4">
                                <div class="form-group">
...
function updateDetails() {
    ...
    formdata.forEach(function(field) {
        if (field.name === 'mobile') {
            ...
            // If the field is readonly, use the stored full number
            if (mobileInput.hasAttribute('readonly') && fullMobile) {
                field.value = fullMobile;
            } else {
                // Otherwise, add 91 prefix if it's 10 digits
