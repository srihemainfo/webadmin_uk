...
$(document).ready(function () {
    ...
    // Handle Dynamic Button Inputs Generation
      ...
      columns: [
        ...
        { 
          ...
          render: function (data, type, row) {
              // 1. Render the main text body
              let html = `<div class="table-body-scroll" style="margin-bottom: 5px;">${data ? data : '-'}</div>`;

              // 2. Safely parse and render the buttons if they exist
                  ...
                  try {
                      ...
                      if (parsedJson.type && parsedJson.type !== 'none' && parsedJson.buttons && parsedJson.buttons.length > 0) {
                          ...
                          parsedJson.buttons.forEach(btn => {
                              let icon = '';
                              let btnClass = 'bg-primary'; // Default blue for quick reply
...
$(document).ready(function () {
    ...
    $('#templateForm').submit(function (e) {
      ...
      // Put the compiled JSON into the hidden field so PHP can save it
      ...
      $.post(baseUrl, formData, function (response) {
        ...
        $('#body').val('');
        $('#button_inputs_container').empty(); // Reset button container after submit
        $('#templateTable').DataTable().ajax.reload(null, false);
