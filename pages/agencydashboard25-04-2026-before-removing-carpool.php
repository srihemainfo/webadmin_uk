    ...
    db.collection('<?= rtrim(FIREBASE_COLLECTION, '/') ?>').doc(jobNo).get().then(doc => {
        if (doc.exists) {
            ...
            Object.values(driverMarkers).forEach(m => {
                const isBidder = bidderIds.includes(String(m.driver_user_id)) || bidderIds.includes(String(m.driver_table_id));
                if (!isBidder) {
                    if (typeof markerClusterGroup !== 'undefined' && markerClusterGroup.hasLayer(m)) {
                        markerClusterGroup.removeLayer(m);
                    }
                    if (typeof driversMap !== 'undefined' && driversMap.hasLayer(m)) {
                        driversMap.removeLayer(m);
                    }
                }
            });
            // ---------------------------------------------------------------
