...
<style>
    ...
    #logDateRange { border-radius: 6px; border: 1px solid #ced4da; }
    ...
    <div class="side-app">
        <div class="main-container container-fluid">
            ...
            <div class="page-header d-flex justify-content-between align-items-center mt-3 mb-4">
                <h1 class="page-title m-0 fw-bold">Customer Activity Log</h1>
            </div>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-light p-4">
                    <div class="row w-100 align-items-end">
                        <div class="col-md-4">
                            <label class="mb-2 text-muted" style="font-weight: 600; font-size: 13px;">Filter Demand by Date</label>
                ...
                <div class="card-body p-0 pt-3">
                    <div class="table-responsive">
                        <table id="adsReportTable" class="table table-hover align-middle mb-0" style="width:100%; font-size: 14px;">
                            <thead>
                                <tr>
...
<!-- 2. THEN MOMENT.JS AND DATERANGEPICKER -->
<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/moment@2.29.4/moment.min.js"></script>
<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js"></script>

<!-- 3. THEN DATATABLES & BOOTSTRAP -->
...
$(document).ready(function() {
    ...
    // Server-Side DataTable for Main Report
    let adsTable = $('#adsReportTable').DataTable({
        processing: true,
        serverSide: true, // Switched to true for backend pagination
        pageLength: 10,   // Fixed 10 entries per page
        ajax: {
            url: window.location.origin + "/ajax/service/activity_log_services.php", 
            type: "POST",
            data: function(d) {
                d.method = 'get_location_ad_reports';
        ...
        columns: [
            ...
            { data: "location_name", className: "fw-bold text-primary", orderable: false },
            { data: "district", className: "fw-semibold", orderable: false }, 
            { data: "state", className: "fw-semibold", orderable: false },
            { data: "search_type", className: "text-center", orderable: false }, 
    ...
    $('#adsReportTable tbody').on('click', '.view-details-btn', function() {
        let currentPlaceId = $(this).data('placeid');
        let currentSearchType = $(this).data('searchtype'); 
        
        $('#modalDynamicTitle').html(`Detailed Customer Logs <span class="badge bg-secondary ms-2">Searched as ${currentSearchType.toUpperCase()}</span>`);

        if (detailsTableInstance) {
            detailsTableInstance.destroy();
        }

        // Server-Side DataTable for Inner Modal Details
        detailsTableInstance = $('#detailsTable').DataTable({
            processing: true,
            serverSide: true, // Enables fast loading for 50,000+ rows
            pageLength: 10,   // Set specifically to 10 entries
            ajax: {
                url: window.location.origin + "/ajax/service/activity_log_services.php",
                type: "POST",
                data: function(d) {
                    d.method = 'get_ad_report_details';
                    d.place_id = currentPlaceId;
