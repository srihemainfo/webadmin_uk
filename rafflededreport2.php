 ...
 *      02-01-2023      Devanathan         super Raffle Draw Pre-Winner Pdf Genration
...
// var_dump($_REQUEST['reportType']);die;
...
if ($_REQUEST['reportType'] == 'pdf') {
    ...
    try {
        ...
        $draw = select_query($con, "superraffledraw", "", "`id`= '$winnerid' AND `deletes` = '0' ", "", "");
        ...
        if ($Ticket_lines['nr'] > 0) {
            ...
            foreach ($Ticket_lines['result'] as $key => $value) {
                ...
                if ($start != '' && $end != '') {
                    ...
                    $datefilter1 = "tl.createdon BETWEEN '$formattedStart' AND '$formattedEnd'AND";
                } else {
        ...
        //                 WHERE $draw_Con $type_Con $datefilter $datefilter1 tl.deletes = '0' AND ur.deletes = '0' order by rand();";

        $queryNew = "SELECT tl.type, tl.id, tl.user_id, tl.product_id, tl.raffle_id, tl.ticket_id, p.rate, p.prize_one, ur.name, ur.mobile, ur.email
        FROM ticket_lines AS tl
        JOIN product AS p ON tl.product_id = p.id
        JOIN user_register AS ur ON tl.user_id = ur.id
        WHERE $draw_Con $type_Con $datefilter $datefilter1
            tl.deletes = '0' 
            AND ur.deletes = '0'
        ORDER BY RAND();";
        //exit;
        ...
        // var_dump("SELECT tl.type, tl.id, tl.user_id, tl.product_id, tl.raffle_id, tl.ticket_id, p.rate, p.prize_one, ur.name, ur.mobile, ur.email
        // FROM ticket_lines AS tl
        // JOIN product AS p ON tl.product_id = p.id
        // JOIN user_register AS ur ON tl.user_id = ur.id
        // WHERE $draw_Con $type_Con $datefilter $datefilter1
            ...
            if ($numRows > 0) {
                                        ...
                                        <table  width="470px"  id="date-table"  align="right"  style="   border-collapse: collapse !important; ">
                                                ...
                                                <tbody >
                                                    ...
                                                    <tr style="border-top:1px solid #000!important; border-left:1px solid #000; border-right:1px solid #000;">
                                                        ...
                                                        <tr  style="border-right:1px solid #000; border-left:1px solid #000;"><td class="fontCom" >Super Raffle Draw No</td>
                ...
                if ($ld->checkRole($pid)) {
                    ...
                    if ($pid == 'G') {
                        ...
                        $namefff = 'Super Raffle Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD';
                    } else if ($pid == '1') {
                        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                        $namefff = 'Super Raffle Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD Category-10AED';
                    } else if ($pid == '2') {
                        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                        $namefff = 'Super Raffle Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD Category-20AED';
                    } else if ($pid == '3') {
                        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                        $namefff = 'Super Raffle Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD Category-50AED';
                    } else if ($pid == '4') {
                        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                        $namefff = 'Super Raffle Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ')' . $tna . '-LD Category-100AED';
                    }
                } else {
                    $txts =  ($_REQUEST['pre'] == "yes") ? 'Preliminary Winner List ' : '';
                    $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
                    $namefff =  'Super Raffle Draw No #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' ' . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . $t . $tna . '-DET';
    ...
    $Ticket_lines = select_query($con, "ticket_lines", "", "$type_Con $datefilter $prizCon $datefilter1 `deletes`='0' $ranCon", "", "");
    ...
    if ($ld->checkRole($pid)) {
        ...
        if ($pid == 'G') {
            $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '  (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' super raffle ' . $txts . $tna .  '-LD';
        } else if ($pid == '1') {
            $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '  (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' super raffle ' . $txts . $tna . '-LD Category-10AED';
        } else if ($pid == '2') {
            $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';

            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '  (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' super raffle ' . $txts . $tna . '-LD Category-20AED';
        } else if ($pid == '3') {
            $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '  (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' super raffle ' . $txts . $tna . '-LD Category-50AED';
        } else if ($pid == '4') {
            $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' super raffle ' . $txts . $tna . '-LD Category-100AED';
        }
    } else {
        ...
        $txts =  ($_REQUEST['pre'] == "yes") ? ' Winner List ' : '';
        $tna =  ($ticket_name != '') ? $ticket_name : 'ALL';
        $namefff =  'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '  (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . '' . $txts . $tna . '-DED';
    }
