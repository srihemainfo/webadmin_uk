...
class ldC
{
    ...
    public function getAgentName($userID, $type)
    {
        if ($userID > 0) {
            ...
            if ($user['nr'] > 0) {
                return $type . ' (' . ucwords(strtolower($user['result'][0]['name'] . ' ' . $user['result'][0]['lname'])) . ')';
            } else {
                return $type . ' (NA)';
            }
        } else {
...
if ($winnerid != '' && $pid != '') {
    ...
    if ($first != '' && $first != '-') {
        ...
        if ($lines1['nr'] > 0) {
            ...
            for ($i = 0; $i < $lines1['nr']; $i++) {
                ...
                $GetWinneruser = select_query($con, "raffledraw", "", "`win_userid`= '$uid' AND `win_raffle_no` = '$first' ORDER BY `id` DESC LIMIT 1", "", "");
                if ($GetWinneruser['nr'] > 0) {
                    $userName = $GetWinneruser['result'][0]['winner_name'];
                } else {
    ...
    if ($ld->checkRole($pid)) {
        if ($pid == 'G') {
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Super Raffle #' . ' Winners List-LD';
        } else if ($pid == '1') {
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' .  ' Super Raffle' . ' Winners List-Category-10AED LD';
        } else if ($pid == '2') {
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Super Raffle' . ' Winners List-Category-20AED LD';
        } else if ($pid == '3') {
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Super Raffle' . ' Winners List-Category-50AED LD';
        } else if ($pid == '4') {
            $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Super Raffle' . ' Winners List-Category-100AED LD';
        }
    } else {
        $namefff = 'Super Raffle #'  . str_pad($draw['result'][0]['draw_no'], 3, "0", STR_PAD_LEFT) . ' - ' . date("dmY", strtotime($draw['result'][0]['result_datetime'])) . '' . str_replace("Draw", "", $naeme[0]) . ' (' .  date("D", strtotime($draw['result'][0]['result_datetime'])) . ') ' . ' Super Raffle' . ' Winners List ALL DED';
    }
